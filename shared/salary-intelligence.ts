...
export interface SalaryEntry {
  countryCode: string;          // ISO-2 (AE, SA, GB, …)
  monthlyMin: number;           // local currency, gross
  monthlyMax: number;           // local currency, gross
  currency: string;             // "AED", "SAR", "GBP", "CAD", "AUD", "EUR", "USD", "QAR", "BHD"
  // Approximate FX to KES at time of authoring. Refreshed quarterly.
  fxToKes: number;
  // Net send-home in KES that a typical worker manages each month after
  // accommodation, food, transport, prepaid SIM, etc.
  sendHomeMonthlyKes: number;
  // Years of experience or qualification gating typical of the upper range
  experienceFor: string;
  // How this compares to a similar role in Nairobi (multiplier)
  vsKenyaMultiplier: number;
  // Free-text career note — visa quirks, common employer types, etc.
  note?: string;
}
...
export interface SalaryRole {
  key: string;
  label: string;
  category: "healthcare" | "hospitality" | "construction" | "transport" | "skilled" | "domestic" | "education" | "casual";
  description: string;          // one-line "what is this role"
  /** Salary data per country. Missing entries = "not commonly hired from Kenya for this role". */
  entries: SalaryEntry[];
}
...
export const SUPPORTED_SALARY_COUNTRIES: Array<{ code: string; name: string; flag: string }> = [
...
export const NAIROBI_BENCHMARK_KES: Record<string, number> = {
  nurse:                  90000,
  care_worker:            40000,
  hotel_staff:            35000,
  chef_cook:              55000,
...
export function getRoleByKey(key: string): SalaryRole | undefined {
  return SALARY_ROLES.find((r) => r.key === key);
}

/**
 * Look up the salary entry for a specific (role, country) pair.
 */
export function getSalaryEntry(roleKey: string, countryCode: string): SalaryEntry | undefined {
 ...
 */
export function compareRoleAcrossCountries(roleKey: string): Array<SalaryEntry & {
  countryName: string; countryFlag: string;
  monthlyMinKes: number; monthlyMaxKes: number;
  monthlyMidKes: number;
}> {
  const role = getRoleByKey(roleKey);
  if (!role) return [];
  return role.entries
    .map((e) => {
      const countryMeta = SUPPORTED_SALARY_COUNTRIES.find((c) => c.code === e.countryCode);
