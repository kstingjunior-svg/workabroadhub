...
export interface PreDepartureStep {
  key: string;                       // stable identifier — saved in DB
  title: string;
  description: string;
  category: "documents" | "money" | "logistics" | "family" | "arrival";
  daysBefore?: number;               // recommended timing (used to sort)
  countrySpecific?: boolean;
}

// Apply to every destination
export const UNIVERSAL_PRE_DEPARTURE: PreDepartureStep[] = [
  {
...
export const COUNTRY_SPECIFIC_PRE_DEPARTURE: Record<string, PreDepartureStep[]> = {
 ...
 */
export function getPreDepartureSteps(countryCode: string): PreDepartureStep[] {
  const code = (countryCode || "").toUpperCase();
  const specific = COUNTRY_SPECIFIC_PRE_DEPARTURE[code] ?? [];
  const all = [...UNIVERSAL_PRE_DEPARTURE, ...specific];
