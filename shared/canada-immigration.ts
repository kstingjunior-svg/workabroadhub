...
export const CAD_TO_KES = 109;

export function cadToKes(cad: number): number {
  return Math.round(cad * CAD_TO_KES);
}
...
export const CRS_AGE: Record<number, { single: number; withSpouse: number }> = {
...
export const CRS_EDUCATION: Record<EducationLevel, { single: number; withSpouse: number }> = {
...
export function crsFirstLangPointsPerAbility(clb: number, withSpouse: boolean): number {
  if (clb < 4) return 0;
  if (clb === 4)  return withSpouse ? 6  : 6;
...
export function crsSecondLangPointsPerAbility(clb: number, withSpouse: boolean): number {
  if (clb < 5)  return 0;
  if (clb <= 6) return 1;
  if (clb <= 8) return 3;
  // CLB 9+
...
export const CRS_SECOND_LANG_MAX_TOTAL = { single: 22, withSpouse: 22 };

// Canadian Work Experience (full-time years in NOC TEER 0/1/2/3)
export function crsCanadianWorkPoints(years: number, withSpouse: boolean): number {
...
function eduCategory(edu: EducationLevel): 0 | 1 | 2 {
  // 0 = less than 1-year post-secondary, 1 = one credential (1+ years),
  // 2 = two credentials or 3+ year program
  if (edu === "less_than_secondary" || edu === "secondary") return 0;
  if (edu === "one_year_post_secondary") return 1;
  // bachelor, two_year, two_or_more, master, doctoral all count as 2 here
  return 2;
}

export function crsTransferabilityPoints(input: {
  education: EducationLevel;
  firstLangAvgClb: number;
  ...
  if (eduTier >= 1) {
    if (input.firstLangAvgClb >= 9) total += eduTier === 2 ? 50 : 25;
    else if (input.firstLangAvgClb >= 7) total += eduTier === 2 ? 25 : 13;
  }

  // (c) Education + Canadian Work
  const canYrs = Math.floor(input.canadianWorkYears);
  if (eduTier >= 1 && canYrs >= 1) {
    if (canYrs >= 2) total += eduTier === 2 ? 50 : 25;
    else total += eduTier === 2 ? 25 : 13;
  }
  ...
  if (fyr >= 1) {
    ...
    if (input.firstLangAvgClb >= 9) total += tier === 2 ? 50 : 25;
    else if (input.firstLangAvgClb >= 7) total += tier === 2 ? 25 : 13;
  }
  ...
  if (fyr >= 1 && canYrs >= 1) {
    ...
    if (fTier === 2 && cTier === 2) total += 50;
    else if (fTier === 2 || cTier === 2) total += 25;
    else total += 13;
  }
...
export interface CanadaProgram {
  ...
  whoForKenya: string;                 // tailored note for Kenyan applicants
  minRequirements: string[];
  pros: string[];
  cons: string[];
  recentCrsCutoffRange: string;        // e.g. "470-540"
  processingMonths: string;            // e.g. "6"
  officialUrl: string;
}

export const CANADA_PROGRAMS: CanadaProgram[] = [
  {
    key: "fsw",
    ...
    whoForKenya:
      "Kenyans with a bachelor's degree + 1 year of skilled work experience (TEER 0/1/2/3) + IELTS CLB 7+. Doctors, nurses, IT specialists, engineers, teachers all qualify.",
    minRequirements: [
  ...
  },
  {
    ...
    whoForKenya:
      "Best for Kenyans already in Canada on work or study permits — international students who graduated and worked 1+ year, IEC/working-holiday participants, and intra-company transferees.",
    minRequirements: [
  ...
  },
  {
    ...
    whoForKenya:
      "Kenyan-trained electricians, plumbers, welders, mechanics, heavy-machine operators. Tradespeople rarely apply via FST but Canadian provinces actively recruit through PNP streams.",
    minRequirements: [
  ...
  },
  {
    ...
    whoForKenya:
      "Strong if your NOC is in-demand in a specific province. Saskatchewan (SINP), Manitoba (MPNP), Alberta (AAIP), Ontario (OINP), BC (BC PNP) all have streams open to Kenyan applicants without a job offer.",
    minRequirements: [
  ...
  },
  {
    ...
    whoForKenya:
      "Best if you can secure an offer in Atlantic Canada — healthcare workers (nurses, PSWs), food processing, hospitality. Designated employers often work directly with Kenyan candidates.",
    minRequirements: [
  ...
  },
  {
    ...
    whoForKenya:
      "Niche but viable — communities like Sudbury (ON), Thunder Bay (ON), Brandon (MB) actively recruit Kenyan healthcare workers, truckers, and food-service workers.",
    minRequirements: [
  ...
  },
  {
    ...
    whoForKenya:
      "Excellent pathway for Kenyan PSWs, nannies, eldercare workers. PR on arrival (replacing the older 'caregiver visa' system that required 2 years of work first).",
    minRequirements: [
...
// ── FEES (current as of 2026, in CAD; KES via helper) ─────────────────────────
...
export const CANADA_FEES: FeeItem[] = [
  {
    key: "ee_application",
...
export function estimateCanadaTotalCAD(): number {
  return CANADA_FEES.filter((f) => f.required).reduce((sum, f) => sum + f.amountCAD, 0);
}

// ── NOC 2021 CODES Kenyans most commonly apply with ──────────────────────────

export interface NocOccupation {
  code: string;
  title: string;
  teer: 0 | 1 | 2 | 3 | 4 | 5;
  category: "healthcare" | "stem" | "trades" | "education" | "business" | "transport" | "agriculture" | "social";
  /** Was this NOC in a recent category-based Express Entry round? */
  recentCategoryDraw?: string;          // e.g. "Healthcare", "STEM"
}
...
// ── DESIGNATED ECA PROVIDERS (verified against IRCC list) ─────────────────────

export interface EcaProvider {
  key: string;
  name: string;
  scope: string;                       // what they assess
  bestFor: string;
  feeCAD: number;
  timelineWeeks: string;
  url: string;
  acceptsKenyanDocs: boolean;
}

export const ECA_PROVIDERS: EcaProvider[] = [
  {
    key: "wes",
...
export interface JobPortal {
  ...
  url: string;
  description: string;
  bestFor: string;                     // Kenyan-specific guidance
  freeToUse: boolean;
  governmentRun: boolean;
  lmiaFilter: boolean;                 // can you filter for jobs offering LMIA?
  category: "general" | "healthcare" | "tech" | "trades" | "atlantic" | "newcomer";
}

export const CANADA_JOB_PORTALS: JobPortal[] = [
  {
    key: "jobbank",
...
// ── RECENT EXPRESS ENTRY DRAWS (Approximate; for orientation) ─────────────────
// Updated periodically — see /api/canada/draws for the most recent fetch.
...
export const RECENT_DRAWS_SEED: ExpressEntryDraw[] = [
