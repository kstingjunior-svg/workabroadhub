...
const applyRateWindow = new Map();
function checkApplyRate(userId) {
    const now = Date.now();
...
setInterval(() => {
    ...
    for (const [uid, arr] of applyRateWindow.entries()) {
        ...
        if (kept.length === 0)
            applyRateWindow.delete(uid);
        else
            applyRateWindow.set(uid, kept);
    }
}, 5 * 60 * 1000).unref?.();
...
function registerLocalJobsRoutes(app) {
    ...
    app.get("/api/local-jobs/companies/:slug", async (req, res) => {
        try {
      ...
      `, [slug]);
       ...
       LEFT JOIN branches b ON b.id = j.branch_id
        ...
        `, [company.id]),
            ...
            // operates in).
            const countiesServed = Array.from(new Set(branchesRes.rows.map((b) => b.county).filter(Boolean))).sort();
    ...
    app.post("/api/local-jobs/jobs/:id/apply", applicationUpload.fields([
        ...
        `, insertParams)
            ...
            const { rows: [{ count: appsTodayCount }] } = await db_1.pool
                .query(`SELECT COUNT(*)::text AS count
               FROM local_job_applications
              WHERE applicant_user_id = $1
                AND applied_at > NOW() - INTERVAL '24 hours'`, [userId])
                .catch(() => ({ rows: [{ count: "1" }] }));
    ...
    app.post("/api/local-jobs/companies/:id/notify", async (req, res) => {
        try {
            const { id } = req.params;
            if (!/^[0-9a-f-]{8,}$/i.test(id)) {
            ...
            const { rows: [company] } = await db_1.pool.query(`SELECT id, name FROM companies WHERE id = $1 LIMIT 1`, [id]);
            if (!company)
                return res.status(404).json({ message: "Company not found." });
      ...
      `, [id, email, phone, userId, sourceJobId])
            ...
            `).catch(() => { });
                    await db_1.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_notify_company_email ON employer_notify_signups(company_id, email)`).catch(() => { });
                    return db_1.pool.query(`
              INSERT INTO employer_notify_signups (company_id, email, phone, signed_up_user_id, source_job_id)
              VALUES ($1, $2, $3, $4, $5)
            `, [id, email, phone, userId, sourceJobId]);
    ...
    async function isCompanyAdmin(userId, companyId) {
        const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS count FROM company_admins
        WHERE user_id = $1 AND company_id = $2 LIMIT 1`, [userId, companyId]).catch(() => ({ rows: [{ count: "0" }] }));
    ...
    app.get("/api/employer/me", async (req, res) => {
        ...
        try {
            const { rows: companies } = await db_1.pool.query(`
        SELECT
    ...
    app.get("/api/employer/companies/:id", async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in." });
        try {
    ...
    app.post("/api/employer/companies/:id/jobs", async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in." });
        try {
            ...
            if (!title || title.length < 3)
                return res.status(400).json({ message: "Job title is required (min 3 chars)." });
    ...
    app.patch("/api/employer/companies/:id", async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in." });
        try {
            ...
            if (logoUrl !== undefined)
                push("logo_url", logoUrl || null);
            if (industry !== undefined)
                push("industry", industry || null);
            if (county !== undefined)
                push("county", county || null);
            ...
            params.push(id);
            await db_1.pool.query(`UPDATE companies SET ${updates.join(", ")} WHERE id = $${params.length}`, params);
    ...
    app.post("/api/employer/companies/:id/branches", async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in." });
        try {
            ...
            const { rows: [branch] } = await db_1.pool.query(`
    ...
    app.patch("/api/employer/jobs/:id", async (req, res) => {
        ...
        try {
            const { id } = req.params;
            const { rows: [job] } = await db_1.pool.query(`SELECT company_id FROM local_jobs WHERE id = $1 LIMIT 1`, [id]);
            if (!job)
                return res.status(404).json({ message: "Job not found." });
            ...
            if (status) {
                await db_1.pool.query(`UPDATE local_jobs SET status = $1, updated_at = NOW() WHERE id = $2`, [status, id]);
            }
    ...
    app.patch("/api/employer/applications/:id", async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in." });
        try {
            const { id } = req.params;
            const { rows: [appRow] } = await db_1.pool.query(`SELECT company_id FROM local_job_applications WHERE id = $1 LIMIT 1`, [id]);
            if (!appRow)
            ...
            await db_1.pool.query(`UPDATE local_job_applications SET status = $1, updated_at = NOW() WHERE id = $2`, [newStatus, id]);
    ...
    app.post("/api/employer/register-company", async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in to register a company." });
        try {
            const b = req.body ?? {};
            const name = String(b.name ?? "").trim().slice(0, 160);
    ...
    app.post("/api/admin/local-jobs/claims/:id/approve", async (req, res) => {
    ...
    app.post("/api/admin/local-jobs/grant-admin", async (req, res) => {
        const adminId = await requireAdminInline(req, res);
        if (!adminId)
            return;
        try {
    ...
    app.delete("/api/admin/local-jobs/companies/:id", async (req, res) => {
        const adminId = await requireAdminInline(req, res);
        if (!adminId)
            return;
        try {
            ...
            const { rows: [company] } = await db_1.pool.query(`SELECT id, name, email, contact_name, phone, status
         FROM companies WHERE id = $1 LIMIT 1`, [id]);
            ...
            const { rowCount } = await db_1.pool.query(`DELETE FROM companies WHERE id = $1`, [id]);
            if (!rowCount) {
                // Race — someone else deleted it between our snapshot and delete
                return res.status(404).json({ message: "Company already removed." });
            }
            console.warn(`[admin] adminId=${adminId} REJECTED+DELETED company="${company.name}" ` +
                ...
                <div style="font-family:-apple-system,Segoe UI,sans-serif;max-width:560px;margin:auto;padding:24px;color:#1a2530;">
                  ...
                  <p style="margin-top:24px;font-size:13px;color:#64748b;">
                    ...
                    hello@workabroadhub.tech
                        ...
                        console.warn(`[admin/reject-delete] warning email FAILED to ${company.email}: ${mailErr?.message}`);
                    }
                })();
            }
            res.json({
    ...
    app.post("/api/admin/local-jobs/jobs", async (req, res) => {
        const adminId = await requireAdminInline(req, res);
        if (!adminId)
            return;
        try {
            ...
            const { rows: [company] } = await db_1.pool.query(`SELECT id, name FROM companies WHERE id = $1 AND status = 'approved' LIMIT 1`, [companyId]);
            if (!company) {
    ...
    app.get("/api/admin/local-jobs/seed-applicants", async (req, res) => {
        const adminId = await requireAdminInline(req, res);
        if (!adminId)
            return;
        try {
