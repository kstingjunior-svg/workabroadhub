...
exports.onPaymentSuccessForServiceOrder = onPaymentSuccessForServiceOrder;
exports.startStuckOrderSweep = startStuckOrderSweep;
exports.registerServiceOrderRoutes = registerServiceOrderRoutes;
...
async function updateOrderStatus(orderId, status, fields = {}) {
    ...
    await db_1.pool.query(`UPDATE service_orders SET status = $2, updated_at = NOW()${sets.length ? ", " + sets.join(", ") : ""} WHERE id = $1`, [orderId, status, ...values]);
}
...
async function processOrder(orderId) {
    try {
        ...
        let userMessage = "";
        if (config.needsCv && order.cv_text)
            userMessage += `Here is the user's CV:\n\n${order.cv_text.slice(0, 6000)}\n\n`;
        if (order.target_country)
            userMessage += `TARGET COUNTRY (include country-appropriate conventions in the output): ${order.target_country}\n\n`;
        if (order.job_description)
            userMessage += `TARGET JOB / ROLE the candidate is applying for (tailor the CV to match these keywords + requirements):\n${order.job_description.slice(0, 2000)}\n\n`;
...
` : "";
        ...
        if ((isCvRevamp || isCvHeavy) && inputLen > 800) {
            ...
            if (ratio < MIN_RATIO || ratio > MAX_RATIO) {
                console.warn(`[quality-guard] orderId=${orderId} ratio=${ratio.toFixed(2)} ` +
                ...
                try {
                    ...
                    if (retryOutput) {
                        ...
                        if (retryRatio >= MIN_RATIO && retryRatio <= MAX_RATIO) {
                            console.warn(`[quality-guard] orderId=${orderId} retry SUCCESS ratio=${retryRatio.toFixed(2)}`);
                            output = cleanedRetry;
                        }
                        else {
                 ...
                 WHERE id = $1`, [
                            ...
                            try {
                                ...
                                console.warn(`[quality-guard] orderId=${orderId} needs manual outreach — ` +
       ...
       WHERE id = $1`, [orderId, output, passedRatio, aiOutputPayload]);
        ...
        console.error(`[ServiceOrder] processOrder PERMANENT error for ${orderId}: ` +
            `status=${status} code=${code} msg="${errMsg}" — marking failed`);
        await db_1.pool.query(`UPDATE service_orders SET status = 'failed', error_message = $2, updated_at = NOW() WHERE id = $1`, [orderId, errMsg]).catch(() => { });
 ...
 */
async function notifyOrderFailed(orderId, technicalReason) {
    try {
        const { rows } = await db_1.pool.query(`SELECT user_id, service_name, service_slug, amount FROM service_orders WHERE id = $1`, [orderId]);
        const order = rows[0];
        if (!order)
            return;
        const { rows: userRows } = await db_1.pool.query(`SELECT email, phone, first_name FROM users WHERE id = $1`, [order.user_id]);
        ...
        if (user.phone) {
            try {
                const { sendWhatsApp } = await Promise.resolve().then(() => __importStar(require("./services/whatsapp")));
                const waMessage = `😔 *We couldn't finish your ${serviceName}, ${firstName}.*\n\n` +
...
async function onPaymentSuccessForServiceOrder(orderId) {
    await db_1.pool.query(`UPDATE service_orders SET status = 'paid', paid_at = NOW(), updated_at = NOW() WHERE id = $1`, [orderId]);
...
function startStuckOrderSweep() {
    if (stuckSweepStarted)
        return;
          ...
          LIMIT 25`);
            ...
            for (const row of rows) {
                ...
                SET status = 'failed',
                    ...
                    updated_at = NOW()
              WHERE id = $1 AND status IN ('paid', 'processing')`, [row.id]).catch(() => { });
                    ...
                    notifyOrderFailed(row.id, `Exhausted ${MAX_RETRIES_PER_ORDER} recovery retries — likely upstream AI outage`).catch(() => { });
                    continue;
                }
                retries.set(row.id, attempts + 1);
                console.log(`[ServiceOrder] Recovery: retrying ${row.id} (${row.service_slug}) attempt ${attempts + 1}`);
                processOrder(row.id).catch((e) => console.error(`[ServiceOrder] Recovery retry failed for ${row.id}:`, e?.message));
            }
        }
        catch (err) {
...
function registerServiceOrderRoutes(app, isAuthenticated) {
    ...
    app.get("/api/services/order/:orderId/download/:format", async (req, res) => {
        try {
            ...
            const stripLeadingServiceTitle = (raw, serviceName, slug) => {
