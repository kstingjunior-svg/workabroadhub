...
function maybeTrack(toolName) {
    return async (req, _res, next) => {
        try {
            const userId = req.user?.id ?? null;
            await storage_1.storage.recordToolUsage({ userId, toolName, metadata: null });
        }
        catch { }
        next();
    };
...
async function runAIEngine(text) {
    try {
...
"""`;
        ...
        return {
            ...
            aiVerdict: (["SAFE", "SUSPICIOUS", "LIKELY SCAM"].includes(parsed.verdict) ? parsed.verdict : "SUSPICIOUS"),
            aiExplanation: typeof parsed.explanation === "string" ? parsed.explanation : "",
            aiFlags: Array.isArray(parsed.flags) ? parsed.flags.filter((f) => typeof f === "string") : [],
...
async function analyzeScamHybrid(text) {
...
function generateTemplatePlaceholder(template) {
...
function registerToolsRoutes(app, isAuthenticated, isAdmin) {
    // ════════════════════════════════════════════════════════════════════════════
    // STEP 1: ATS CV CHECKER
    // POST /api/tools/ats-check
    // ════════════════════════════════════════════════════════════════════════════
    app.post("/api/tools/ats-check", upload.single("cv"), maybeTrack("ats_cv_checker"), async (req, res) => {
        try {
            if (!req.file) {
            ...
            const { ATS_ANALYSIS_ENGINE } = await Promise.resolve().then(() => __importStar(require("./lib/ats-analysis-engine")));
            let aiMessages;
            ...
            if (cvText.trim().length >= extract_text_1.MIN_CV_LENGTH) {
                ...
                return res.status(422).json({
                    message: "We couldn't extract text from your CV. " +
            ...
            const completion = await openai_1.openai.chat.completions.create(completionOptions, {
                timeout: 45000,
            });
            let aiResult = {};
            try {
            ...
            const atsPlanId = userId ? (await storage_1.storage.getUserPlan(userId) || "free").toLowerCase() : "free";
            ...
            if (userId && cvText.trim().length >= extract_text_1.MIN_CV_LENGTH) {
                storage_1.storage.upsertUserCareerProfile(userId, {
                    parsedCvText: cvText.slice(0, 12000), // cap at ~12k chars — ample for any CV
            ...
            try {
                const webUser = await storage_1.storage.getUserById(userId);
                if (webUser?.email) {
                    const { scheduleCvEmailSequence } = await Promise.resolve().then(() => __importStar(require("./cv-email-sequence")));
    ...
    // ════════════════════════════════════════════════════════════════════════════
    app.post("/api/util/extract-cv-text", upload.single("cv"), async (req, res) => {
        try {
            if (!req.file) {
                return res.status(400).json({ message: "No file uploaded. Please upload a PDF or DOCX." });
            }
    ...
    // ════════════════════════════════════════════════════════════════════════════
    app.post("/api/tools/scam-check", maybeTrack("job_scam_checker"), async (req, res) => {
        try {
            const { text } = req.body;
    ...
    }, maybeTrack("job_scam_checker"), async (req, res) => {
        try {
            if (!req.file) {
            ...
            if (mimetype === "application/pdf" ||
                ...
                const extracted = await (0, extract_text_1.extractTextFromBuffer)(buffer, mimetype).catch(() => ({ text: "", method: "error" }));
                    ...
                    const safeType = mimetype.includes("gif") ? "image/gif"
                        : mimetype.includes("png") ? "image/png"
                            : mimetype.includes("webp") ? "image/webp"
                                : "image/jpeg";
                    const visionRes = await openai_1.openai.chat.completions.create({
                        model: "gpt-4o",
                        messages: [
                            {
                                role: "user",
                ...
                if (!extractedText || extractedText.length < 10) {
                    try {
                        ...
                        if (extractedText)
                            console.log("[ScamFileCheck] Tesseract OCR succeeded:", extractedText.length, "chars");
                    }
                    catch (ocrErr) {
            ...
            if (extractedText.trim().length >= 150) {
                ...
                const forceAnalyzeJARaw = req.body?.forceAnalyze ?? req.query?.forceAnalyze;
    ...
    // ════════════════════════════════════════════════════════════════════════════
    app.post("/api/tools/scam-feedback", async (req, res) => {
        try {
            ...
            if (wasScam && advertText && typeof advertText === "string") {
                try {
                    await storage_1.storage.createScamReport({
    ...
    // ════════════════════════════════════════════════════════════════════════════
    app.post("/api/tools/job-assistant", upload.single("cv"), maybeTrack("job_assistant"), async (req, res) => {
        try {
            const userId = req.user?.id ?? null;
            if (!userId) {
            ...
            if (usageCount >= 1 && !isPremium) {
                return res.status(402).json({
                    ...
                    usageCount,
                    message: "You've used your 1 free generation. Upgrade to WorkAbroad Hub Premium for unlimited AI-powered applications.",
                });
            }
            let cvText = "";
            if (req.file) {
            ...
            const toolType = req.body.toolType ?? "cover_letter";
...
Return ONLY this JSON (no markdown): { "content": "<numbered Q&A with each question and answer, sections separated by \\n\\n>", "suggestions": ["<tone tip>", "<STAR method tip>", "<salary negotiation tip>"] }`,
            ...
            const validTypes = ["cover_letter", "cv_optimize", "application_answers"];
            const selectedType = validTypes.includes(toolType)
                ? toolType
                : "cover_letter";
            ...
            let aiResult = { content: "", suggestions: [] };
            try {
                const raw = completion.choices[0]?.message?.content ?? "{}";
            ...
            await storage_1.storage.recordToolUsage({
                userId,
    ...
    app.get("/api/jobs/sponsorship", async (req, res) => {
        try {
            ...
            const jobs = await storage_1.storage.getVisaJobs({ country, category });
            res.json(jobs);
        }
        catch (err) {
    ...
    app.post("/api/admin/jobs", isAuthenticated, isAdmin, async (req, res) => {
        try {
            const data = schema_1.insertJobSchema.parse(req.body);
            const job = await storage_1.storage.createJob(data);
    ...
    app.patch("/api/admin/jobs/:id", isAuthenticated, isAdmin, async (req, res) => {
        try {
            const job = await storage_1.storage.updateJob(req.params.id, req.body);
            res.json(job);
        }
        catch (err) {
    ...
    app.delete("/api/admin/jobs/:id", isAuthenticated, isAdmin, async (req, res) => {
        try {
            await storage_1.storage.deleteJob(req.params.id);
            res.json({ success: true });
        }
        catch (err) {
            res.status(500).json({ message: err.message });
    ...
    app.get("/api/templates", (_req, res) => {
        res.json(exports.CV_TEMPLATES.map(({ fileUrl: _, ...t }) => t));
    });
    app.get("/api/templates/download/:templateId", async (req, res) => {
        try {
            const template = exports.CV_TEMPLATES.find((t) => t.id === req.params.templateId);
            if (!template)
                return res.status(404).json({ message: "Template not found." });
            // Track download (Step 5 & 6)
            const userId = req.user?.id ?? null;
            await storage_1.storage.recordTemplateDownload({ templateId: template.id, userId }).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            await storage_1.storage.recordToolUsage({ userId, toolName: "cv_templates", metadata: { templateId: template.id } }).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
    ...
    // ════════════════════════════════════════════════════════════════════════════
    app.post("/api/jobs/match-upload", isAuthenticated, (req, res, next) => {
        ...
        try {
            ...
            const matchPlanId = (await storage_1.storage.getUserPlan(matchUserId) || "free").toLowerCase();
            if (matchPlanId === "free") {
            ...
            const extracted = await (0, extract_text_1.extractTextFromBuffer)(req.file.buffer, req.file.mimetype, req.file.originalname).catch((e) => {
            ...
            if (!cvTextU || cvTextU.trim().length < extract_text_1.MIN_CV_LENGTH) {
            ...
            const matchModule = await Promise.resolve().then(() => __importStar(require("./services/jobMatchingService")));
            ...
            await storage_1.storage
                ...
                .catch((e) => { console.warn("[job_match_upload] usage record:", e?.message); });
    ...
    app.post("/api/jobs/match", isAuthenticated, async (req, res) => {
        try {
            ...
            const { getJobMatches } = await Promise.resolve().then(() => __importStar(require("./services/jobMatchingService")));
