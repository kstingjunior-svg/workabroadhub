...
exports.resolveSupabaseUuidFromPhone = resolveSupabaseUuidFromPhone;
exports.syncSubscriptionToSupabase = syncSubscriptionToSupabase;
exports.upgradeUserToPro = upgradeUserToPro;
exports.isUserPro = isUserPro;
exports.downgradeSupabaseUser = downgradeSupabaseUser;
exports.downgradeExpiredSupabaseSubscriptions = downgradeExpiredSupabaseSubscriptions;
exports.confirmPaymentInSupabase = confirmPaymentInSupabase;
exports.recordCommission = recordCommission;
exports.matchPaymentToUser = matchPaymentToUser;
exports.createServiceRequest = createServiceRequest;
exports.isFraudUser = isFraudUser;
exports.logCvUpload = logCvUpload;
exports.updateCvUpload = updateCvUpload;
exports.updateLatestCvUpload = updateLatestCvUpload;
exports.logPayout = logPayout;
exports.reconcilePayout = reconcilePayout;
exports.trackReferral = trackReferral;
exports.extendOrCreate = extendOrCreate;
exports.grantAccess = grantAccess;
exports.hasSubscription = hasSubscription;
exports.hasAccess = hasAccess;
exports.incrementPromoUsageInSupabase = incrementPromoUsageInSupabase;
exports.syncPaymentToSupabase = syncPaymentToSupabase;
exports.syncUserToSupabase = syncUserToSupabase;
...
if (RAW_SUPABASE_URL !== supabaseUrl) {
    console.warn(`[Supabase] SUPABASE_URL was malformed ("${RAW_SUPABASE_URL}") — using "${supabaseUrl}" instead. ` +
...
function getClient() {
    ...
    if (!_client) {
        try {
            _client = (0, supabase_js_1.createClient)(supabaseUrl, supabaseKey, {
                auth: { persistSession: false },
            });
        }
        catch (err) {
            _clientInitFailed = true;
            console.error(`[Supabase] createClient() failed with url="${supabaseUrl}": ${err?.message ?? err}`);
            return null;
        }
    }
    return _client;
...
async function resolveSupabaseUuidFromPhone(phone) {
    try {
        const client = getClient();
        if (!client)
            return null;
 ...
 */
async function syncSubscriptionToSupabase(opts) {
    try {
        const client = getClient();
        if (!client) {
        ...
        if (error) {
            ...
            console.log(`[Supabase] Subscription synced: user=${opts.user_id} plan=${opts.plan_id} provider=${opts.provider} auto_renew=${opts.auto_renew}`);
        }
    }
    catch (err) {
...
async function upgradeUserToPro(user_id) {
    try {
...
async function downgradeSupabaseUser(user_id) {
    try {
        const client = getClient();
        if (!client)
            return;
        const { error } = await client
            ...
            .eq("status", "active");
        if (error)
            console.error("[Supabase] downgradeSupabaseUser error:", error);
        else
            console.log(`[Supabase] Subscription expired for user ${user_id}`);
    }
    catch (err) {
...
async function downgradeExpiredSupabaseSubscriptions() {
    try {
        const client = getClient();
        if (!client)
            return 0;
        ...
        if (count > 0)
            console.log(`[Supabase] Bulk sweep: expired ${count} subscription(s)`);
        return count;
    }
    catch (err) {
...
async function confirmPaymentInSupabase(mpesaCode, phone, amount) {
    try {
        const client = getClient();
        if (!client)
            return;
 ...
 */
async function recordCommission(userId, paymentId, amount, rate = 0.10) {
    const client = getClient();
    if (!client)
        return;
    try {
        ...
        console.log(`[Commission] KES ${commissionAmount} (${rate * 100}%) pending for referrer=${user.referred_by} | payment=${paymentId}`);
    }
    catch (err) {
 ...
 */
async function matchPaymentToUser(payment, user) {
    const client = getClient();
    if (!client)
        return { success: false, alreadyMatched: false, error: "Supabase client unavailable" };
    ...
    const serviceId = payment.service_id ?? payment.serviceId ?? null;
    ...
    try {
        ...
        recordCommission(userId, paymentId, amount).catch((e) => console.error("[matchPaymentToUser] Commission error:", e?.message));
        // 5 — Service request (fire-and-forget)
        createServiceRequest(userId, serviceId, paymentId).catch((e) => console.error("[matchPaymentToUser] ServiceRequest error:", e?.message));
        console.log(`[matchPaymentToUser] ✅ payment=${payment.id} → user=${userId} ` +
 ...
 */
async function createServiceRequest(userId, serviceId, paymentId) {
    if (!serviceId)
        return; // subscription-only payments carry no service_id
    const client = getClient();
    if (!client)
        return;
    try {
 ...
 */
async function isFraudUser(userId) {
    const client = getClient();
    if (!client)
        return false; // fail open — never block a legitimate user on a DB outage
    try {
        const { data, error } = await client
            .from("users")
 ...
 */
async function logCvUpload(opts) {
    const client = getClient();
    if (!client)
        return null;
    let fileUrl = null;
 ...
 */
async function updateCvUpload(cvUploadId, improvedCv, score) {
    const client = getClient();
    if (!client)
        return;
    try {
 ...
 */
async function updateLatestCvUpload(userId, improvedCv, score) {
    const client = getClient();
    if (!client)
        return;
    try {
...
async function logPayout(opts) {
    ...
    try {
        ...
        console.log(`[logPayout] ✅ id=${data?.id} phone=${opts.phone} amount=${opts.amount} ` +
 ...
 */
async function reconcilePayout(conversationId, status, opts = {}) {
    const client = getClient();
    if (!client)
        return;
    try {
 ...
 */
async function trackReferral(refCodeFromURL, newUserId, newUserPhone) {
    const client = getClient();
    if (!client || !refCodeFromURL?.trim())
        return;
    try {
        ...
        await client
            ...
            .then(({ error }) => {
            if (error)
                console.warn("[Referral] referred_by update skipped:", error.message);
        });
 ...
 */
async function extendOrCreate(userId, serviceId, days = 30, paymentId = "manual") {
    const client = getClient();
    if (!client)
        return null;
    ...
    try {
        ...
        if (existing) {
            ...
            const { error } = await client
                ...
                .eq("id", existing.id);
            if (error) {
                console.error("extendOrCreate update error:", error.message);
                return null;
            }
        ...
        if (error) {
            console.error("extendOrCreate insert error:", error.message);
            return null;
        }
 ...
 */
async function grantAccess(userId, serviceId, expiresAt, paymentId = "manual") {
    const client = getClient();
    if (!client)
        return false;
    try {
 ...
 */
async function hasSubscription(userId, serviceId) {
    const client = getClient();
    if (!client)
        return false;
    try {
 ...
 */
async function hasAccess(userId, serviceId) {
    const client = getClient();
    if (!client)
        return false;
    try {
 ...
 */
async function incrementPromoUsageInSupabase(code) {
    try {
        const client = getClient();
        if (!client)
            return;
