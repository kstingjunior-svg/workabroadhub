...
function registerWriteFromScratchRoutes(app) {
     ...
     */
    app.get("/api/write-from-scratch/mine", async (req, res) => {
        try {
            const userId = currentUserId(req);
            if (!userId)
                return res.json({ drafts: [], count: 0 });
            const limit = Math.min(Math.max(Number(req.query.limit ?? 10), 1), 50);
            const { rows } = await db_1.pool.query(`SELECT id, doc_type, status,
     ...
     */
    app.get("/api/write-from-scratch/:id/body", async (req, res) => {
        try {
            const { rows } = await db_1.pool.query(`SELECT id, user_id, doc_type, status, output_body, error_message
           FROM write_from_scratch_drafts
          WHERE id = $1`, [req.params.id]);
            if (rows.length === 0)
                return res.status(404).json({ error: "Not found" });
