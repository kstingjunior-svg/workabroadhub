...
async function seedHubPartnerAgenciesIfEmpty() {
    const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS c FROM hub_partner_agencies`);
    ...
    for (const a of agencies) {
        await db_1.pool.query(`INSERT INTO hub_partner_agencies (name, blurb, country_focus_iso2, role_focus, spotlight_fee_per_month_kes, referral_fee_per_lead_kes, verified_at, spotlight_active_from, spotlight_active_until)
       VALUES ($1,$2,$3,$4,$5,$6, NOW(), CURRENT_DATE, CURRENT_DATE + INTERVAL '30 days')`, [a.name, a.blurb, a.countries, a.roles, a.spotlight, a.fee]);
    }
...
function registerHubRoutes(app) {
    ...
    app.get("/api/hub/me", async (req, res) => {
        const userId = sessionUserId(req);
        if (!userId)
            return res.json({ signedIn: false, tier: "free" });
        try {
            const { rows } = await db_1.pool.query(`SELECT COALESCE(hub_subscription_tier, 'free') AS tier, hub_vault_renews_at AS vault_renews_at, email, first_name
    ...
    app.get("/api/hub/partner-agencies", async (req, res) => {
        try {
            ...
            const { rows } = await db_1.pool.query(`SELECT id, name, blurb, logo_url, contact_whatsapp, contact_email, country_focus_iso2, role_focus
    ...
    app.post("/api/hub/partner-agencies/:id/connect", async (req, res) => {
        const userId = sessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in to connect." });
        try {
            const agencyId = String(req.params.id);
            const journeyId = req.body?.journeyId ? String(req.body.journeyId) : null;
            const { rows: aRows } = await db_1.pool.query(`SELECT id, name, contact_whatsapp, contact_email, referral_fee_per_lead_kes FROM hub_partner_agencies WHERE id = $1 AND is_active = true LIMIT 1`, [agencyId]);
            const agency = aRows[0];
            if (!agency)
                return res.status(404).json({ message: "Agency not found." });
            // Record a 'contacted' lead — this is the billable event.
            await db_1.pool.query(`INSERT INTO hub_lead_transactions (user_id, partner_agency_id, journey_id, referral_fee_kes, status)
         VALUES ($1,$2,$3,$4,'contacted')`, [userId, agencyId, journeyId, agency.referral_fee_per_lead_kes]);
            res.json({
                agency: {
                    name: agency.name,
    ...
    app.post("/api/hub/journeys/:id/unlock-score", async (req, res) => {
        const userId = sessionUserId(req);
        if (!userId)
            return res.status(401).json({ message: "Please sign in." });
        try {
            const { rowCount } = await db_1.pool.query(`UPDATE hub_user_journeys SET full_score_unlocked_at = COALESCE(full_score_unlocked_at, NOW()), updated_at = NOW()
          WHERE id = $1 AND user_id = $2`, [String(req.params.id), userId]);
            if (!rowCount)
                return res.status(404).json({ message: "Journey not found." });
