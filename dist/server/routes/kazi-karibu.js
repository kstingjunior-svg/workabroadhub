...
function registerKaziKaribuRoutes(app, isAuthenticated, isAdmin) {
     ...
     */
    app.post("/api/kazi-karibu/posts/:id/interest", requireKaziKaribuEnabled, isAuthenticated, async (req, res) => {
        const postId = String(req.params.id);
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Please sign in first." });
        try {
            ...
            const { rows: postRows } = await db_1.pool.query(`SELECT id, poster_user_id, moderation_state
             FROM kazi_karibu_posts WHERE id = $1 LIMIT 1`, [postId]);
            const post = postRows[0];
            if (!post)
            ...
            //    them until reveal.
            const { rows: userRows } = await db_1.pool.query(`SELECT first_name, last_name, email, phone, city, country
             FROM users WHERE id = $1 LIMIT 1`, [userId]);
            const u = userRows[0];
            if (!u)
                return res.status(404).json({ error: "Profile not found." });
            // 3. Best-effort career profile (may not exist for every user).
            const { rows: careerRows } = await db_1.pool.query(`SELECT headline, summary FROM user_career_profiles WHERE user_id = $1 LIMIT 1`, [userId]).catch(() => ({ rows: [] }));
            ...
            const { rows: interestRows } = await db_1.pool.query(`INSERT INTO kazi_karibu_interest
     ...
     */
    app.get("/api/kazi-karibu/posts/:id/interests", requireKaziKaribuEnabled, isAuthenticated, async (req, res) => {
        const postId = String(req.params.id);
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Please sign in first." });
        try {
            ...
            const { rows: postRows } = await db_1.pool.query(`SELECT poster_user_id FROM kazi_karibu_posts WHERE id = $1 LIMIT 1`, [postId]);
            if (postRows.length === 0)
                return res.status(404).json({ error: "Post not found." });
            if (postRows[0].poster_user_id !== userId) {
                return res.status(403).json({ error: "Only the poster can view applicants." });
            }
            const { rows } = await db_1.pool.query(`SELECT id, applicant_user_id, message, shared_profile_snapshot,
     ...
     */
    app.post("/api/kazi-karibu/interests/:id/reveal-contact", requireKaziKaribuEnabled, isAuthenticated, async (req, res) => {
        ...
        try {
            ...
            const { rows } = await db_1.pool.query(`SELECT i.id,
                  i.post_id,
                  p.poster_user_id,
                  u.phone      AS poster_phone,
                  u.email      AS poster_email,
     ...
     */
    app.post("/api/kazi-karibu/interests/:id/report", requireKaziKaribuEnabled, isAuthenticated, async (req, res) => {
        const interestId = String(req.params.id);
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Please sign in first." });
        try {
            const { rows } = await db_1.pool.query(`SELECT i.applicant_user_id, p.poster_user_id
             FROM kazi_karibu_interest i
             JOIN kazi_karibu_posts   p ON p.id = i.post_id
            WHERE i.id = $1 LIMIT 1`, [interestId]);
            const rec = rows[0];
            ...
            await db_1.pool.query(`UPDATE kazi_karibu_interest
              SET reported = true,
                  report_reason = $2,
                  reported_at = NOW()
            WHERE id = $1`, [interestId, reason]);
     ...
     */
    app.get("/api/kazi-karibu/interests/mine", requireKaziKaribuEnabled, isAuthenticated, async (req, res) => {
        const userId = readSessionUserId(req);
        if (!userId)
            return res.status(401).json({ error: "Please sign in first." });
        try {
