...
const neaimsSync_1 = require("../nea/neaimsSync");
function registerAdminNeaSyncRoutes(app, isAuthenticated, isAdmin) {
     ...
     */
    app.post("/api/admin/nea-sync/run", isAuthenticated, isAdmin, async (req, res) => {
        const adminId = req.user?.claims?.sub ?? req.user?.id ?? "unknown";
        try {
            const result = await (0, neaimsSync_1.runNeaimsSync)({
                triggeredBy: "admin",
     ...
     */
    app.get("/api/admin/nea-sync/runs", isAuthenticated, isAdmin, async (req, res) => {
        const limit = Math.min(Math.max(Number(req.query.limit ?? 20), 1), 200);
        try {
            const { rows } = await db_1.pool.query(`SELECT id, triggered_by, triggered_by_user_id,
     ...
     */
    app.get("/api/admin/nea-sync/latest", isAuthenticated, isAdmin, async (_req, res) => {
        try {
            const { rows } = await db_1.pool.query(`SELECT id, triggered_by, started_at, finished_at, duration_ms,
