...
function initWebSocketServer(httpServer, sessionParser) {
    ...
    userWss = new ws_1.WebSocketServer({ noServer: true });
    ...
    visitorWss = new ws_1.WebSocketServer({ noServer: true });
    visitorWss.on("connection", (ws, req) => {
        ...
        let attachedVisitorId = null;
        ws.on("message", (raw) => {
            try {
                const msg = JSON.parse(raw.toString());
                if (msg.type === "identify" && typeof msg.visitorId === "string" && msg.visitorId.length > 0 && msg.visitorId.length < 100) {
                    if (attachedVisitorId)
                        return; // already attached this connection
                    attachedVisitorId = msg.visitorId;
                    (0, presence_1.attachVisitor)(attachedVisitorId);
                }
            }
            catch { /* ignore malformed */ }
        });
        // Send the current count immediately so the page renders without waiting
        ws.send(JSON.stringify({ type: "presence_count", total: (0, presence_1.getVisitorCount)(), timestamp: Date.now() }));
        ws.on("close", () => {
            if (attachedVisitorId)
                (0, presence_1.detachVisitor)(attachedVisitorId);
            const cur = ipConnectionCount.get(ip) ?? 0;
            if (cur <= 1)
                ipConnectionCount.delete(ip);
            else
                ipConnectionCount.set(ip, cur - 1);
        });
        ws.on("error", () => { });
    });
    ...
    let visitorBroadcastTimer = null;
    const broadcastVisitorCount = () => {
        if (!visitorWss)
            return;
        const payload = JSON.stringify({
            type: "presence_count",
            total: (0, presence_1.getVisitorCount)(),
            timestamp: Date.now(),
        });
    ...
    (0, presence_1.subscribeVisitorCount)(() => {
        if (visitorBroadcastTimer)
            return;
    ...
    httpServer.on("upgrade", (req, socket, head) => {
        ...
        if (!path.startsWith("/ws/"))
            return;
        let target = null;
        if (path === "/ws/analytics")
            target = wss;
        else if (path === "/ws/user")
            target = userWss;
        else if (path === "/ws/presence-count")
            target = visitorWss;
        if (!target) {
