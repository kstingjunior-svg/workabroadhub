...
const nanjila_1 = require("./ai/nanjila");
...
Crawl-delay: 1`);
    ...
    app.get("/api/services", async (req, res) => {
      ...
      `);
            ...
            console.log(`✅ SERVICES LOADED FROM DB: ${rows.map((s) => `${s.code}=KES${s.finalPrice}${s.isFlashSale ? `(FLASH-${s.discountPercent}%OFF)` : ""}`).join(", ")}`);
    ...
    app.post("/api/mpesa/callback", async (req, res) => {
        try {
            ...
            const existingPayment = await db_1.pool.query(`SELECT id FROM payments WHERE mpesa_code = $1 LIMIT 1`, [mpesaCode]);
            if (existingPayment.rows.length === 0) {
                await db_1.pool.query(`INSERT INTO payments (mpesa_code, phone, amount, status)
           VALUES ($1, $2, $3, 'completed')`, [mpesaCode, phone, amount]);
            }
            // 👤 2. Find or create user
            let userRes = await db_1.pool.query(`SELECT id FROM users WHERE phone = $1 LIMIT 1`, [phone]);
            let userId;
            if (userRes.rows.length === 0) {
                const newUser = await db_1.pool.query(`INSERT INTO users (phone, email)
           ...
           RETURNING id`, [phone]);
            ...
            const existingSub = await db_1.pool.query(`SELECT id FROM user_subscriptions WHERE user_id = $1 AND status = 'active' ORDER BY end_date DESC NULLS LAST LIMIT 1`, [userId]);
            if (existingSub.rows.length > 0) {
                await db_1.pool.query(`UPDATE user_subscriptions SET status = 'active', end_date = $1, updated_at = NOW() WHERE id = $2`, [expiresAt, existingSub.rows[0].id]);
            }
            else {
                await db_1.pool.query(`INSERT INTO user_subscriptions (user_id, plan, status, end_date) VALUES ($1, 'pro', 'active', $2)`, [userId, expiresAt]);
            }
...
If your instinct says "3,500", STOP and re-read the SERVICES block above.`;
    ...
    app.post("/api/voice/process", async (req, res) => {
        ...
        const user = await db_1.pool.query("SELECT * FROM users WHERE phone = $1", [caller]);
        const lang = user.rows[0]?.language || (0, utils_1.detectLanguage)(userSpeech);
        const voice = (0, utils_1.getVoice)(lang);
        const intent = (0, router_1.detectIntent)(userSpeech);
        let reply = "";
        if (intent === "payment") {
            ...
            reply = await (0, nanjila_1.nanjilaAgent)(null, userSpeech);
        }
        const twiml = new VoiceResponse();
