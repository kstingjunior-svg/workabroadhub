...
exports.startNEASyncScheduler = startNEASyncScheduler;
const db_1 = require("../../db");
const index_1 = require("./index");
const CHECK_INTERVAL_MS = 10 * 60 * 1000; // 10 minutes
let started = false;
function startNEASyncScheduler() {
    if (started)
        return;
    started = true;
...
async function tick() {
    try {
        const now = new Date();
        if (!isInSyncWindow(now))
            return;
        if (await syncedThisWeek())
            return;
        console.log("[nea-sync] weekly window hit — starting auto-sync");
        const summary = await (0, index_1.runAutoSync)("cron");
        console.log(`[nea-sync] weekly sync ${summary.status}: ` +
...
function isInSyncWindow(d) {
    const dayUTC = d.getUTCDay(); // 0=Sunday
    const hourUTC = d.getUTCHours();
    const minUTC = d.getUTCMinutes();
    if (dayUTC !== 0)
        return false; // Sunday UTC = Monday EAT after 03:00 EAT... but we want early Monday EAT which is Sunday 23:xx UTC
    if (hourUTC !== 23)
        return false;
    return minUTC < 30;
}
async function syncedThisWeek() {
