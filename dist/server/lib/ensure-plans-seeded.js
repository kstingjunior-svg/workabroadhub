...
exports.getCanonicalPlanPrice = getCanonicalPlanPrice;
/**
...
async function ensurePlansSeeded() {
    ...
    for (const p of PLAN_SEEDS) {
        try {
            await db_1.pool.query(`INSERT INTO plans (plan_id, plan_name, price, billing_period, description, display_order, currency, is_active, features, metadata, created_at, updated_at)
         ...
         ON CONFLICT (plan_id) DO UPDATE
           SET plan_name      = EXCLUDED.plan_name,
               ...
               is_active      = true,
               updated_at     = NOW()`, [p.planId, p.planName, p.price, p.billingPeriod, p.description, p.displayOrder]);
        }
        catch (err) {
 ...
 */
function getCanonicalPlanPrice(planId) {
