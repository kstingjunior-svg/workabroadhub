...
exports.getActiveSmtpProfile = getActiveSmtpProfile;
...
async function sendWithFailover(args) {
    ...
    if (resendAttempt.error !== "Resend not configured") {
        console.warn(`[email] Resend send failed to ${args.to}: ${resendAttempt.errorCode || ""} ${resendAttempt.error?.slice(0, 200)} — trying SMTP fallback`);
    }
    ...
    const finalOutcome = resendAttempt.error === "Resend not configured" ? smtpAttempt : resendAttempt;
