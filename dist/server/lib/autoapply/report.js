...
async function sendDailyDigest({ agent, matches }) {
    if (matches.length === 0)
        return;
    const { rows } = await db_1.pool.query(`SELECT email, first_name FROM users WHERE id = $1`, [agent.user_id]);
    const user = rows[0];
    if (!user?.email)
        return;
    ...
    await (0, email_1.sendEmail)({ to: user.email, subject, html, text });
}
function renderHtml(args) {
    const rowsHtml = args.matches.map((m, idx) => {
        ...
        const reasons = (m.match_reasons ?? []).slice(0, 2).map((r) => `<span style="color:#64748b;font-size:12px;">• ${escapeHtml(r)}</span>`).join(" &nbsp;");
