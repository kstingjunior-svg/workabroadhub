...
exports.collectConcurrencySnapshot = collectConcurrencySnapshot;
...
// operational signal).
const eventLoopMonitor = (0, node_perf_hooks_1.monitorEventLoopDelay)({ resolution: 10 });
eventLoopMonitor.enable();
// ─────────────────────────────────────────────────────────────────────────────
// Main entry point
// ─────────────────────────────────────────────────────────────────────────────
async function collectConcurrencySnapshot() {
    const errors = [];
...
async function collectDatabaseStats() {
    ...
    let slow = 0;
    try {
        const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS n
...
const KNOWN_QUEUES = [
    {
        ...
        loader: async () => {
            try {
                ...
                const mod = await Promise.resolve().then(() => __importStar(require("../jobs/nightlyReadiness")));
    ...
    },
    {
        ...
        loader: async () => {
            try {
                const mod = await Promise.resolve().then(() => __importStar(require("../../lib/cvQueue")));
...
function zeroQueue(name, reachable) {
    return {
...
async function tableExists(name) {
    try {
        const { rows } = await db_1.pool.query(`SELECT EXISTS (
...
// ─────────────────────────────────────────────────────────────────────────────
function deriveBottlenecks(proc, db, queues, ai) {
    const out = [];
    // ── Process ────────────────────────────────────────────────────────────
