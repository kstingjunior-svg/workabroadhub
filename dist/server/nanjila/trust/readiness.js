...
exports.computeReadinessReport = computeReadinessReport;
exports.persistReadinessSnapshot = persistReadinessSnapshot;
const db_1 = require("../../db");
// ─────────────────────────────────────────────────────────────────────────────
// The nine scores
// ─────────────────────────────────────────────────────────────────────────────
async function scoreCvStrength(userId) {
    const { rows } = await db_1.pool.query(`SELECT
       ...
       (SELECT delivered_at FROM cv_fingerprints
          WHERE user_id = $1 ORDER BY delivered_at DESC LIMIT 1) AS delivered_at`, [userId]);
...
async function scoreApplicationReadiness(userId) {
    const { rows } = await db_1.pool.query(`SELECT
       COUNT(*)::text AS apps,
...
async function scoreScamAwareness(userId) {
    // Signals: has run the scam checker / offer checker / visa checker? has
    // read scam alerts? has viewed the /verify-us page?
    const { rows: usageRows } = await db_1.pool.query(`SELECT COUNT(*)::text AS n FROM tool_usage
      WHERE user_id = $1 AND tool_name IN ('job_scam_checker', 'offer_check', 'visa_check')`, [userId]).catch(() => ({ rows: [] }));
...
async function scoreDocumentCompleteness(userId) {
    const { rows } = await db_1.pool.query(`SELECT
       ...
       (SELECT kyc_verified FROM users WHERE id = $1 LIMIT 1) AS kyc_verified`, [userId]).catch(() => ({ rows: [] }));
    const r = rows[0] ?? {};
    const hasCv = !!r.cv;
...
async function scoreVerificationStatus(userId) {
    const { rows } = await db_1.pool.query(`SELECT email_verified AS email, phone_verified AS phone
       FROM users WHERE id = $1 LIMIT 1`, [userId]).catch(() => ({ rows: [] }));
    const r = rows[0] ?? {};
...
async function scoreCountryReadiness(userId) {
    const { rows } = await db_1.pool.query(`SELECT progress_pct AS progress, country_code AS country
...
async function scoreLanguageReadiness(userId) {
    const { rows } = await db_1.pool.query(`SELECT english_level AS level
       FROM user_career_profiles
      WHERE user_id = $1 LIMIT 1`, [userId]).catch(() => ({ rows: [] }));
...
async function scoreInterviewReadiness(userId) {
    const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS n FROM interview_sessions
      WHERE user_id = $1`, [userId]).catch(() => ({ rows: [] }));
 ...
 */
async function computeReadinessReport(userId) {
    const [cv, application, scam, documents, verification, country, language, interview] = await Promise.all([
    ...
    const overallRaw = cv.score * OVERALL_WEIGHTS.cv +
 ...
 */
async function persistReadinessSnapshot(report) {
    await db_1.pool.query(`INSERT INTO nanjila_readiness_snapshots
...
function pickTopNextAction(scores) {
...
function collectNextActions(report) {
    return [
