...
exports.runCommissionBatch = runCommissionBatch;
exports.startCommissionScheduler = startCommissionScheduler;
exports.stopCommissionScheduler = stopCommissionScheduler;
exports.setCommissionSchedulerEnabled = setCommissionSchedulerEnabled;
exports.getCommissionSchedulerStatus = getCommissionSchedulerStatus;
...
// ── Core batch runner ────────────────────────────────────────────────────────
async function runCommissionBatch() {
    const result = {
        processed: 0,
        succeeded: 0,
        skipped: 0,
    ...
    const commissions = (pending ?? [])
        .filter(c => (c.retry_count ?? 0) < MAX_AUTO_RETRIES)
        .slice(0, BATCH_SIZE);
    console.log(`[CommissionScheduler] ${commissions.length} pending commission(s). Processing up to ${BATCH_SIZE}.`);
    for (const commission of commissions) {
        ...
        try {
            ...
            if (commission.amount < 100) {
                result.skipped++;
                console.warn(`[CommissionScheduler] Commission ${commission.id} amount KES ${commission.amount} < 100 — skipping micro-payout`);
                continue;
            }
            // Resolve referrer from Supabase users table (phone + fraud/verification flags)
            const { data: referrer } = await supabaseClient_1.supabase
                .from("users")
            ...
            if (!referrer?.phone) {
                result.skipped++;
                console.warn(`[CommissionScheduler] No phone for referrer=${commission.referrer_user_id} ` +
            ...
            if (referrer.suspected_fraud) {
                result.skipped++;
                console.warn(`[CommissionScheduler] Referrer ${commission.referrer_user_id} flagged suspected_fraud — ` +
            ...
            if (!referrer.phone_verified) {
                result.skipped++;
                console.warn(`[CommissionScheduler] Referrer ${commission.referrer_user_id} phone not verified — ` +
                    `commission ${commission.id} held`);
                continue;
            }
            // Fire M-Pesa B2C
            const payoutResult = await (0, mpesa_1.b2cPayout)(referrer.phone, commission.amount, `WorkAbroad Referral Commission - ${commission.id}`);
            ...
            result.succeeded++;
            state.totalPaid++;
            console.log(`[CommissionScheduler] B2C initiated — commission=${commission.id} ` +
            ...
            if (nextRetry >= MAX_AUTO_RETRIES) {
                await supabaseClient_1.supabase
                    ...
                    .eq("id", commission.id);
                console.warn(`[CommissionScheduler] Commission ${commission.id} marked FAILED after ${nextRetry} attempt(s)`);
            }
            else {
                await supabaseClient_1.supabase
                    .from("commissions")
...
function startCommissionScheduler() {
    ...
    console.log(`[CommissionScheduler] Started — interval: ${SCHEDULER_INTERVAL_MS / 1000}s, ` +
        `max retries: ${MAX_AUTO_RETRIES}, batch size: ${BATCH_SIZE}`);
}
function stopCommissionScheduler() {
    if (schedulerTimer) {
        clearInterval(schedulerTimer);
        schedulerTimer = null;
    }
}
function setCommissionSchedulerEnabled(enabled) {
    state.enabled = enabled;
    console.log(`[CommissionScheduler] ${enabled ? "Enabled" : "Disabled"}`);
}
function getCommissionSchedulerStatus() {
    return {
        ...state,
