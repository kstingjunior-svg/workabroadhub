 ...
 */
async function enqueue(phone, message, opts = {}) {
    const source = opts.source ?? "manual";
    const delayMs = opts.delayMs ?? 0;
    const sendAfter = new Date(Date.now() + delayMs);
    try {
        const dedupCutoff = new Date(Date.now() - DEDUP_WINDOW_HOURS * 3600000);
        const dup = await db_2.pool.query(`SELECT id FROM whatsapp_queue
       WHERE phone = $1 AND source = $2 AND status = 'pending'
         AND created_at > $3
       LIMIT 1`, [phone, source, dedupCutoff]);
        if (dup.rowCount && dup.rowCount > 0) {
            console.log(`[WaQueue] enqueue skip — dup pending (${source}) for ${phone}`);
            return;
        }
        await db_1.db.insert(schema_1.whatsappQueue).values({
...
async function isSpamBlocked(phone) {
    const cutoff = new Date(Date.now() - SPAM_WINDOW_HOURS * 3600000);
    const res = await db_2.pool.query(`SELECT id FROM whatsapp_queue
     WHERE phone = $1 AND status = 'sent' AND sent_at > $2
     LIMIT 1`, [phone, cutoff]);
...
async function processQueue() {
    ...
    try {
        ...
        const { rows } = await db_2.pool.query(`SELECT id, phone, message, source, retry_count
       FROM whatsapp_queue
       WHERE status = 'pending' AND send_after <= $1
       ORDER BY send_after ASC
       LIMIT $2`, [now, BATCH_SIZE]);
        if (rows.length === 0)
            return;
        ...
        for (const row of rows) {
            ...
            if (await isSpamBlocked(row.phone)) {
                ...
                const reschedule = new Date(Date.now() + SPAM_WINDOW_HOURS * 3600000);
                await db_2.pool.query(`UPDATE whatsapp_queue SET send_after = $1 WHERE id = $2`, [reschedule, row.id]);
            ...
            // ── Attempt send ────────────────────────────────────────────────────
            try {
                await (0, whatsapp_1.sendWhatsApp)(row.phone, row.message);
                await db_2.pool.query(`UPDATE whatsapp_queue
           ...
           WHERE id = $1`, [row.id]);
             ...
             WHERE id = $3`, [newRetries, String(err.message).slice(0, 500), row.id]);
                    console.error(`[WaQueue] ✗  Permanently failed (${row.source}) → ${row.phone} ` +
                        `after ${newRetries} attempt(s): ${err.message}`);
                }
                else {
                    // Exponential back-off: 5 min × 2^attempt
                    const backoffMs = 5 * 60000 * Math.pow(2, row.retry_count);
                    const retryAt = new Date(Date.now() + backoffMs);
                    await db_2.pool.query(`UPDATE whatsapp_queue
             SET retry_count = $1, send_after = $2, error_msg = $3
             WHERE id = $4`, [newRetries, retryAt, String(err.message).slice(0, 500), row.id]);
                    console.warn(`[WaQueue] ⚠  Retry ${newRetries}/${MAX_RETRIES} (${row.source}) → ${row.phone} ` +
        ...
        console.log(`[WaQueue] Cycle complete — sent: ${sent}, skipped: ${skipped}, ` +
...
// ── Abandonment scanner ───────────────────────────────────────────────────────
async function scanAbandonedPayments() {
    try {
        const { rows } = await db_2.pool.query(`SELECT DISTINCT ON (u.phone)
              u.phone,
              u.id                                               AS user_id,
...
async function scanAbandonedApplications() {
    try {
        const { rows } = await db_2.pool.query(`SELECT DISTINCT ON (u.phone)
              u.phone,
              u.id                                               AS user_id,
...
function startWhatsappQueueProcessor() {
    console.log("[WaQueue] Processor started — " +
