...
exports.normalizeKenyaPhone = normalizeKenyaPhone;
 ...
 */
async function getCachedLookup(phone) {
    try {
        const { rows } = await db_1.pool.query(`SELECT phone, valid, carrier_name, line_type, checked_at
       FROM phone_lookups
...
async function cacheLookup(phone, valid, carrier, lineType) {
    try {
        await db_1.pool.query(`INSERT INTO phone_lookups (phone, valid, carrier_name, line_type, checked_at)
       VALUES ($1, $2, $3, $4, NOW())
       ON CONFLICT (phone) DO UPDATE
         SET valid = EXCLUDED.valid,
             carrier_name = EXCLUDED.carrier_name,
             line_type = EXCLUDED.line_type,
             checked_at = NOW()`, [phone, valid, carrier, lineType]);
    }
    catch (err) {
 ...
 */
async function callTwilioLookup(phone) {
