...
function registerVisaJobsRoutes(app, isAuthenticated) {
    ...
    app.get("/api/visa-jobs/:id/apply", isAuthenticated, async (req, res) => {
        ...
        if (!userId) {
            if (wantsHtml)
                return res.redirect(302, "/login?redirect=" + encodeURIComponent("/tools/visa-sponsorship-jobs"));
            return res.status(401).json({ message: "Please sign in." });
        }
        const hasAccess = await userHasPaidAccess(userId);
        if (!hasAccess) {
            if (wantsHtml)
                return res.redirect(302, "/pricing?from=visa-jobs-apply");
        ...
        if (!url) {
            if (wantsHtml)
                return res.redirect(302, "/tools/visa-sponsorship-jobs?error=job-not-found");
