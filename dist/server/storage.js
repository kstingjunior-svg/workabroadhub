...
class DatabaseStorage {
    ...
    async activateUserPlan(userId, planId, paymentId, expiresAt, options) {
        ...
        if (!planId || !CANONICAL_TIERS.has(String(planId).toLowerCase())) {
            const err = new Error(`[activateUserPlan] BLOCKED non-canonical planId="${planId}" for userId=${userId} ` +
        ...
        try {
         ...
         RETURNING *`, [userId, paymentId || null, planId, finalExpiry]);
            ...
            if (resetMode === "reset" && currentExpiry && currentExpiry > now) {
                console.log(`[activateUserPlan] RESET "${planId}" for userId=${userId} ` +
