...
exports.sendProActivationEmail = sendProActivationEmail;
...
async function sendProActivationEmail(to, firstName, expiresAt, transactionRef) {
...
async function sendAccountDeletedEmail(to, firstName) {
    const name = escapeHtml(firstName || "there");
    const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px;">
      <div style="text-align:center;margin-bottom:24px;">
...
async function sendDeadlineReminderEmail(to, firstName, job) {
    const name = escapeHtml((firstName || "").trim() || "there");
    const jobTitle = escapeHtml(job.title);
    const company = escapeHtml(job.company);
    const deadlineStr = escapeHtml(job.deadline.toLocaleDateString("en-KE", { weekday: "long", year: "numeric", month: "long", day: "numeric" }));
