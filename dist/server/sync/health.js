...
exports.nextHealthState = nextHealthState;
exports.performHealthProbe = performHealthProbe;
...
function nextHealthState(current, outcome) {
    if (outcome.ok) {
        return { status: "healthy", consecutiveFailures: 0 };
    }
    const next = current.consecutiveFailures + 1;
    let status = current.status;
    if (next >= BROKEN_THRESHOLD)
        status = "broken";
    else if (next >= DEGRADED_THRESHOLD)
        status = "degraded";
 ...
 */
async function performHealthProbe(provider, timeoutMs = HEALTH_CHECK_TIMEOUT_MS) {
    let timer;
 ...
 * transition.
 */
async function readHealthState(providerId) {
    const { rows } = await db_1.pool.query(`SELECT health FROM sync_providers WHERE id = $1 LIMIT 1`, [providerId]);
    const status = (rows[0]?.health ?? "unknown");
    const consecutiveFailures = status === "broken" ? BROKEN_THRESHOLD :
 ...
 * client argument.
 */
async function runHealthCheck(provider, providerId, buffer, flushClient) {
    ...
    await db_1.pool.query(`UPDATE sync_providers
        SET health = $2,
            last_health_check_at = NOW(),
            updated_at = NOW()
      WHERE id = $1`, [providerId, after.status]);
    const transitioned = before.status !== after.status;
    if (transitioned) {
        (0, events_1.emitProviderHealthChanged)(buffer, {
            providerId,
