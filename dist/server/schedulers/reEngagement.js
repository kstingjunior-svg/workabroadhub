...
function getMessage(serviceName) {
...
async function markSent(userId, event = 're_engagement_sent') {
    await db_1.pool.query(`INSERT INTO funnel_events (user_id, event, metadata)
     VALUES ($1, $2, $3)`, [userId, event, JSON.stringify({ sent_at: new Date().toISOString() })]);
}
// ── Score-based nudge ─────────────────────────────────────────────────────────
...
async function runScoredNudge() {
    ...
    `);
        ...
        for (const user of res.rows) {
            await (0, whatsapp_1.sendWhatsApp)(user.phone, `🔥 You're close! Finish your application today and get priority support.`);
...
async function runReEngagement() {
    try {
        ...
        if (!abandonedUsers.length)
            return;
        console.log(`[ReEngagement] Sending to ${abandonedUsers.length} abandoned user(s)`);
        for (const user of abandonedUsers) {
            await (0, whatsapp_1.sendWhatsApp)(user.phone, getMessage(user.service_name));
