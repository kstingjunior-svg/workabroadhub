...
function getWeightValue(weights, factorName, defaultWeight) {
    const w = weights.find(w => w.factorName === factorName);
    return (w && w.isActive) ? w.weight : defaultWeight;
}
async function calculateAgencyScore(agencyId, triggeredBy = "system") {
    const existingScore = await storage_1.storage.getAgencyScore(agencyId);
    ...
    const recentEvents = await storage_1.storage.getRecentComplianceEvents(agencyId, 12);
    ...
    let paymentRaw = 0;
    try {
        const renewals = await storage_1.storage.getLicenseRenewalPaymentsByAgency(agencyId);
        const verified = renewals.filter((p) => p.paymentStatus === "completed").length;
        const unverified = renewals.filter((p) => p.paymentStatus === "pending" || p.paymentStatus === "failed").length;
        if (verified > 0)
            paymentRaw = 1;
    ...
    let govRaw = 0;
    try {
        const overrides = await storage_1.storage.getManualOverrides({ limit: 1000 });
        const agencyOverride = overrides.find((o) => o.agencyId === agencyId);
        if (agencyOverride && agencyOverride.syncStatus === "synced") {
            govRaw = 1;
        }
        else {
            const syncLogs = await storage_1.storage.getGovernmentSyncLogs({ status: "success", limit: 100 });
            const hasAgencySync = syncLogs.some((log) => log.licenseNumber === agency.licenseNumber && (log.status === "success"));
            if (hasAgencySync)
                govRaw = 1;
        }
    }
    ...
    try {
        ...
        const agencyReports = reports.filter((r) => r.agencyId === agencyId || r.agencyName === agency.agencyName);
    ...
    const saved = await storage_1.storage.upsertAgencyScore(scoreData);
    if (previousScore !== overallScore || previousTier !== tier) {
        await storage_1.storage.createScoreHistory({
            agencyId,
            previousScore,
...
async function recalculateAllScores(triggeredBy = "system") {
