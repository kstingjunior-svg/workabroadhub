...
exports.SALARY_TABLES = exports.OCCUPATION_KEYS = void 0;
exports.getSalaryTable = getSalaryTable;
exports.classifyJobTitle = classifyJobTitle;
 ...
 */
function getSalaryTable(countryCode) {
    if (!countryCode)
        return null;
    return exports.SALARY_TABLES.find((t) => t.countryCode === countryCode.toUpperCase()) ?? null;
}
/**
...
function classifyJobTitle(jobTitle) {
    ...
    if (/nurse|nursing|rn\b/.test(t))
        return "nurse";
    if (/care\s?(giver|worker|assistant|aide)|carer|senior\s?carer/.test(t))
        return "care_worker";
    if (/domestic|housemaid|nanny|maid|househelp/.test(t))
        return "domestic_worker";
    if (/driver|chauffeur|hgv|lgv/.test(t))
        return "driver";
    if (/security|guard/.test(t))
        return "security_guard";
    if (/cleaner|janitor|housekeep/.test(t))
        return "cleaner_janitor";
    if (/waiter|waitress|server|f&b|barista/.test(t))
        return "hospitality_waiter";
    if (/chef|cook|kitchen/.test(t))
        return "hospitality_chef";
    if (/manager|supervisor/.test(t) && /hospitality|restaurant|hotel/.test(t))
        return "hospitality_manager";
    ...
    if (/warehouse|forklift|picker|packer|operative/.test(t))
        return "warehouse_operative";
    if (/farm|harvest|agri|picker\b.*fruit/.test(t))
        return "farm_worker";
    if (/office|admin|secretary|clerk|reception/.test(t))
        return "office_admin";
    if (/sales|business development/.test(t))
        return "sales";
    if (/it\b|software|developer|engineer(?!ing)|programmer|analyst\b.*data|it\s?professional/.test(t))
        return "it_professional";
    if (/engineer/.test(t))
        return "engineer";
    if (/teacher|tutor|instructor|lecturer/.test(t))
        return "teacher";
    if (/technician|lab\b/.test(t))
