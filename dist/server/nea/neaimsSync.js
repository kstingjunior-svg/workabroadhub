...
exports.runNeaimsSync = runNeaimsSync;
const db_1 = require("../db");
const neaimsClient_1 = require("./neaimsClient");
const neaimsNormalize_1 = require("./neaimsNormalize");
/**
 ...
 */
const NEAIMS_SYNC_LOCK_KEY = 4127301502;
/**
 ...
 */
async function runNeaimsSync(opts) {
    const t0 = Date.now();
    const counts = {
        verifiedFetched: 0,
        expiredFetched: 0,
    ...
    const lockRes = await db_1.pool.query(`SELECT pg_try_advisory_lock($1::bigint) AS locked`, [NEAIMS_SYNC_LOCK_KEY]);
    ...
    try {
        ...
        try {
            const { rows } = await db_1.pool.query(`INSERT INTO nea_sync_runs (triggered_by, triggered_by_user_id, status)
         VALUES ($1, $2, 'running')
         RETURNING id`, [opts.triggeredBy, opts.triggeredByUserId ?? null]);
            runId = rows[0].id;
        }
        catch (err) {
        ...
        try {
            ...
            verifiedRows = await (0, neaimsClient_1.fetchNeaimsAgencies)(true);
            invalidRows = await (0, neaimsClient_1.fetchNeaimsAgencies)(false);
        }
        catch (err) {
            const code = err instanceof neaimsClient_1.NeaimsClientError ? err.code : "FETCH_UNKNOWN";
        ...
        for (const r of invalidRows) {
            if (r.instStatus === "LICENSE_EXPIRED")
                counts.expiredFetched++;
            else if (r.instStatus === "LICENSE_DEREGISTERED")
                counts.deregisteredFetched++;
            else if (r.instStatus === "LICENSE_PENDING")
                counts.pendingFetched++;
        }
        // ── 3. Normalize ───────────────────────────────────────────────────────
        const verifiedNorm = (0, neaimsNormalize_1.normalizeNeaimsBatch)(verifiedRows, "verified");
        const invalidNorm = (0, neaimsNormalize_1.normalizeNeaimsBatch)(invalidRows, "invalid");
        counts.skippedJunk = verifiedNorm.skippedJunk + invalidNorm.skippedJunk;
        // Combine drop-reason maps for the return value.
        const skipReasons = {};
        for (const [reason, n] of verifiedNorm.skipReasons)
            skipReasons[reason] = (skipReasons[reason] ?? 0) + n;
        for (const [reason, n] of invalidNorm.skipReasons)
            skipReasons[reason] = (skipReasons[reason] ?? 0) + n;
        // ── 4. Dedupe (verified wins over expired for same license number) ────
        const merged = (0, neaimsNormalize_1.dedupeByLicenseNumber)([...verifiedNorm.clean, ...invalidNorm.clean]);
        ...
        try {
            ...
            const orphanResult = await client.query(`UPDATE nea_agencies
            SET status_override = 'unlisted',
                last_updated    = NOW(),
                updated_by      = 'neaims-sync'
          WHERE license_number NOT IN (SELECT unnest($1::text[]))
            AND status_override IS DISTINCT FROM 'unlisted'`, [licenseNumbers]);
        ...
        const message = `NEAIMS sync ${finalStatus}: ` +
        ...
        await db_1.pool.query(`SELECT pg_advisory_unlock($1::bigint)`, [NEAIMS_SYNC_LOCK_KEY]).catch(err => console.warn("[NEAIMS sync] Failed to release advisory lock:", err?.message));
    }
}
// ─────────────────────────────────────────────────────────────────────────────
// Helpers
...
async function upsertBatch(client, batch) {
    ...
    batch.forEach((r, idx) => {
        ...
        values.push(r.agencyName, r.licenseNumber, r.email, r.serviceType, r.issueDate, r.expiryDate, r.statusOverride, // null / 'expired' / 'revoked'
        // last_updated → NOW() in SQL (can't parameterise it usefully with defaults)
        new Date(), "neaims-sync");
    });
    const sql = `
 ...
 */
async function finishRun(runId, status, counts, extra) {
    try {
        await db_1.pool.query(`UPDATE nea_sync_runs SET
         finished_at          = NOW(),
         status               = $2,
         verified_fetched     = $3,
         expired_fetched      = $4,
