...
exports.normalizeNeaimsBatch = normalizeNeaimsBatch;
exports.dedupeByLicenseNumber = dedupeByLicenseNumber;
 ...
 * 34 no-status").
 */
function normalizeNeaimsBatch(rows, bucket) {
    const clean_ = [];
    ...
    for (const row of rows) {
        ...
        if (TEST_REGNOS.has(regno)) {
            bump("test_regno");
            continue;
        }
        if (JUNK_REGNO_PATTERNS.some(p => p.test(regno))) {
            bump("junk_regno_pattern");
            continue;
        }
        if (name.length < 4) {
            bump("short_name");
        ...
        if (JUNK_NAME_PATTERNS.some(p => p.test(name))) {
            bump("junk_name_pattern");
            continue;
        }
 ...
 * Hart.
 */
function dedupeByLicenseNumber(rows) {
