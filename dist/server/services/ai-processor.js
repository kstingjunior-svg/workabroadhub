...
exports.processOrderWithAI = processOrderWithAI;
exports.processAndDeliverOrder = processAndDeliverOrder;
...
const COUNTRY_MOTIVATION_LETTER_STANDARDS = {
    "USA": `
...
function getCountryStandards(targetCountry) {
    const country = targetCountry?.toLowerCase() || "";
    if (country.includes("australia"))
        return COUNTRY_CV_STANDARDS["Australia"];
    if (country.includes("usa") || country.includes("united states") || country.includes("america"))
        return COUNTRY_CV_STANDARDS["USA"];
    if (country.includes("canada"))
        return COUNTRY_CV_STANDARDS["Canada"];
    if (country.includes("uk") || country.includes("united kingdom") || country.includes("britain"))
        return COUNTRY_CV_STANDARDS["UK"];
    if (country.includes("uae") || country.includes("arab") || country.includes("emirates") || country.includes("dubai"))
        return COUNTRY_CV_STANDARDS["UAE"];
    if (country.includes("europe") || country.includes("germany") || country.includes("france") || country.includes("netherlands"))
        return COUNTRY_CV_STANDARDS["Europe"];
    if (country.includes("turkey") || country.includes("türkiye") || country.includes("istanbul"))
        return COUNTRY_CV_STANDARDS["Turkey"];
    return "";
}
function getSOPStandards(targetCountry) {
    const country = targetCountry?.toLowerCase() || "";
    if (country.includes("australia"))
        return COUNTRY_SOP_STANDARDS["Australia"];
    if (country.includes("usa") || country.includes("united states") || country.includes("america"))
        return COUNTRY_SOP_STANDARDS["USA"];
    if (country.includes("canada"))
        return COUNTRY_SOP_STANDARDS["Canada"];
    if (country.includes("uk") || country.includes("united kingdom") || country.includes("britain"))
        return COUNTRY_SOP_STANDARDS["UK"];
    if (country.includes("uae") || country.includes("arab") || country.includes("emirates") || country.includes("dubai"))
        return COUNTRY_SOP_STANDARDS["UAE"];
    if (country.includes("europe") || country.includes("germany") || country.includes("france") || country.includes("netherlands"))
        return COUNTRY_SOP_STANDARDS["Europe"];
    if (country.includes("turkey") || country.includes("türkiye") || country.includes("istanbul"))
        return COUNTRY_SOP_STANDARDS["Turkey"];
    return "";
}
function getMotivationLetterStandards(targetCountry) {
    const country = targetCountry?.toLowerCase() || "";
    if (country.includes("australia"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["Australia"];
    if (country.includes("usa") || country.includes("united states") || country.includes("america"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["USA"];
    if (country.includes("canada"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["Canada"];
    if (country.includes("uk") || country.includes("united kingdom") || country.includes("britain"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["UK"];
    if (country.includes("uae") || country.includes("arab") || country.includes("emirates") || country.includes("dubai"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["UAE"];
    if (country.includes("europe") || country.includes("germany") || country.includes("france") || country.includes("netherlands"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["Europe"];
    if (country.includes("turkey") || country.includes("türkiye") || country.includes("istanbul"))
        return COUNTRY_MOTIVATION_LETTER_STANDARDS["Turkey"];
    return "";
}
...
function detectHallucinations(content, clientInfo) {
...
`;
async function processOrderWithAI(order) {
    try {
...
async function runQualityCheck(content, serviceType, clientInfo) {
    try {
        ...
        const missingSections = failConditions.some((fc) => fc.toLowerCase().includes("missing") || fc.toLowerCase().includes("section"));
        const aiHallucinationDetected = failConditions.some((fc) => fc.toLowerCase().includes("hallucination") ||
        ...
        if (pages > 4 ||
            ...
            if (pages > 4)
                issues.push(`FAIL: CV exceeds 4 pages (${pages} pages detected) - ATS risk`);
            if (missingSections)
                issues.push("FAIL: Missing key sections - Low quality");
            ...
            if (languageScore < 0.75)
                issues.push(`FAIL: Language quality below threshold (${Math.round(languageScore * 100)}%) - Reputation risk`);
        }
        else {
        ...
        return {
            ...
            passed,
            issues: [...issues, ...failConditions.map((fc) => `CONDITION: ${fc}`)],
...
async function processAndDeliverOrder(orderId) {
    try {
        ...
        if (result.qualityCheck?.passed) {
            ...
            await storage_1.storage.updateServiceOrder(orderId, { ...updateData, userNotifiedAt: new Date() });
            ...
            if (userPhone) {
                const waMsg = `🎉 *Your ${order.serviceName} is Ready!*\n\n` +
                    `${preview}${hasMoreContent ? "...\n\n_(Full document available on dashboard)_" : ""}\n\n` +
                    `━━━━━━━━━━━━━━━━━━━━\n` +
                    `📎 *View Full Document:*\n${dashboardUrl}\n\n` +
                    `💬 Reply *Revise* for changes\n` +
                    `📞 Need help? WhatsApp us any time`;
                (0, whatsapp_1.sendWhatsApp)(userPhone, waMsg).catch((err) => console.error(`[AIProcessor] WhatsApp delivery failed for order ${orderId}:`, err.message));
            }
            // Email delivery
          ...
          `,
                }).catch((err) => console.error(`[AIProcessor] Email delivery failed for order ${orderId}:`, err.message));
            }
            ...
            await storage_1.storage.updateServiceOrder(orderId, updateData);
...
async function processQueue() {
    ...
    try {
        ...
        const orders = await storage_1.storage.getServiceOrders({ status: "processing" });
