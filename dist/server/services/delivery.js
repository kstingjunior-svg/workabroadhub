...
exports.deliverService = deliverService;
// @ts-nocheck
const db_1 = require("../db");
const whatsapp_1 = require("./whatsapp");
const storage_1 = require("../storage");
const cvQueue_1 = require("../lib/cvQueue");
...
function resolveSlug(payment) {
    // 1. Explicit slug field
    if (payment.serviceSlug)
        return String(payment.serviceSlug).toLowerCase();
...
async function deliverService(payment, user) {
    const slug = resolveSlug(payment);
    const phone = user.phone ?? payment.phone ?? "";
    const firstName = user.firstName ?? user.first_name ?? "";
    const lastName = user.lastName ?? user.last_name ?? "";
    const name = [firstName, lastName].filter(Boolean).join(" ") || user.email?.split("@")[0] || "there";
    const amount = Number(payment.amount ?? 0);
    // The raw serviceId stored on the payment — used as the key in user_services.
    const rawServiceId = payment.serviceId ?? payment.service_id ?? payment.planId ?? payment.plan_id ?? slug;
    ...
    switch (slug) {
        ...
        case "cv_service":
        case "cv_services": {
            await cvQueue_1.cvQueue.add("generate_cv", { userId: user.id, phone }, { attempts: 3, backoff: { type: "exponential", delay: 10000 } });
            await (0, whatsapp_1.sendWhatsApp)(phone, `✅ Payment Confirmed — KES ${amount.toLocaleString()} received!\n\nHi ${name}, your ATS-optimised CV is being generated and will be sent to this WhatsApp within minutes.\n\nThank you for choosing WorkAbroad Hub 🌍`).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "info",
                title: "CV Generation Started",
                message: "Your ATS CV is being generated and will be delivered to your WhatsApp shortly.",
        ...
        case "interview_coaching": {
            await (0, whatsapp_1.sendWhatsApp)(phone, `🎯 Interview Coaching Activated!\n\nHi ${name}, thank you for your payment of KES ${amount.toLocaleString()}.\n\nNanjila will reach out shortly on WhatsApp to schedule your session.\n\nPlease reply with:\n1️⃣ Target Job Role\n2️⃣ Target Country\n\nGet ready to land your overseas job! 💼`).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "success",
                title: "Interview Coaching Activated",
                message: "Check your WhatsApp — Nanjila will reach out to schedule your coaching session.",
            }).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            break;
        }
        // ─── 3. JOB ALERTS SUBSCRIPTION ──────────────────────────────────────────
        case "job_alerts": {
            await db_1.pool.query(`UPDATE users SET job_alerts_active = true WHERE id = $1`, [user.id]);
            await (0, whatsapp_1.sendWhatsApp)(phone, `🚀 Job Alerts Activated!\n\nHi ${name}, you'll now receive verified international jobs directly on WhatsApp.\n\nPayment of KES ${amount.toLocaleString()} confirmed ✅\n\nStay ready — your next opportunity is coming!`).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "success",
                title: "Job Alerts Activated",
                message: "You'll now receive verified international job alerts directly on WhatsApp.",
            }).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            break;
        }
        ...
        case "basic":
        case "starter": {
            const planLabel = slug === "pro" ? "Pro" : slug === "basic" ? "Basic" : "Starter";
            ...
            try {
                ...
                cvText = await (0, cv_1.generateCV)(user, careerProfile ?? null);
                // Persist so the dashboard can display it
                await db_1.pool.query(`UPDATE users SET generated_cv = $1 WHERE id = $2`, [cvText, user.id]);
            ...
            await (0, whatsapp_1.sendWhatsApp)(phone, waMessage).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "success",
        ...
        case "visa_guide":
        case "visa": {
            await (0, whatsapp_1.sendWhatsApp)(phone, `📄 Visa Guide Ready!\n\nHi ${name}, thank you for your payment of KES ${amount.toLocaleString()} ✅\n\nYour comprehensive visa guide is now available in your WorkAbroad Hub dashboard under *My Services*.\n\nFor any questions, reply to this message and Nanjila will assist you 🤝`).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "success",
                title: "Visa Guide Unlocked",
                message: "Your visa guide is available in your dashboard under My Services.",
            }).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            break;
        }
        ...
        case "consultation":
        case "consult": {
            await (0, whatsapp_1.sendWhatsApp)(phone, `📞 Consultation Booked!\n\nHi ${name}, your payment of KES ${amount.toLocaleString()} has been received ✅\n\nOur team will contact you within 24 hours on WhatsApp to confirm your consultation slot.\n\nIf you have any questions before then, simply reply to this message.\n\n— WorkAbroad Hub Team 🌍`).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "success",
                title: "Consultation Confirmed",
                message: "Our team will contact you within 24 hours to confirm your consultation slot.",
            }).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            break;
        }
        // ─── 7. CV FIX LITE ──────────────────────────────────────────────────────
        ...
        case "ats_cv_optimization_v2": {
            await cvQueue_1.cvQueue.add("generate_cv", { userId: user.id, phone }, { attempts: 3, backoff: { type: "exponential", delay: 10000 } });
            await (0, whatsapp_1.sendWhatsApp)(phone, `✅ ATS CV Optimization Confirmed — KES ${amount.toLocaleString()} received!\n\nHi ${name}, your ATS-optimised CV is being generated and will be sent to this WhatsApp within minutes.\n\n— WorkAbroad Hub 🌍`).catch((err) => { console.error('[deliverService] WhatsApp failed:', { error: err?.message, timestamp: new Date().toISOString() }); });
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "info",
        ...
        default: {
            ...
            if (phone) {
                await (0, whatsapp_1.sendWhatsApp)(phone, `✅ Payment Confirmed!\n\nHi ${name}, your payment of KES ${amount.toLocaleString()} has been received.\n\nYour service is being processed. Our team will reach out if any further action is needed.\n\n— WorkAbroad Hub 🌍`).catch((err) => { console.error('[] Unhandled rejection:', { error: err?.message, timestamp: new Date().toISOString() }); });
            }
            storage_1.storage.createUserNotification({
                userId: user.id,
                type: "info",
