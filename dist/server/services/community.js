...
exports.fetchRoomsSummary = fetchRoomsSummary;
exports.adminFetchRecent = adminFetchRecent;
...
function isValidRoomSlug(s) {
    return exports.ROOMS.some((r) => r.slug === s);
}
// ─── Schema setup ────────────────────────────────────────────────────────────
...
async function ensureSchema() {
    ...
    `);
        ...
        for (const r of exports.ROOMS) {
            await db_1.pool.query(`INSERT INTO chat_rooms (slug, name, description) VALUES ($1, $2, $3)
           ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name, description = EXCLUDED.description`, [r.slug, r.name, r.description]);
        }
...
async function hasReferredPayingFriend(userId) {
    try {
        ...
        const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS cnt FROM referrals
         WHERE ref_code = $1 AND status IN ('paid','processing','completed','sent')`, [refCode]);
...
async function getOrInitQuota(userId) {
    await ensureSchema();
    const today = new Date().toISOString().slice(0, 10); // UTC YYYY-MM-DD
    const { rows } = await db_1.pool.query(`SELECT posts_today, day_bucket::text FROM chat_post_quota WHERE user_id = $1`, [userId]);
    if (!rows.length || rows[0].day_bucket !== today) {
        // Either no row, or it's a stale bucket — reset.
        await db_1.pool.query(`INSERT INTO chat_post_quota (user_id, day_bucket, posts_today)
         VALUES ($1, $2::date, 0)
       ON CONFLICT (user_id) DO UPDATE SET day_bucket = EXCLUDED.day_bucket, posts_today = 0, updated_at = NOW()`, [userId, today]);
        return 0;
    }
    return rows[0].posts_today;
}
async function bumpQuota(userId) {
    await db_1.pool.query(`UPDATE chat_post_quota SET posts_today = posts_today + 1, updated_at = NOW() WHERE user_id = $1`, [userId]);
}
const lastPostAt = new Map(); // userId -> epoch ms
async function checkEligibility(userId) {
    if (!userId)
        return { canPost: false, reason: "not_signed_in", tier: "none" };
...
async function postMessage(userId, roomSlug, rawBody) {
    await ensureSchema();
...
async function fetchMessages(roomSlug, limit = 50, beforeId) {
    await ensureSchema();
    if (!isValidRoomSlug(roomSlug))
        throw new Error("invalid_room");
    const args = [roomSlug, Math.min(100, Math.max(1, limit))];
    let beforeClause = "";
    if (beforeId && Number.isFinite(beforeId)) {
        beforeClause = ` AND id < $3`;
        args.push(beforeId);
    }
    const { rows } = await db_1.pool.query(`SELECT id, room_slug, user_id, body, original_body, strip_count, hidden,
...
async function reportMessage(messageId) {
    await ensureSchema();
    await db_1.pool.query(`UPDATE chat_messages SET reported_count = reported_count + 1,
...
async function adminDeleteMessage(messageId, reason = "admin_delete") {
    await ensureSchema();
    await db_1.pool.query(`UPDATE chat_messages SET deleted_at = NOW(), hidden = TRUE, hidden_reason = $2 WHERE id = $1`, [messageId, reason]);
}
async function fetchRoomsSummary() {
    await ensureSchema();
    const { rows } = await db_1.pool.query(`SELECT slug, message_count, last_message_at FROM chat_rooms`);
    const map = new Map(rows.map((r) => [r.slug, r]));
    return exports.ROOMS.map((r) => {
...
async function adminFetchRecent(limit = 100) {
    await ensureSchema();
    const { rows } = await db_1.pool.query(`SELECT m.id, m.room_slug, m.user_id, m.body, m.original_body, m.strip_count,
