...
exports.AMOUNT_SERVICE_MAP = void 0;
exports.fbPut = fbPut;
exports.fbPost = fbPost;
exports.logErrorToFirebase = logErrorToFirebase;
exports.trackCvFunnelEvent = trackCvFunnelEvent;
exports.pushActivityEvent = pushActivityEvent;
exports.mirrorSubscription = mirrorSubscription;
exports.mirrorPayment = mirrorPayment;
exports.recordPaymentEvent = recordPaymentEvent;
exports.allocateCredits = allocateCredits;
exports.decrementCredit = decrementCredit;
exports.recordJobApplicationFB = recordJobApplicationFB;
exports.trackRevenue = trackRevenue;
exports.expireSubscriptionFB = expireSubscriptionFB;
exports.fbPatch = fbPatch;
exports.storeCvAnalysis = storeCvAnalysis;
exports.logWhatsAppMessage = logWhatsAppMessage;
exports.getNanjilaMetrics = getNanjilaMetrics;
exports.pruneFirebaseCollections = pruneFirebaseCollections;
exports.startFirebasePruneScheduler = startFirebasePruneScheduler;
...
async function fbPut(path, data) {
    ...
    try {
        ...
        if (!res.ok)
            console.warn(`[RTDB] PUT ${path} failed:`, res.status, await res.text());
    }
    catch (err) {
...
async function fbPost(path, data) {
    ...
    try {
        ...
        if (!res.ok)
            console.warn(`[RTDB] POST ${path} failed:`, res.status, await res.text());
    }
    catch (err) {
 ...
 */
async function logErrorToFirebase(errorData, origin = "backend") {
    try {
 ...
 */
async function trackCvFunnelEvent(userId, event, meta = {}) {
    if (!DB_URL)
        return;
    try {
...
async function fbPatch(path, data) {
    ...
    try {
        ...
        if (!res.ok)
            console.warn(`[RTDB] PATCH ${path} failed:`, res.status, await res.text());
    }
    catch (err) {
 ...
 */
async function fbMultiPathUpdate(updates) {
    if (!DB_URL)
        return;
    try {
 ...
 */
async function fbTransaction(path, updateFn, maxRetries = 5) {
    if (!DB_URL)
        return;
...
async function pushActivityEvent(type, location, extra) {
    if (!DB_URL)
        return;
...
async function mirrorSubscription(userId, planKey, opts) {
...
async function mirrorPayment(userId, paymentId, opts) {
...
async function recordPaymentEvent(opts) {
    const { userId, paymentId, amountKes, reference, method, serviceId, subscriptionKey, subscriptionExpiryMs, } = opts;
    const now = Date.now();
    // Resolve credit params — use explicit overrides, fall back to amount map
    const mapped = exports.AMOUNT_SERVICE_MAP[amountKes];
    ...
    const expiryDate = expiryDays != null ? now + expiryDays * 86400000 : null;
    ...
    await fbTransaction(`users/${userId}/totalSpent`, (current) => {
        return (current ?? 0) + amountKes;
    });
    console.log(`[RTDB][recordPaymentEvent] userId=${userId} | KES ${amountKes} | ` +
        `credits=${creditCount}×${creditType} | ref=${reference}`);
}
async function allocateCredits(userId, creditType, data) {
    ...
    await fbTransaction(path, (existing) => {
        const prev = existing || { total: 0, used: 0, remaining: 0 };
...
async function decrementCredit(userId, creditType) {
    const path = `users/${userId}/credits/${creditType}`;
...
async function recordJobApplicationFB(userId, app) {
    const result = await decrementCredit(userId, "job_applications");
    if (!result.ok)
        throw new Error("No job application credits remaining");
...
function inferRevenueCategory(serviceId) {
    if (!serviceId)
        return "other";
    const s = serviceId.toLowerCase();
    if (s.includes("plan_") || s.includes("subscription") || s.includes("pro"))
        return "consultation_fees";
    if (s.includes("job") || s.includes("application"))
        return "job_packs";
    if (s.includes("cv"))
        return "cv_services";
    if (s.includes("university") || s.includes("student"))
        return "university_applications";
    return "other";
}
async function trackRevenue(opts) {
    ...
    await fbTransaction(`revenue/daily/${dateKey}`, (current) => {
        const day = current ?? {
            total: 0,
            consultation_fees: 0,
            job_packs: 0,
    ...
    await fbTransaction(`revenue/monthly/${monthKey}`, (current) => {
        const month = current ?? {
            total: 0,
            transactions: 0,
            consultation_fees: 0,
...
async function expireSubscriptionFB(userId, planKey) {
 ...
 */
async function storeCvAnalysis(phoneNumber, record) {
...
async function logWhatsAppMessage(opts) {
    ...
    await fbTransaction(`nanjila/daily/${dateKey}`, (current) => {
 ...
 */
async function getNanjilaMetrics(days = 7) {
    const dates = [];
    ...
    const records = await Promise.all(dates.map(async (date) => {
        const data = await fbGet(`nanjila/daily/${date}`);
 ...
 */
async function pruneFirebaseCollections(maxAgeDays = 30) {
    if (!DB_URL)
        return;
    const now = Date.now();
    ...
    for (const { path, tsField } of agePaths) {
        try {
            ...
            for (const [key, entry] of Object.entries(data)) {
                const ts = Number(entry[tsField] ?? 0);
                if (ts > 0 && ts < cutoff) {
    ...
    for (const { path, expiresField } of expiryPaths) {
        try {
            ...
            const deletions = {};
            for (const [key, entry] of Object.entries(data)) {
                const exp = Number(entry[expiresField] ?? 0);
                if (exp > 0 && exp < now) {
                    deletions[`${path}/${key}`] = null;
                }
            }
 ...
 */
function startFirebasePruneScheduler(maxAgeDays = 30) {
    if (_pruneTimer)
        return;
