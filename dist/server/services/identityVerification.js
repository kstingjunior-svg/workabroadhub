...
async function sendEmailVerificationCode(userId, email) {
    ...
    try {
        await (0, email_1.sendEmail)({ to: dest, subject: `Your WorkAbroad Hub verification code: ${code}`, html, text });
        return { ok: true };
    }
    catch (err) {
...
async function requireVerifiedForPayment(req, res, next) {
    ...
    try {
        ...
        console.error("[requireVerifiedForPayment] DB error:", err.message);
        res.status(500).json({ message: "Could not verify your account status." });
    }
}
