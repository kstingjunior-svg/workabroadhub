...
exports.extractCvInsights = extractCvInsights;
exports.scoreJobKeywordMatch = scoreJobKeywordMatch;
...
async function extractCvInsights(cvText) {
    ...
    try {
        const res = await openai_1.openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: `You are a CV parsing expert. Extract all available information from the CV text below, even if the text is messy or partially formatted.\n` +
...
function scoreJobKeywordMatch(job, insights) {
    const haystack = [
        job.title,
        job.description ?? "",
        job.jobCategory ?? "",
    ...
    for (const edu of insights.education) {
        if (haystack.includes(edu.toLowerCase()))
            score += 5;
    }
    // Country bonus
    if (insights.recommendedCountries?.length > 0 &&
        insights.recommendedCountries.some((c) => c.toLowerCase() === (job.country ?? "").toLowerCase())) {
        score += 25;
    }
...
async function batchAIMatchScores(cvText, insights, candidates) {
...
async function getJobMatches(cvText) {
    ...
    const merged = scored.map(({ job, kwScore }) => {
