...
exports.getBasicHealth = getBasicHealth;
...
function checkQueue() {
    try {
        const stats = queue_1.asyncQueue.getStats();
        let status = "healthy";
...
async function checkRedis() {
    const start = Date.now();
    try {
        const pong = await redis_1.redisConnection.ping();
        const latency = Date.now() - start;
        if (pong !== "PONG") {
...
function checkVapidKeys() {
    ...
    if (pub && priv)
        return { status: "healthy", message: "VAPID keys configured — push notifications enabled" };
    if (!pub && !priv)
        return { status: "unhealthy", message: "VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY both missing" };
    return {
        status: "unhealthy",
...
async function checkStuckPayments() {
    try {
        const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS count FROM payments WHERE status = 'retry_available'`);
        const count = parseInt(rows[0]?.count ?? "0", 10);
        if (count === 0)
            return { status: "healthy", message: "No stuck payments", details: { retryAvailable: 0 } };
        const status = count >= 5 ? "unhealthy" : "degraded";
...
async function checkOrphanSubscriptions() {
    try {
       ...
       WHERE status = 'active'
        ...
        const count = parseInt(rows[0]?.count ?? "0", 10);
        if (count === 0)
            return { status: "healthy", message: "No orphan subscriptions" };
...
async function getDetailedHealth() {
    const [database, cacheHealth, queueHealth, circuitBreakers, memory, redis, vapidKeys, stuckPayments, orphanSubscriptions,] = await Promise.all([
        checkDatabase(),
