...
exports.startCvEmailSequenceScheduler = startCvEmailSequenceScheduler;
exports.scheduleCvEmailSequence = scheduleCvEmailSequence;
...
function buildEmail1(firstName, jobCount, topCountry) {
    ...
    const text = `Hi ${firstName || "there"},\n\nYour CV analysis is ready! ${jobCount ? `We found ${jobCount} matching jobs.` : "Visit your dashboard to see matching jobs."}\n\nView your results: ${BASE_URL}/upload-cv\n\n— WorkAbroad Hub`;
...
function buildEmail2(firstName, topJobs, topCountry) {
    const name = esc(firstName || "there");
...
function buildEmail3(firstName, topCountry, profession) {
    const name = esc(firstName || "there");
...
async function processEmailQueue() {
    try {
        ...
        const due = await db_1.db
            .select()
            .from(schema_1.cvEmailQueue)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.sent, false), (0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.failed, false), (0, drizzle_orm_1.lte)(schema_1.cvEmailQueue.sendAfter, now)))
            .limit(50);
        if (due.length === 0)
            return;
        ...
        for (const row of due) {
            try {
                const jobs = (Array.isArray(row.topJobs) ? row.topJobs : []);
                let email;
                if (row.type === "email2") {
                ...
                const result = await (0, email_1.sendEmail)({ to: row.email, subject: email.subject, html: email.html, text: email.text });
                if (result.success) {
                    await db_1.db.update(schema_1.cvEmailQueue).set({ sent: true, sentAt: new Date(), errorMsg: null }).where((0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.id, row.id));
                    console.log(`[CvEmail] Sent ${row.type} to ${row.email}`);
                    sent++;
                }
                else {
                    await db_1.db.update(schema_1.cvEmailQueue).set({ failed: true, errorMsg: (result.error || "").slice(0, 500) }).where((0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.id, row.id));
                    failed++;
                }
            }
            catch (err) {
                console.error(`[CvEmail] Error sending ${row.type} to ${row.email}:`, err.message);
                await db_1.db.update(schema_1.cvEmailQueue).set({ failed: true, errorMsg: err.message.slice(0, 500) }).where((0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.id, row.id));
                failed++;
            }
        }
...
// ── Public API ────────────────────────────────────────────────────────────────
function startCvEmailSequenceScheduler() {
 ...
 */
async function scheduleCvEmailSequence(opts) {
    try {
        // Deduplicate: skip if we already queued an email1 for this address in the last 24h
        const recent = await db_1.db
            .select({ id: schema_1.cvEmailQueue.id })
            .from(schema_1.cvEmailQueue)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.email, opts.email), (0, drizzle_orm_1.eq)(schema_1.cvEmailQueue.type, "email1")))
            .limit(1);
        if (recent.length > 0) {
        ...
        const base = {
            ...
            topJobs: (opts.topJobs && opts.topJobs.length > 0 ? opts.topJobs : null),
        };
        await db_1.db.insert(schema_1.cvEmailQueue).values([
