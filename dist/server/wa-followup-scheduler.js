...
exports.startWaFollowupScheduler = startWaFollowupScheduler;
exports.scheduleWaFollowup = scheduleWaFollowup;
...
async function sendWaMessage(phone, body) {
    await (0, whatsapp_1.sendWhatsApp)(phone, body);
}
// ── Message builders ────────────────────────────────────────────────────────
function buildDay1Message(firstName, jobCount, topCountry) {
...
function matchStory(profession) {
    if (!profession)
        return SUCCESS_STORIES["nurse"]; // default
    const lower = profession.toLowerCase();
    if (lower.includes("nurs") || lower.includes("midwi"))
        return SUCCESS_STORIES["nurse"];
    if (lower.includes("doctor") || lower.includes("physic"))
        return SUCCESS_STORIES["doctor"];
    if (lower.includes("engineer") || lower.includes("tech"))
        return SUCCESS_STORIES["engineer"];
    if (lower.includes("teach") || lower.includes("tutor"))
        return SUCCESS_STORIES["teacher"];
    if (lower.includes("account") || lower.includes("financ"))
        return SUCCESS_STORIES["accountant"];
    if (lower.includes("driv") || lower.includes("truck"))
        return SUCCESS_STORIES["driver"];
    if (lower.includes("care") || lower.includes("support"))
        return SUCCESS_STORIES["caregiver"];
    if (lower.includes("hotel") || lower.includes("hospit") || lower.includes("chef"))
        return SUCCESS_STORIES["hospitality"];
    if (lower.includes("software") || lower.includes("developer") || lower.includes("it ") || lower.includes("tech"))
        return SUCCESS_STORIES["it"];
    if (lower.includes("construct") || lower.includes("civil") || lower.includes("mason"))
        return SUCCESS_STORIES["construction"];
    return SUCCESS_STORIES["nurse"];
}
function buildDay3Message(firstName, jobCount, topCountry, profession) {
    const name = firstName ? ` ${firstName}` : "";
    const country = topCountry || "top destinations";
    const story = matchStory(profession);
    return (`📢 *Success Story:* ${story.name} (a ${story.role}) uploaded her CV on WorkAbroad Hub last month. She just started her new role in ${story.destination}!\n\n` +
...
async function processFollowups() {
    try {
        ...
        const due = await db_1.db
            .select()
            .from(schema_1.waFollowups)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.waFollowups.sent, false), (0, drizzle_orm_1.eq)(schema_1.waFollowups.failed, false), (0, drizzle_orm_1.lte)(schema_1.waFollowups.sendAfter, now)))
            .limit(50);
        if (due.length === 0)
            return;
        ...
        for (const row of due) {
            try {
                ...
                await db_1.db
                    .update(schema_1.waFollowups)
                    .set({ sent: true, sentAt: new Date(), errorMsg: null })
                    .where((0, drizzle_orm_1.eq)(schema_1.waFollowups.id, row.id));
...
// ── Public API ──────────────────────────────────────────────────────────────
function startWaFollowupScheduler() {
 ...
 */
async function scheduleWaFollowup(opts) {
    try {
        const now = Date.now();
        ...
        await db_1.db.insert(schema_1.waFollowups).values([
            { ...base, type: "day1", sendAfter: day1After },
            { ...base, type: "day3", sendAfter: day3After },
        ]);
        console.log(`[WaFollowup] Scheduled day1 (${day1After.toISOString()}) + day3 (${day3After.toISOString()}) for ${opts.phone}`);
    }
    catch (err) {
