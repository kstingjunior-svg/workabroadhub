...
exports.spikeDetectionMiddleware = spikeDetectionMiddleware;
exports.underAttackModeMiddleware = underAttackModeMiddleware;
exports.getUnderAttackState = getUnderAttackState;
exports.setUnderAttackMode = setUnderAttackMode;
exports.geoRestrictionMiddleware = geoRestrictionMiddleware;
exports.slowlorisGuardMiddleware = slowlorisGuardMiddleware;
exports.dynamicRateLimitMiddleware = dynamicRateLimitMiddleware;
exports.updateIpRiskCache = updateIpRiskCache;
exports.applyDdosProtection = applyDdosProtection;
const storage_1 = require("../storage");
// Lightweight event tracker — writes directly to storage, avoids circular import with security.ts
function logThreatEvent(eventType, ip, endpoint, ua, meta) {
    storage_1.storage.createSecurityEvent({
        eventType,
...
// ─────────────────────────────────────────────────────────────────────────────
const ipBanStore = new Map();
const requestWindows = new Map();
const geoCache = new Map();
const dynamicRiskCache = new Map();
...
function isWebhookRoute(path) {
    return (path.startsWith("/api/mpesa/") ||
...
function ipBlockMiddleware(req, res, next) {
    ...
    if (isPrivateIp(ip))
        return next(); // never block internal traffic
...
// ─────────────────────────────────────────────────────────────────────────────
function spikeDetectionMiddleware(req, res, next) {
    if (!req.path.startsWith("/api"))
        return next();
...
// ─────────────────────────────────────────────────────────────────────────────
function underAttackModeMiddleware(req, res, next) {
    const now = Date.now();
    // Reset the global counter every 10 seconds
    if (now - lastWindowReset > 10000) {
...
function setUnderAttackMode(active) {
...
function geoRestrictionMiddleware(req, res, next) {
    // Only apply to API routes; only when attack mode is active (avoid latency in normal mode)
    if (!req.path.startsWith("/api") || !underAttackMode)
        return next();
    const ip = getClientIp(req);
    if (isPrivateIp(ip))
        return next();
...
// ─────────────────────────────────────────────────────────────────────────────
function slowlorisGuardMiddleware(req, res, next) {
...
const dynamicWindowStore = new Map();
const DYNAMIC_WINDOW_MS = 60000; // 1 minute
...
function dynamicRateLimitMiddleware(req, res, next) {
    ...
    if (!isHighRisk)
        return next(); // only apply to flagged IPs
    const now = Date.now();
...
function updateIpRiskCache(ip, score) {
