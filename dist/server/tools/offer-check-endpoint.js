...
exports.registerOfferCheckRoute = registerOfferCheckRoute;
const multer_1 = __importDefault(require("multer"));
const crypto_1 = __importDefault(require("crypto"));
const db_1 = require("../db");
const openai_1 = require("../lib/openai");
const extract_text_1 = require("../utils/extract-text");
const offer_screening_1 = require("./offer-screening");
...
async function overDailyLimit(userId, guestFp) {
    ...
    const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS n FROM offer_letter_checks
      WHERE ${idCol} = $1
...
// ─────────────────────────────────────────────────────────────────────────────
function detectPhysicalAddress(ocr) {
...
async function saveCheck(args) {
    ...
    await db_1.pool.query(`INSERT INTO offer_letter_checks
...
// Route registration
// ─────────────────────────────────────────────────────────────────────────────
function registerOfferCheckRoute(app) {
    app.post("/api/tools/offer-check", upload.single("file"), async (req, res) => {
        const t0 = Date.now();
        try {
            if (!req.file) {
            ...
            // ── OCR ─────────────────────────────────────────────────────────
            const sha256 = crypto_1.default.createHash("sha256").update(req.file.buffer).digest("hex");
            let ocrText = "";
            let method = "";
            let vision = null;
            let hasLetterhead = null;
            let hasSignature = null;
            ...
            // ── Parse + screen ──────────────────────────────────────────────
            const parsed = (0, offer_screening_1.parseVisibleFields)(ocrText);
            const domainCk = (0, offer_screening_1.extractSenderDomain)(ocrText, parsed.employerName);
            ...
            const report = (0, offer_screening_1.screenOffer)({ ocrText, parsed, employer, aiVision: vision });
            // ── Persist ─────────────────────────────────────────────────────
            ...
            console.log(`[OfferCheck] checkId=${checkId} band=${report.riskBand} score=${report.riskScore} ` +
    ...
    // ═══════════════════════════════════════════════════════════════════════
    app.post("/api/tools/offer-verify", (req, res, next) => upload.single("file")(req, res, (err) => {
        if (!err)
            return next();
        ...
        try {
            if (!req.file)
                return res.status(400).json({ message: "Please attach the offer letter (image or PDF)." });
            ...
            const { analyzeOffer } = await Promise.resolve().then(() => __importStar(require("../offer-verify/analyzer")));
            const report = await analyzeOffer(dataUrl);
            if (!report.ok) {
                return res.status(502).json({ ok: false, message: report.message });
            }
            console.log(`[OfferVerify] verdict=${report.verdict} trust=${report.overallTrust} country=${report.country?.code ?? "?"} ` +
