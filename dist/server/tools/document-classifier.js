...
exports.DOCUMENT_CLASSIFIER_VERSION = void 0;
exports.classifyDocument = classifyDocument;
exports.checkDocumentType = checkDocumentType;
exports.humanType = humanType;
// ─────────────────────────────────────────────────────────────────────────────
// Public types
// ─────────────────────────────────────────────────────────────────────────────
exports.DOCUMENT_CLASSIFIER_VERSION = 1;
...
function classifyDocument(opts) {
    ...
    if (opts.visionHint &&
        ...
        scores[opts.visionHint.type] > 0) {
        return {
            version: exports.DOCUMENT_CLASSIFIER_VERSION,
    ...
    const ranked = ([
        { type: "cv", score: scores.cv, hits: cvHits },
        { type: "job_advert", score: scores.job_advert, hits: jobAdvertHits },
        { type: "offer_letter", score: scores.offer_letter, hits: offerLetterHits },
        { type: "visa", score: scores.visa, hits: visaHits },
...
function scoreCategory(_type, text, signals) {
    let total = 0;
    const matched = [];
...
function inferCategoryFromSignal(hits, s) {
    if (CV_SIGNALS.includes(s))
        return "cv";
    if (JOB_ADVERT_SIGNALS.includes(s))
        return "job_advert";
    if (OFFER_LETTER_SIGNALS.includes(s))
        return "offer_letter";
    if (VISA_SIGNALS.includes(s))
        return "visa";
    return "unknown";
}
const TOOL_FOR_TYPE = {
 ...
 */
function checkDocumentType(classification, expected) {
