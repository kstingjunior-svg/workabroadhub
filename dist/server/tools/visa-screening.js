...
exports.VISA_SCREENING_VERSION = void 0;
exports.parseMrz = parseMrz;
exports.mrzCheckDigit = mrzCheckDigit;
exports.screenVisa = screenVisa;
// ─────────────────────────────────────────────────────────────────────────────
// Public types
// ─────────────────────────────────────────────────────────────────────────────
exports.VISA_SCREENING_VERSION = 1;
// ─────────────────────────────────────────────────────────────────────────────
...
function parseMrz(ocrText) {
    ...
    const isMrzLine = (s) => s.length >= 30 && s.length <= 46 && mrzChar.test(s.replace(/\s+/g, ""));
...
function decodeMrzTypeV(l1, l2) {
    ...
    if (l1[0] !== "V" && l1[0] !== "P")
        return null; // not a visa/passport line
...
function screenVisa(input) {
    ...
    return {
        version: exports.VISA_SCREENING_VERSION,
        riskScore,
        riskBand,
        findings,
        parsed: input.parsed,
...
function computeRiskScore(findings, input) {
    ...
    if (input.mrz.present && input.mrz.checksumValid === true)
        score -= 15;
...
function mrzYymmddToDate(yymmdd) {
    if (!/^\d{6}$/.test(yymmdd))
        return null;
...
function normalizeCountryToIso3(input) {
...
function classifyVisionSeverity(flag) {
    ...
    if (/tamper|edit|photoshop|forged|forgery|altered|clon(e|ed)|paste|inconsist(ent|ency)/.test(f))
        return "critical";
    if (/misalign|font|blur|artifact|artefact|shadow|watermark|hologram/.test(f))
        return "warning";
    return "info";
}
