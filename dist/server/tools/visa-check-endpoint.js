...
exports.registerVisaCheckRoute = registerVisaCheckRoute;
...
async function overDailyLimit(userId, guestFp) {
    ...
    const { rows } = await db_1.pool.query(`SELECT COUNT(*)::text AS n FROM visa_checks
      WHERE ${idCol} = $1
...
function parseVisibleFields(ocr) {
    ...
    const country = (() => {
        ...
        for (const n of names)
            if (new RegExp(`\\b${n}\\b`, "i").test(ocr))
                return n;
        return null;
    })();
...
function extractDate(text, prefixRe) {
...
async function saveCheck(args) {
    ...
    await db_1.pool.query(`INSERT INTO visa_checks
...
// Route registration
// ─────────────────────────────────────────────────────────────────────────────
function registerVisaCheckRoute(app) {
    app.post("/api/tools/visa-check", upload.single("file"), async (req, res) => {
        const t0 = Date.now();
        try {
            if (!req.file) {
            ...
            const mrz = (0, visa_screening_1.parseMrz)(ocrText);
            const report = (0, visa_screening_1.screenVisa)({ ocrText, parsed, mrz, aiVision: vision });
            // ── Persist ─────────────────────────────────────────────────────
            ...
            console.log(`[VisaCheck] checkId=${checkId} band=${report.riskBand} score=${report.riskScore} ` +
    ...
    // ═══════════════════════════════════════════════════════════════════════
    app.post("/api/tools/visa-verify", (req, res, next) => upload.single("file")(req, res, (err) => {
        if (!err)
            return next();
        ...
        try {
            ...
            console.log(`[VisaVerify] verdict=${report.verdict} trust=${report.overallTrust} band=${report.riskBand} country=${report.country?.code ?? "?"} in ${Date.now() - t0}ms`);
            res.json({
                ok: true,
