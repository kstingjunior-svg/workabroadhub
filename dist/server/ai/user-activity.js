...
async function buildActivitySummary(userId) {
    ...
    try {
        ...
        const pageViews = await db_1.pool.query(`SELECT page, event, metadata, created_at
         FROM funnel_events
        WHERE user_id = $1
        ...
        const orders = await db_1.pool.query(`SELECT service_slug, service_name, status, created_at
         FROM service_orders
        WHERE user_id = $1
        ORDER BY created_at DESC
        LIMIT 5`, [userId]);
        ...
        for (const v of pageViews.rows) {
            ...
            if (m)
                countByCountry.set(m[1], (countByCountry.get(m[1]) ?? 0) + 1);
        ...
        if (pageViews.rows.length === 0) {
            ...
            if (totalDwellMs > 60000) {
                lines.push(`   • Total dwell time: ~${Math.round(totalDwellMs / 60000)} min on-site`);
            }
            if (topCountry) {
