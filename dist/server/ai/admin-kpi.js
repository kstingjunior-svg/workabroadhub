...
exports.buildAdminKpiSnapshot = buildAdminKpiSnapshot;
exports.isAdminUser = isAdminUser;
const db_1 = require("../db");
async function buildAdminKpiSnapshot() {
    const lines = ["▸ Business snapshot (you are the founder — Nanjila has read-only KPI access):"];
    ...
    try {
        const { rows: [a] } = await db_1.pool.query(`
      SELECT
...
async function isAdminUser(userId) {
    if (userId == null)
        return false;
    try {
        const { rows: [r] } = await db_1.pool.query(`SELECT is_admin, role FROM users WHERE id = $1 LIMIT 1`, [String(userId)]);
        if (!r)
            return false;
