...
exports.SIGNATURE_CELEBRATION = exports.SIGNATURE_WARNING = exports.SIGNATURE_STANDARD = void 0;
exports.scrubForbiddenPhrases = scrubForbiddenPhrases;
exports.appendSignature = appendSignature;
exports.applyPersonaGuards = applyPersonaGuards;
// ─────────────────────────────────────────────────────────────────────────────
// Signatures
// ─────────────────────────────────────────────────────────────────────────────
exports.SIGNATURE_STANDARD = "I'm Nanjila. Let's build your future abroad — safely.";
exports.SIGNATURE_WARNING = "Slow down. Verify. I'm Nanjila — I've got your back.";
exports.SIGNATURE_CELEBRATION = "That's what we're here for. Go do great things — I'm Nanjila.";
 ...
 */
function scrubForbiddenPhrases(reply) {
    let scrubbed = reply;
    const matched = [];
 ...
 */
function appendSignature(reply, opts = {}) {
    ...
    if (/i'?m nanjila/i.test(cleaned))
        return reply;
    // Too short?
    if (cleaned.length < SIGNATURE_MIN_REPLY_LENGTH && mode === "auto")
        return reply;
    // Explicit override.
    if (mode === "warning")
        return `${cleaned}\n\n_${exports.SIGNATURE_WARNING}_`;
    if (mode === "celebration")
        return `${cleaned}\n\n_${exports.SIGNATURE_CELEBRATION}_`;
    if (mode === "standard")
        return `${cleaned}\n\n_${exports.SIGNATURE_STANDARD}_`;
    // Auto-detect.
 ...
 */
function looksLikeWarning(reply) {
    const r = reply.toLowerCase();
    return (/\bhigh risk\b/.test(r) ||
...
function looksLikeCelebration(reply) {
    const r = reply.toLowerCase();
    return (/🎉/.test(reply) ||
 ...
 * admin dashboard.
 */
function applyPersonaGuards(raw, opts = {}) {
    ...
    if (withSig !== scrubbed.trim() && withSig !== scrubbed) {
        if (withSig.includes(exports.SIGNATURE_WARNING))
            sigApplied = "warning";
        else if (withSig.includes(exports.SIGNATURE_CELEBRATION))
            sigApplied = "celebration";
        else if (withSig.includes(exports.SIGNATURE_STANDARD))
            sigApplied = "standard";
    }
