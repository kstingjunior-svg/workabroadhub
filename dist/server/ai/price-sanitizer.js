...
exports.invalidatePriceWhitelist = invalidatePriceWhitelist;
exports.sanitizeReply = sanitizeReply;
const db_1 = require("../db");
let WHITELIST_CACHE = null;
...
const FORBIDDEN_PRICES = new Set([3500, 3000, 2500, 1500, 4500]);
...
function invalidatePriceWhitelist() {
 ...
 */
async function sanitizeReply(reply) {
    ...
    for (const hit of matches.reverse()) {
        ...
        let guessedSlug = null;
        for (const kw of SLUG_KEYWORDS) {
            if (kw.rx.test(context)) {
                guessedSlug = kw.slug;
                break;
            }
        }
