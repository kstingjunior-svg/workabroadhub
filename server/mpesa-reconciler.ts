    ...
    if (phone && amount > 0) {
      try {
        ...
        if (matchedPayment) {
          ...
          const sid = ((matchedPayment as any).serviceId ?? "").toLowerCase();
          const fromService = sid.startsWith("plan_") ? sid.replace("plan_", "") : null;
          const fromPlanId = (matchedPayment as any).planId ? String((matchedPayment as any).planId).toLowerCase() : null;
          const resolvedTier =
            (fromService && RECON_VALID_TIERS.has(fromService)) ? fromService :
          ...
          if (!resolvedTier) {
            ...
            if (sid) {
              await storage.unlockService(matchedPayment.userId, sid, matchedPayment.id, {
                reconciled: true, transId,
              }).catch((err: any) => console.warn(`[Reconciler] unlockService failed: ${err?.message}`));
            }
