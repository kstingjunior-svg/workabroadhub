...
export interface PreflightResult {
  ...
  weaknesses: string[];     // machine-readable issues to feed into retry
  suggestion: string;       // one-line hint for the retry prompt
}
...
export async function preflightScoreCV(
  cvText: string,
  threshold = 70,
  ...
  try {
    ...
    const weaknesses = Array.isArray(parsed.weaknesses)
      ? parsed.weaknesses.filter((w: any) => typeof w === "string").slice(0, 6)
      : [];
    return {
      ok:         true,
      score,
 ...
 */
export function buildRetryFeedback(pre: PreflightResult, threshold = 70): string {
  const lines = [
