...
function buildClient(): OpenAI {
  ...
  if (!apiKey) {
    ...
    console.log(
      `[OpenAI] Client (re)initialised | keyPrefix=${apiKey.slice(0, 7)}*** baseURL=${baseURL ?? "default(api.openai.com)"}`
    );
...
export const openai: OpenAI = new Proxy({} as OpenAI, {
  get(_target, prop, receiver) {
    if (!_cachedClient) _cachedClient = buildClient();
 ...
 */
export async function openAIHealthProbe(): Promise<{ ok: boolean; reason?: string; keyPrefix?: string }> {
  ...
  try {
    ...
    else if (status === 429) reason = "rate_limited_or_billing";
    else if (code === "ECONNREFUSED" || code === "ETIMEDOUT") reason = "network";
    else if (err?.message) reason = err.message.slice(0, 200);
