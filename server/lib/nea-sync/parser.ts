...
export interface ParsedAgency {
  ...
  rawStatus:     string | null;  // whatever NEA displayed (Active/Expired/Suspended…)
}

export interface ParseResult {
  rows:     ParsedAgency[];
  warnings: string[];
  format:   "html" | "csv" | "tsv" | "unknown";
}

// ─── Public API ───────────────────────────────────────────────────────────

export function parseNEASource(raw: string): ParseResult {
  const trimmed = raw.trim();
  if (!trimmed) return { rows: [], warnings: ["Empty input"], format: "unknown" };
  ...
  if (tabs > commas) return parseTsv(trimmed);
...
function parseHtml(html: string): ParseResult {
  ...
  const agencyTable = tableMatch.find((t) =>
    /(licen[cs]e\s*(no|number)|licence)/i.test(t) &&
  ...
  for (const rowHtml of rowMatches) {
    ...
    if (/^licence|^license|^agency|^no\.?$|^#$/i.test(cells[0] ?? "")) continue;

    const parsed = mapColumnsToAgency(cells);
    if (parsed) rows.push(parsed);
  }

  if (rows.length === 0) warnings.push("HTML parsed but produced 0 usable rows");
  return { rows, warnings, format: "html" };
}
...
function parseTsv(tsv: string): ParseResult {
  return parseDelimited(tsv, "\t");
}

function parseDelimited(text: string, sep: string): ParseResult {
  const warnings: string[] = [];
  const rows: ParsedAgency[] = [];
  ...
  if (idx.name < 0 || idx.license < 0) {
    warnings.push(
      `Header missing required columns. Expected 'Agency Name' + 'Licence No' (got: ${headerCells.join(", ")})`,
    );
    return { rows, warnings, format: sep === "," ? "csv" : "tsv" };
  }
...
function stripCell(cellHtml: string): string {
  return cellHtml
...
function mapColumnsToAgency(cells: string[]): ParsedAgency | null {
  const guess = {
    name:    cells[1] ?? "",
    license: cells[2] ?? "",
    contact: cells[4] ?? "",
  ...
  if (!guess.name || !guess.license) return null;
...
function safe(cells: string[], idx: number): string {
