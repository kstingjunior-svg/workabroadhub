...
function exactHash(cvText: string): string {
...
function structuralHash(cvText: string): string {
...
async function ensureSchema(): Promise<void> {
  ...
  try {
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_dcf_exact ON delivered_cv_fingerprints (exact_hash);`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_dcf_structural ON delivered_cv_fingerprints (structural_hash);`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_dcf_user ON delivered_cv_fingerprints (user_id);`);
 ...
 * still holds.
 */
export async function lookupDeliveredCv(cvText: string): Promise<DeliveryMatch | null> {
  if (!cvText || cvText.trim().length < 100) return null;
  await ensureSchema();
  try {
    ...
    const rows = await pool.query<{
      delivered_score: number;
      delivered_at:    Date;
      service_slug:    string;
      exact_hash:      string;
    }>(
      `SELECT delivered_score, delivered_at, service_slug, exact_hash
...
export const CV_OUTPUT_SLUGS = new Set<string>([
  "cv_fix_lite",
  "ats_cv_optimization",
  "cv_rewrite",
