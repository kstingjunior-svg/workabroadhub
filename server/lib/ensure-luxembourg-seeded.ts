...
const LUX_GUIDES = [
  {
    ...
    content:
      "Be honest with yourself first — Luxembourg is real but narrow. " +
  ...
  },
  {
    section: "cv_tips",
    content:
      "European CV format (Europass is widely accepted). 2 pages maximum. " +
  ...
  },
  {
    ...
    content:
      "⚠️ READ THIS BEFORE PAYING. Luxembourg's work-permit rules are some of " +
...
export async function ensureLuxembourgSeeded(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[ensureLuxembourg] ✓ Luxembourg seeded: countryId=${countryId} ` +
