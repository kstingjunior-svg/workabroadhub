...
export function validateKenyanPhone(rawPhone: string | null | undefined): PhoneValidation {
  ...
  if (digits.length === 10 && digits.startsWith("0")) {
    ...
    return {
      ok: false,
      reason:
        "M-Pesa STK Push only works on Safaricom Kenya numbers (07XXX or 011XXX). " +
  ...
  if (!KENYA_SAFARICOM_REGEX.test(normalized)) {
    ...
    return {
      ok: false,
      reason:
        "M-Pesa STK Push only works on Safaricom lines (07XX or 011XX numbers issued by Safaricom). " +
