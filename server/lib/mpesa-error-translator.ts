...
export interface MpesaFriendlyError {
  /** The original Safaricom code if we recognised it. */
  daraja_code:    string | null;
  /** Short heading the client renders as the toast title. */
  title:          string;
  /** One-sentence Kenyan-English explanation. */
  message:        string;
  /** Direct next-step the user can take. */
  next_step:      string;
  /** Whether retrying right now is likely to succeed. */
  retry_safe:     boolean;
  /** Whether falling back to manual Paybill is recommended. */
  offer_paybill:  boolean;
  /** Helpful for client UI: should we ask them to enter their phone again? */
  bad_phone:      boolean;
}

const PAYBILL_FALLBACK_MESSAGE =
  "If this keeps failing, pay manually: M-Pesa Paybill 4153025, Account = your email.";

// Common Daraja STK + receipt error codes mapped to friendly Kenyan messages.
const MAP: Array<{
  match: RegExp | string;
  build: (raw: string) => MpesaFriendlyError;
}> = [
 ...
 */
export function friendlyMpesaError(raw: unknown): MpesaFriendlyError {
  let haystack = "";
  let daraja_code: string | null = null;
