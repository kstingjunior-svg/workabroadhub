...
export async function bootstrapLocalJobs(): Promise<void> {
  ...
  try {
    ...
    await pool.query(`ALTER TABLE local_job_applications ADD COLUMN IF NOT EXISTS applicant_county    VARCHAR(60)`);
    await pool.query(`ALTER TABLE local_job_applications ADD COLUMN IF NOT EXISTS highest_education   VARCHAR(80)`);
    await pool.query(`ALTER TABLE local_job_applications ADD COLUMN IF NOT EXISTS years_experience    INTEGER`);
    await pool.query(`ALTER TABLE local_job_applications ADD COLUMN IF NOT EXISTS certificates_url    TEXT`);
    ...
    await pool.query(`ALTER TABLE local_job_applications ADD COLUMN IF NOT EXISTS company_id UUID`);
    await pool.query(`
    ...
    await pool.query(`ALTER TABLE company_claims ADD COLUMN IF NOT EXISTS verification_code           VARCHAR(8)`);
    await pool.query(`ALTER TABLE company_claims ADD COLUMN IF NOT EXISTS verification_code_expires_at TIMESTAMP`);
    await pool.query(`ALTER TABLE company_claims ADD COLUMN IF NOT EXISTS verification_attempts       INTEGER NOT NULL DEFAULT 0`);
    await pool.query(`ALTER TABLE company_claims ADD COLUMN IF NOT EXISTS email_verified_at           TIMESTAMP`);
    await pool.query(`ALTER TABLE company_claims ADD COLUMN IF NOT EXISTS domain_match                BOOLEAN`);
    await pool.query(`ALTER TABLE company_claims ADD COLUMN IF NOT EXISTS trust_score                 VARCHAR(8)`);   // 'high'|'medium'|'low'
    ...
    await pool.query(`ALTER TABLE companies ADD COLUMN IF NOT EXISTS known_domains TEXT[]`);
    ...
    await pool.query(`ALTER TABLE local_jobs ADD COLUMN IF NOT EXISTS is_seed BOOLEAN NOT NULL DEFAULT false`);
    // Backfill: every row currently in local_jobs was inserted from the
    // catalogue, so mark them all as seed. Safe to re-run.
    await pool.query(`UPDATE local_jobs SET is_seed = true WHERE is_seed = false`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_local_jobs_is_seed ON local_jobs(is_seed) WHERE is_seed = true`);
    ...
    `);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_notify_company_email ON employer_notify_signups(company_id, email)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_notify_company ON employer_notify_signups(company_id)`);
    ...
    `);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_company_admin_user ON company_admins(company_id, user_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_company_admin_user ON company_admins(user_id)`);
    ...
    for (const c of SEED_COMPANIES) {
      const { rows } = await pool.query<{ id: string; was_inserted: boolean }>(
        `INSERT INTO companies (name, slug, industry, county, description, website, status, verified_at, known_domains)
         VALUES ($1, $2, $3, $4, $5, $6, 'approved', NOW(), $7)
         ON CONFLICT (slug) DO UPDATE
            SET known_domains = EXCLUDED.known_domains
         RETURNING id, (xmax = 0) AS was_inserted`,
        [c.name, c.slug, c.industry, c.hqCounty, c.description, c.website, c.knownDomains ?? []],
      );
