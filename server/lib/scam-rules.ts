...
export interface Rule {
  /** Stable machine-readable id — this string ends up in kazi_karibu_moderation.reason_codes. */
  id: string;
  severity: Severity;
  /**
   * Regex tested against the concatenated (title + " " + description).
   * Alternative: a predicate function for structural checks.
   */
  pattern: RegExp | ((ctx: RuleContext) => boolean);
  /** Message shown to the poster if this rule fires. */
  poster_reason: string;
  /** Short internal note for moderators. */
  moderator_note: string;
  /** If set, rule only applies when post.category is in this set. */
  onlyIfCategoryIn?: string[];
  /** If set, rule ONLY applies when post.category is NOT in this set. */
  exceptIfCategoryIn?: string[];
}
...
export interface RuleEngineResult {
  ...
  hasFlag:   boolean;
  /**
   * Convenience: the moderation "decision" this run would trigger before
   * Layer 4 (Nanjila) runs. reject → post can't go live; flag → post
   * proceeds to payment + Nanjila with the flag context attached;
   * approve → post proceeds unflagged.
   */
  layer3Decision: "reject" | "flag" | "approve";
}
...
export const SCAM_RULES: Rule[] = [
  ...
  // upfront — the most common scam pattern in Kenyan household hiring.
  {
    ...
    poster_reason:
      "Kazi Karibu never asks applicants to pay. If your post asks for a registration fee, uniform fee, deposit, or any upfront payment from the applicant, please remove it.",
  ...
  },
  {
    ...
    poster_reason:
      "Training and starter-kit fees paid by the applicant are not allowed on Kazi Karibu.",
  ...
  // ── Guaranteed placement / no interview ────────────────────────────────
  {
    ...
    poster_reason:
      "Real employers interview applicants — nothing is guaranteed. Please remove wording that promises hiring.",
  ...
  },
  {
    ...
    poster_reason:
      "Every legitimate hire involves at least a phone conversation. Please remove 'no interview' wording.",
  ...
  // categories — event promoters legitimately mention travel domestically.
  {
    ...
    poster_reason:
      "Overseas placements belong on the main WorkAbroad Hub board, not Kazi Karibu. Please post there instead.",
  ...
  // ── MLM / opportunity roles ────────────────────────────────────────────
  {
    ...
    poster_reason:
      "Business opportunities, MLM, and network marketing roles are not allowed on Kazi Karibu.",
  ...
  // ── Adult / escort ─────────────────────────────────────────────────────
  {
    ...
    poster_reason:
      "This category isn't supported on Kazi Karibu.",
  ...
  // ── Drug-mule pattern ──────────────────────────────────────────────────
  {
    ...
    poster_reason:
      "This description contains language commonly associated with unsafe or illegal work. Please rephrase what the job actually involves.",
  ...
  // ── Impersonation of corporate employers ───────────────────────────────
  {
    ...
    poster_reason:
      "Posts on behalf of established companies or government bodies must go through Kenya Careers with company verification, not Kazi Karibu.",
...

  {
    ...
    pattern: (ctx) => {
      ...
      return (
        /\banywhere\s+in\s+kenya\b/.test(text) ||
...

  {
    ...
    severity: "flag",
    pattern: (ctx) => {
      if (!ctx.budgetMaxKes || !ctx.budgetPeriod) return false;
      ...
      if (ctx.budgetPeriod === "month" && ctx.budgetMaxKes > 100_000) return true;
      if (ctx.budgetPeriod === "day"   && ctx.budgetMaxKes > 5_000)   return true;
      if (ctx.budgetPeriod === "hour"  && ctx.budgetMaxKes > 1_000)   return true;
      return false;
    },
    poster_reason:
      "The pay you've listed is significantly higher than typical for this kind of role. If this is correct, please add a note explaining the details so applicants understand.",
...

  {
    ...
    poster_reason:
      "Real businesses usually contact from a branded email. If you're a household, that's fine — no action needed. If you're a business, add your company name in the description.",
...

  {
    ...
    poster_reason:
      "Please give at least a rough pay range. 'Negotiable' with no range makes it hard for applicants to know if the role fits them.",
...

  {
    ...
    poster_reason:
      "'Urgent' works better when you say why. Add the deadline or the day you need the person to start.",
 ...
 */
export function evaluatePostAgainstRules(ctx: RuleContext): RuleEngineResult {
  const combinedText = `${ctx.title}\n${ctx.description}`;
  const hits: RuleHit[] = [];

  for (const rule of SCAM_RULES) {
    if (rule.onlyIfCategoryIn && !rule.onlyIfCategoryIn.includes(ctx.category)) continue;
    if (rule.exceptIfCategoryIn && rule.exceptIfCategoryIn.includes(ctx.category)) continue;
