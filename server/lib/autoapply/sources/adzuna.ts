...
interface FetchArgs {
  country:         string;      // 'uk' | 'canada' | 'usa' | 'germany' | ...
  what:            string;      // search term, e.g. 'registered nurse'
  resultsPerPage?: number;      // Adzuna caps at 50
...
const YEARLY_TO_MONTHLY_KES: Record<string, number> = {
...
export async function fetchAdzunaJobs(args: FetchArgs): Promise<NormalisedJob[]> {
...
// ─── Adzuna raw response shape (only what we use) ─────────────────────
...
function normaliseAdzunaJob(raw: AdzunaRawJob, adzunaCountry: string): NormalisedJob {
...
function formatSalary(min: number | null, max: number | null, country: string): string | null {
  if (!min && !max) return null;
  const currency = COUNTRY_CURRENCY[country] ?? "";
  if (min && max && min !== max) return `${currency}${Math.round(min).toLocaleString()} – ${currency}${Math.round(max).toLocaleString()}`;
  const one = min ?? max ?? 0;
