...
export async function draftCoverLetter(args: DraftArgs): Promise<string> {
...
Draft the cover letter following the rules exactly. Only output the letter body, nothing else.`;
  ...
  try {
    const completion = await Promise.race<any>([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error("openai timeout")), 20_000)),
    ]);
...
function templateFallback(args: DraftArgs): string {
