...
6. ~~**Postgres additions**~~ — DONE for `cv_generations` + pgvector uniqueness gate (`db-bootstrap.ts` + `uniqueness-gate.ts`). Table auto-installs at server startup (idempotent CREATE IF NOT EXISTS). Every successful generation is embedded (`text-embedding-3-small`) and saved for future comparison. New generations are cosine-compared against the last 100 CVs in the same `(industry, seniority_band)` bucket; sim ≥ 0.85 sets `uniquenessWarning: true` on the response but doesn't block (score is the trust promise, not uniqueness — we surface it so we can iterate). Fail-open: any pgvector or embedding error degrades to "uniqueness disabled" cleanly. `cv_banned_phrases` and `cv_style_variants` tables not yet built — banned-phrase list lives in code (`orchestrator.ts` + `pass3-style-jd.ts`), and permutation table in `pass3-style-jd.ts SECTION_ORDER_VARIANTS`. Promote to Postgres when admins need to edit without a deploy.
