...
export async function runVerificationReminderSweep(): Promise<SweepResult> {
  ...
  try {
    ...
    for (const user of rows) {
      ...
      // saves a click.
      let codeForEmail: string | null = null;
      try {
        const codeRow = await pool.query<{ code_hash: string }>(`
...
export function startVerificationReminderSweep(): void {
  if (_timer) return;
  console.log(`[verification-reminder] Started — running every ${SWEEP_INTERVAL_MS / 60_000} min`);
