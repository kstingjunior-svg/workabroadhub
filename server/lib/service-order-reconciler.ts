...
interface SweepResult {
  scanned:    number;
  linked:     number;
  orphaned:   number;   // payment exists but no matching order found
  errors:     number;
  durationMs: number;
}
...
export async function runServiceOrderReconciler(): Promise<SweepResult> {
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        if (!userId) {
          console.warn(
            `[service-order-reconciler] Skip paymentId=${row.payment_id} — no user resolvable ` +
            `(phone=${row.phone} mpesa=${row.mpesa_code})`
          );
          // Mark matched to avoid re-scanning; needs manual attention
          await pool.query(
            `UPDATE payments SET matched = true, needs_review = true WHERE id = $1`,
            [row.payment_id]
          ).catch(() => {});
          orphaned++;
          continue;
        ...
        if (!orderId) {
          const { rows: oRows } = await pool.query<CandidateOrderRow>(
            `SELECT id, status, created_at
             ...
             LIMIT 1`,
            [userId, row.service_id, row.created_at]
          );
        ...
        if (!orderId) {
          console.warn(
            `[service-order-reconciler] ORPHAN paymentId=${row.payment_id} userId=${userId} ` +
          ...
          await pool.query(
            `UPDATE payments SET matched = true, matched_user_id = $2, needs_review = true WHERE id = $1`,
            [row.payment_id, userId]
          ).catch(() => {});
          orphaned++;
          continue;
        }
        ...
        console.warn(
          `[service-order-reconciler] LINK paymentId=${row.payment_id} → orderId=${orderId} ` +
        ...
        await pool.query(
          `UPDATE service_orders
           ...
           WHERE id = $1 AND status IN ('pending_payment', 'processing', 'failed')`,
          [orderId]
        );
        ...
        onPaymentSuccessForServiceOrder(orderId).then(() => {
          ...
          console.error(
            `[service-order-reconciler] AI trigger FAILED for orderId=${orderId}: ${err?.message}`
          );
        });
        ...
        errors++;
        console.error(
          `[service-order-reconciler] recovery threw for paymentId=${row.payment_id}: ${err?.message}`
        );
      }
    }
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      if (r.scanned > 0 || r.linked > 0 || r.orphaned > 0 || r.errors > 0) {
        console.warn(
          `[service-order-reconciler] First sweep: scanned=${r.scanned} linked=${r.linked} ` +
  ...
  _timer = setInterval(async () => {
    ...
    try {
      ...
      if (r.linked > 0 || r.orphaned > 0 || r.errors > 0) {
        console.warn(
          `[service-order-reconciler] Sweep: scanned=${r.scanned} linked=${r.linked} ` +
          `orphaned=${r.orphaned} errors=${r.errors} (${r.durationMs}ms)`
        );
      }
    } catch (err: any) {
...
export function stopServiceOrderReconciler(): void {
  if (_timer) {
