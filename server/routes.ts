...
import { checkNeaLookupCap, recordNeaLookup } from "./nea/lookupCounter";
...
export async function registerRoutes(
  ...
  const { registerAdminNanjilaRoutes } = await import("./routes/admin-nanjila");
  registerAdminNanjilaRoutes(app, isAuthenticated, isAdmin);
...
Crawl-delay: 1`);
  ...
  app.post("/api/heartbeat", async (req: any, res) => {
    ...
    if (userId && sessionId) {
      ...
      try {
        const { heartbeat: presenceHeartbeat } = await import("./lib/presence");
  ...
  app.get("/api/public/stats", async (_req, res) => {
    try {
      ...
      `);
      const onlineNow = Number((onlineRows as any).rows?.[0]?.c ?? 0);
      ...
      const inMemoryActive = getActiveUserCounts();
  ...
  app.get("/api/jobs", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { shuffleSeedFor, withFreshness, rotateByScore, pickDiverseTop, computeActivityStats } =
      ...
      const tail = rotateByScore(
        enriched.filter((j) => !diverseTop.some((d) => d.id === j.id)),
        seed,
      );
      ...
      res.setHeader("Access-Control-Expose-Headers",
        "X-WAH-Total-Active, X-WAH-New-This-Week, X-WAH-New-Today, X-WAH-Last-Updated");
      res.json(finalOrder);
    } catch (err) {
  ...
  app.get("/api/go/job/:jobId", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!PAID_TIERS.has(planId)) {
        if (wantsHtml) return res.redirect(302, "/pricing?from=job-apply");
        return res.status(403).json({
  ...
  app.post("/api/payments/initiate", isAuthenticated, requireVerifiedForPayment, async (req: any, res) => {
    try {
      ...
      if (method === "mpesa") {
        ...
        setTimeout(async () => {
          try {
            const row = await storage.getPaymentById(pendingId);
            if (!row) return;
            const status = String((row as any).status ?? "");
            const checkoutSet =
              !!(row as any).checkoutRequestId &&
              String((row as any).checkoutRequestId) !== pendingId;
            ...
            if (status === "pending" && !checkoutSet) {
              console.error(
                `[PAYMENT-SILENT-BUG] payment ${pendingId} still 'pending' 8s after /api/payments/initiate — caller forgot to POST /api/mpesa/stk. ` +
    ...
    setImmediate(async () => {
      try {
        ...
        if (newStatus === "success") {
          ...
          let guestOrderIdForPipeline: string | null = null;
          try {
            const gmeta: any = typeof payment.metadata === "string"
          ...
          if (isGuestOrderPayment && guestOrderIdForPipeline) {
            ...
            try {
              ...
              onPaymentSuccessForServiceOrder(guestOrderIdForPipeline).then(() =>
                console.log(`[MPESA/PAYMENTS CALLBACK] Guest AI generation triggered | orderId=${guestOrderIdForPipeline}`)
              ).catch((err: any) =>
                console.error(`[MPESA/PAYMENTS CALLBACK] Guest AI generation FAILED: ${err?.message}`)
              );
              await storage.updatePayment(payment.id, {
  ...
  app.post("/api/admin/regenerate-cv", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const adminId = req.user?.claims?.sub;
      ...
      let user: any;
      if (userIdOverride) {
        user = await storage.getUserById(String(userIdOverride));
      } else {
        const [row] = await db.select().from(users).where(eq(users.email, rawEmail)).limit(1);
        user = row;
      }
      if (!user) return res.status(404).json({ message: "User not found" });
      ...
      try {
        ...
        await pool.query("UPDATE users SET generated_cv = $1 WHERE id = $2", [cv, user.id]);
        ...
        await storage.logAdminAction(adminId, "regenerate_cv", {
  ...
  app.post("/api/admin/service-order/:orderId/retry", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const orderId = String(req.params.orderId || "").trim();
      if (!orderId) return res.status(400).json({ message: "orderId required" });
      const { rows } = await pool.query(`SELECT id, status, service_slug, output_text FROM service_orders WHERE id = $1`, [orderId]);
      const order = rows[0];
      ...
      await pool.query(`UPDATE service_orders SET status = 'paid', error_message = NULL, output_text = NULL, updated_at = NOW() WHERE id = $1`, [orderId]);
      const mod = await import("./service-order-routes");
      ...
      const { rows: after } = await pool.query(`SELECT status, output_text IS NOT NULL AS has_output, error_message FROM service_orders WHERE id = $1`, [orderId]);
  ...
  app.get("/api/admin/service-orders/stuck", isAuthenticated, isAdmin, async (_req: any, res) => {
    try {
      const { rows } = await pool.query(`
  ...
  app.post("/api/admin/cv-queue/drain", isAuthenticated, isAdmin, async (_req: any, res) => {
    try {
      const { cvQueue } = await import("./lib/cvQueue");
      const waiting  = await cvQueue.getJobs(["waiting", "delayed", "failed"]);
      const summary  = waiting.map((j) => ({ id: j.id, userId: (j.data as any)?.userId, state: j.opts?.delay ? "delayed" : "waiting", attempts: j.attemptsMade }));
      // Promote any delayed/failed back to active by retrying
      for (const j of waiting) {
        try { await j.retry(); } catch { /* job not in retryable state, skip */ }
      }
  ...
  app.get("/api/admin/mpesa/health", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const { getTokenStatus, isMpesaAvailable, isB2CAvailable } = await import("./mpesa");
      const { getReconcilerState } = await import("./mpesa-reconciler");
      ...
      if (!config.shortcode)      missingCore.push("MPESA_SHORTCODE");
      if (!config.consumerKey)    missingCore.push("MPESA_CONSUMER_KEY");
      if (!config.consumerSecret) missingCore.push("MPESA_CONSUMER_SECRET");
      if (!config.passKey)        missingCore.push("MPESA_PASSKEY");
      if (!config.callbackBaseUrl) missingCore.push("APP_URL (callback base)");
      ...
      const volumeRows = await pool.query<{
        window: string; initiated: string; success: string; failed: string; pending: string;
      }>(`
        SELECT * FROM (
      ...
      const stuckRows = await pool.query<{ cnt: string; oldest_age_min: string }>(`
        SELECT
          COUNT(*)::text AS cnt,
      ...
      const recentRows = await pool.query<{
        ...
        created_at: Date; mpesa_code: string | null; service_id: string | null;
      }>(`
      ...
      const lastHour = volume.last_1h as any;
      ...
      if (reconcilerState.lastSuccessAt) {
        const lastSuccessAge = (Date.now() - new Date(reconcilerState.lastSuccessAt).getTime()) / 60_000;
      ...
      res.json({
        ...
        recentPayments: recentRows.rows.map((r) => ({
          ...
          ageMins:   Math.round((Date.now() - new Date(r.created_at).getTime()) / 60_000),
        })),
        generatedAt: new Date().toISOString(),
      });
    } catch (err: any) {
  ...
  app.get("/api/admin/diagnose-user", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      let user: any = null;
      if (rawEmail) {
        const [u] = await db.select().from(users).where(eq(users.email, rawEmail)).limit(1);
        user = u;
      }
      if (!user && rawPhone) {
        // Try the phone in a few common formats (07XX, 254XX, +254XX)
        const candidates = [rawPhone];
        if (rawPhone.startsWith("0"))      candidates.push("254" + rawPhone.slice(1));
        if (rawPhone.startsWith("+254"))   candidates.push(rawPhone.slice(1));
        if (rawPhone.startsWith("254"))    candidates.push("0" + rawPhone.slice(3));
        for (const cand of candidates) {
          const [u] = await db.select().from(users).where(eq(users.phone, cand)).limit(1);
          if (u) { user = u; break; }
        }
      }
      if (!user && rawRef) {
        ...
        const refRow = await db.execute(sql`
          SELECT user_id FROM payments
           ...
           LIMIT 1
        `);
        const refUserId = (refRow as any).rows?.[0]?.user_id;
        if (refUserId) {
          const [u] = await db.select().from(users).where(eq(users.id, refUserId)).limit(1);
          user = u;
        }
      }
...
<body>
  ...
  app.get("/api/admin/users/:id/plan-diagnostic", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      const [
        ...
        pool.query<{
          id: string; email: string; phone: string; plan: string;
          subscription_status: string; created_at: Date; last_login: Date | null;
        }>(`
          SELECT id, email, phone, plan, subscription_status, created_at, last_login
            FROM users WHERE id = $1 LIMIT 1
        `, [userId]),
        pool.query<{
          id: string; status: string; plan: string; end_date: Date | null;
          start_date: Date | null; payment_id: string | null;
        ...
        pool.query<{
          id: string; amount: number; status: string; plan_id: string | null;
          service_id: string | null; method: string;
          mpesa_receipt_number: string | null; created_at: Date;
        ...
        `, [userId]),
        storage.getUserPlan(userId).catch((e: any) => `ERROR: ${e?.message}`),
      ]);
      ...
      if (isLivePlanPaid && activeSub) {
        ...
        const ageMins = Math.round((Date.now() - new Date(lastPaid.created_at).getTime()) / 60_000);
        if (ageMins < 5) {
      ...
      res.json({
        ...
        activeSubscription: activeSub ? {
          ...
          minutesUntilExpiry: activeSub.end_date
            ? Math.round((new Date(activeSub.end_date).getTime() - Date.now()) / 60_000)
            : null,
        } : null,
        ...
        paymentHistory: pays.map((p) => ({
          ...
          failReason: p.fail_reason,
          createdAt: p.created_at,
          ageMinutes: Math.round((Date.now() - new Date(p.created_at).getTime()) / 60_000),
        })),
      });
    } catch (err: any) {
  ...
  app.get("/api/admin/stats", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      async function computeLiveTierBreakdown(): Promise<{
        ...
        try {
          const [totalUsersRow, paidCountsRow] = await Promise.all([
            db.execute(sql`SELECT COUNT(*)::text AS cnt FROM users`),
            db.execute(sql`
              SELECT
          ...
          const totalUsers = Number(((totalUsersRow as any).rows?.[0] ?? {}).cnt ?? 0);
          const counts = ((paidCountsRow as any).rows?.[0] ?? {});
          ...
          const anyPaidRow = await db.execute(sql`
            ...
            WHERE LOWER(status) IN ('success', 'completed', 'paid')
              ...
              AND amount >= 99
          `);
          const anyPaid = Number(((anyPaidRow as any).rows?.[0] ?? {}).cnt ?? 0);
      ...
      for (const row of planRows_rows) {
        ...
        if (p === "yearly")  planBreakdown.yearly  += n;
        else if (p === "monthly") planBreakdown.monthly += n;
        else if (p === "trial")   planBreakdown.trial   += n;
        else planBreakdown.free += n;
      }
  ...
  app.delete("/api/admin/subscriptions/:id", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const subId = String(req.params.id);
      const adminId = req.user?.claims?.sub ?? req.user?.id;
      const reason = String(req.body?.reason || "admin-dedup").slice(0, 200);

      const { rows } = await pool.query<{ user_id: string; plan: string; end_date: Date | null }>(
        `UPDATE user_subscriptions
      ...
      const { rows: stillActive } = await pool.query<{ cnt: string }>(
  ...
  app.get("/api/admin/duplicate-users", isAuthenticated, isAdmin, async (_req: any, res) => {
    try {
      const [sameEmail, multiSubs] = await Promise.all([
        pool.query<{ lower_email: string; user_ids: string[]; emails: string[]; count: string }>(`
        ...
        pool.query<{
          ...
          subscription_ids: string[]; plans: string[]; count: string;
        }>(`
          SELECT us.user_id,
                 u.email,
                 u.first_name,
...
`.trim();
  ...
  app.get("/api/admin/payments/stats", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      const revenueByServiceRows = await pool.query<{
        ...
        WHERE p.status IN ('completed', 'success')
          ${startDate ? `AND p.created_at >= '${(startDate as string).replace(/[^0-9\-T:Z.]/g, '')}'` : ""}
          ${endDate   ? `AND p.created_at <= '${(endDate as string).replace(/[^0-9\-T:Z.]/g, '')}'`   : ""}
  ...
  app.post("/api/agencies/bulk-verify", async (req: any, res) => {
    try {
      ...
      const cap = await checkNeaLookupCap(userId);
      if (!cap.allowed) {
        return res.status(429).json({
      ...
      recordNeaLookup(userId, "bulk-verify").catch(() => { /* logged in helper */ });
  ...
  app.get("/api/nea-agencies/:id", async (req: any, res) => {
    try {
      ...
      recordNeaLookup(userId, "detail").catch(() => { /* logged in helper */ });
...
- Use plain language`;
  ...
  const { registerFraudIntelligenceRoutes } = await import("./routes/fraud-intelligence");
...
- Warm, sharp, protective. Always reinforce trust and safety.`;
  ...
  app.get("/api/admin/portals", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
  ...
  app.patch("/api/admin/portals/:id", isAuthenticated, isAdmin, async (req: any, res) => {
    const id = parseInt(req.params.id);
  ...
  app.delete("/api/admin/portals/:id", isAuthenticated, isAdmin, async (req: any, res) => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) return res.status(400).json({ message: "Invalid portal id" });
    try {
  ...
  app.post("/api/admin/portals/check-now", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
  ...
  app.post("/api/log/client-error", async (req: Request, res: Response) => {
    try {
      ...
      if (!isNoise) {
        ...
        const ref = (rest as any)?.ref ?? "no-ref";
        const errUrl = url ?? filename ?? "unknown";
        const userId = (req as any).user?.id?.toString() ?? "anonymous";
        console.error(
          `[CLIENT-ERROR] ref=${ref} user=${userId} url=${errUrl} msg=${message?.slice(0, 200) ?? ""}`
        );
        if (stack) {
  ...
  // the live apply link.
  // ───────────────────────────────────────────────────────────────────────────
  app.get("/api/dashboard/best-match", async (req: any, res: Response) => {
    try {
      const { VISA_JOBS } = await import("./visa-jobs-routes");
      const { findMatchesFromCachedEmbedding } = await import("./services/jobMatchEmbeddings");
  ...
  app.post("/api/interview/start", aiHotPathLimiter, async (req: any, res: Response) => {
    try {
      ...
      if (!country || !role) return res.status(400).json({ message: "country and role are required." });
      const { startSession } = await import("./services/voiceInterview");
  ...
  app.get("/api/interview/:id", async (req: any, res: Response) => {
    try {
      const userId = (req.session as any)?.customUserId as string | undefined;
      if (!userId) return res.status(401).json({ message: "Please sign in first." });
      const { getSession } = await import("./services/voiceInterview");
...
If your instinct says "3,500", STOP and re-read the SERVICES block above.`;
  ...
  app.post("/api/support/check-payment", isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.post("/api/support/request-refund", isAuthenticated, async (req: any, res) => {
    try {
      const userId: string | undefined = req.user?.claims?.sub ?? req.user?.id;
      if (!userId) return res.status(401).json({ message: "Please sign in first." });
      ...
      if (!payments?.length) return res.status(404).json({ message: "❌ No payment found on your account" });
