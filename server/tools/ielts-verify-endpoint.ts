...
async function overDailyLimit(userId: string | null, guestFp: string): Promise<boolean> {
  ...
  const { rows } = await pool.query<{ n: string }>(
    `SELECT COUNT(*)::text AS n FROM ielts_checks
      WHERE ${idCol} = $1
...
interface ParsedTrf {
  ...
  testType?:       "Academic" | "General Training" | "Life Skills";
  overallBand?:    number;
...
function parseTrf(text: string): ParsedTrf {
  ...
  if (trfMatch) {
    ...
    if (centre) p.testCentreCode = centre[1];
  }

  // Test type
  if (/\bacademic\b/i.test(t))              p.testType = "Academic";
  else if (/\bgeneral training\b/i.test(t)) p.testType = "General Training";
  else if (/\blife skills\b/i.test(t))      p.testType = "Life Skills";

  // Bands: look for "Listening: 7.5", "Reading  8", etc. Also overall.
  const bandRe = (label: string) =>
    new RegExp(`${label}\\s*(?:band\\s*score|score|band)?\\s*[:\\-]?\\s*(\\d(?:\\.\\d)?)`, "i");
  const m1 = t.match(bandRe("listening"));   if (m1) p.listeningBand = Number(m1[1]);
  const m2 = t.match(bandRe("reading"));      if (m2) p.readingBand   = Number(m2[1]);
  const m3 = t.match(bandRe("writing"));      if (m3) p.writingBand   = Number(m3[1]);
  const m4 = t.match(bandRe("speaking"));     if (m4) p.speakingBand  = Number(m4[1]);
  const m5 = t.match(bandRe("overall"));      if (m5) p.overallBand   = Number(m5[1]);
...
function runHeuristics(parsed: ParsedTrf, rawText: string): { findings: Finding[]; verdict: string; confidence: number } {
  ...
  const skills = [parsed.listeningBand, parsed.readingBand, parsed.writingBand, parsed.speakingBand].filter((b) => typeof b === "number") as number[];
  ...
  let verdict:    "likely_genuine" | "suspicious" | "likely_fake" | "undetermined";
    ...
    try {
      ...
      try {
        if (mime.startsWith("image/")) {
          ...
          const r = await extractTextFromBuffer(buf, mime, filename);
      ...
      const { rows: created } = await pool.query<{ id: string }>(
        ...
        [
          ...
          verdict, confidence, JSON.stringify(findings), aiVisionUsed, rawText.slice(0, 10_000),
        ],
      );
