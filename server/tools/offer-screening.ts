...
export const OFFER_SCREENING_VERSION = 1;
...
export interface Finding {
  ...
  message:  string;
  matched?: string;    // The literal snippet that triggered the rule
}

export interface ParsedOfferFields {
  candidateName:  string | null;
  employerName:   string | null;
  positionTitle:  string | null;
  workCountry:    string | null;
  salaryAmount:   string | null;    // Raw as it appears (may include currency + range)
  salaryCurrency: string | null;
  startDate:      string | null;    // YYYY-MM-DD
}

export interface EmployerSignals {
  senderDomain:        string | null;   // Domain of the HR/contact email in the letter
  domainMatchesCompany: boolean | null; // Rough match domain-root vs employer name
  hasLetterhead:       boolean | null;  // From AI vision (null if unknown)
...
export interface AiVisionObservation {
  notes:            string;
  anomalyFlags:     string[];
  visionConfidence: number | null;   // 0-100 estimate of authenticity
  hasLetterhead?:   boolean;
  hasSignature?:    boolean;
}

export interface ScreenOfferInput {
  ocrText:     string;
  parsed:      ParsedOfferFields;
  employer:    EmployerSignals;
  aiVision:    AiVisionObservation | null;
}

export interface ScreenOfferReport {
  version:        typeof OFFER_SCREENING_VERSION;
  riskScore:      number;
  riskBand:       RiskBand;
  findings:       Finding[];
  parsed:         ParsedOfferFields;
...
// ─────────────────────────────────────────────────────────────────────────────
// Rule library — 40+ patterns
//
// Weights are RAW severity contributions; the composite score converts them
// to the 0-100 range in computeRiskScore(). Positive corporate signals
// deduct.
//
// Reused categories from job-scam-checker (same fraud vocabulary applies)
// plus offer-letter-specific patterns (letterhead, sender-vs-company match).
// ─────────────────────────────────────────────────────────────────────────────
...
export function screenOffer(input: ScreenOfferInput): ScreenOfferReport {
  ...
  const hasOfferSignal = offerKeywords.some((k) =>
  ...
  return {
    version:        OFFER_SCREENING_VERSION,
    riskScore,
    riskBand,
    findings,
    parsed:         input.parsed,
...
function computeRiskScore(findings: Finding[], input: ScreenOfferInput): number {
  let score = 0;
  for (const f of findings) score += SEVERITY_WEIGHT[f.severity];

  // Positive corporate signals — deduct.
  if (input.employer.hasLetterhead === true)       score -= 8;
  if (input.employer.hasSignature === true)        score -= 6;
  if (input.employer.hasPhysicalAddress === true)  score -= 5;
  if (input.employer.domainMatchesCompany === true) score -= 8;
...
function classifyVisionSeverity(flag: string): FindingSeverity {
  ...
  if (/tamper|edit|photoshop|forged|forgery|altered|clon(e|ed)|paste|fabricat/.test(f)) return "critical";
  if (/misalign|font|blur|artifact|artefact|watermark|inconsist(ent|ency)|mismatch/.test(f)) return "warning";
  return "info";
}
 ...
 */
export function parseVisibleFields(ocr: string): ParsedOfferFields {
  ...
  let workCountry: string | null = null;
  for (const n of countryNames) {
    if (new RegExp(`\\b${n}\\b`, "i").test(text)) { workCountry = n; break; }
  }
 ...
 */
export function extractSenderDomain(ocr: string, employerName: string | null): {
  ...
  if (!senderDomain) return { senderDomain: null, domainMatchesCompany: null };

  if (!employerName) return { senderDomain, domainMatchesCompany: null };
  ...
  if (!companyRoot) return { senderDomain, domainMatchesCompany: null };

  const matches =
    companyRoot.includes(domainRoot) ||
