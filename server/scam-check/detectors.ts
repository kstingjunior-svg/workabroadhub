...
export interface ScamCheckFinding {
  id: string;
  label: string;
 ...
 */
const COUNTRY_CODES: Record<string, { name: string; expectedMobilePrefixes?: RegExp[] }> = {
...
export function analyzePhone(rawPhone: string, claimedCountry: string | null): ScamCheckFinding[] {
  const findings: ScamCheckFinding[] = [];
  if (!rawPhone) return findings;
  const cleaned = rawPhone.replace(/[^\d+]/g, "");

  // Extract country code
  let countryCode: string | null = null;
  let localPart:   string        = cleaned;
...
export function analyzeUrl(url: string, claimedEmployer: string | null): ScamCheckFinding[] {
  const findings: ScamCheckFinding[] = [];
...
const CHAT_SCAM_PHRASES = [
  { p: "send your passport",            severity: "hard" as const, note: "Real employers never ask for your original passport before verification." },
  { p: "scan of your passport",         severity: "soft" as const, note: "Passport SCAN is normal at final stages, but never in the first messages." },
  { p: "telegram me",                   severity: "soft" as const, note: "Legitimate overseas HR uses email and corporate channels — not Telegram-only." },
  { p: "add me on telegram",            severity: "soft" as const, note: "Same as above." },
  { p: "whatsapp only",                 severity: "soft" as const, note: "WhatsApp-only recruitment is a strong scam pattern." },
  { p: "we don't do interviews",        severity: "hard" as const, note: "Every legitimate overseas job requires at least one interview." },
  { p: "start work in 3 days",          severity: "soft" as const, note: "Real work visas take weeks. Any promise of days-to-departure is fabrication." },
  { p: "no cv needed",                  severity: "hard" as const, note: "Real employers always require a CV." },
  { p: "just send your details",        severity: "soft" as const, note: "Vague personal-info requests before a formal application = phishing pattern." },
  { p: "guaranteed job",                severity: "hard" as const, note: "Nothing about employment is guaranteed. This phrase alone is a scam tell." },
  { p: "airport pickup arranged",       severity: "info" as const, note: "Real employers may arrange this, but scammers use it to sound legitimate — verify everything else too." },
];

export function analyzeText(fullText: string): ScamCheckFinding[] {
  const findings: ScamCheckFinding[] = [];
  if (!fullText) return findings;
...
function looksLikeTypoDomain(domain: string): string | null {
...
export function analyzeEmail(email: string, claimedEmployer: string | null, companyWebsite: string | null): ScamCheckFinding[] {
  const findings: ScamCheckFinding[] = [];
  if (!email) return findings;
