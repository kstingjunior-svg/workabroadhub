...
import { getSalaryTable, classifyJobTitle } from "../offer-verify/salary-benchmarks";
import { detectFraudPatterns, type ExtractedOfferFields } from "../offer-verify/fraud-patterns";
import { analyzePhone, analyzeUrl, analyzeText, analyzeEmail, type ScamCheckFinding } from "./detectors";
...
export interface ScamAnalyzerReport {
  ok: true;
  overallTrust: number;
  confidence: number;
  riskBand: RiskBand;
  ...
  extractedFields: ExtractedOfferFields & {
    ...
    imageObserved?: string | null;
  };
  country: CountryVisaResources | null;
  subScores: SubScore[];
  findings: ScamCheckFinding[];
  positiveIndicators: string[];
  recommendations: string[];
  scamPatternsMatched: string[];
}

export interface ScamAnalyzerFailure {
  ok: false;
  error: string;
  message: string;
}

export type ScamAnalyzerResult = ScamAnalyzerReport | ScamAnalyzerFailure;
...
export async function analyzeScam(input: { text?: string; imageDataUrl?: string }): Promise<ScamAnalyzerResult> {
  ...
  const fields: ExtractedOfferFields & { imageObserved?: string | null } = {
  ...
  // Run every detector
  const findings: ScamCheckFinding[] = [];

  // 1. Universal fraud patterns (reused from offer-letter engine)
  const offerFraud = detectFraudPatterns(fields);
  ...
  if (fields.fullText) findings.push(...analyzeText(fields.fullText));
  ...
  if (salaryCheck) findings.push(salaryCheck);
...
function benchmarkSalary(fields: ExtractedOfferFields, country: CountryVisaResources | null): ScamCheckFinding | null {
  if (!country || !fields.salaryMonthly || !fields.jobTitle) return null;
  const table = getSalaryTable(country.code);
  if (!table) return null;
  const occ = classifyJobTitle(fields.jobTitle);
  if (!occ) return null;
...
function scoreEmployer(f: ExtractedOfferFields): number {
  let s = 15;
  if (f.employerName)   s += 30;
  if (f.companyAddress) s += 20;
  if (f.companyWebsite) s += 20;
  if (f.recruiterName)  s += 15;
  return Math.min(100, s);
}
function scoreAgency(f: ExtractedOfferFields, findings: ScamCheckFinding[]): number {
  return f.recruitmentAgency ? 60 : 70; // no agency = neutral, not a fail
}
function scoreWebsite(f: ExtractedOfferFields, findings: ScamCheckFinding[]): number {
  if (!f.companyWebsite) return 45;
  let s = 90;
  for (const fx of findings) {
    if (fx.category === "url" && fx.severity === "hard") s -= 40;
    if (fx.category === "url" && fx.severity === "soft") s -= 15;
  }
  return Math.max(0, s);
}
function scoreDocument(f: ExtractedOfferFields, findings: ScamCheckFinding[]): number {
  const authCount = [f.hasLetterhead, f.hasSignature, f.hasStamp].filter(Boolean).length;
  if (authCount === 0) return 40;
  if (authCount === 3) return 100;
  return 60 + authCount * 15;
}
function scoreRecruitment(findings: ScamCheckFinding[]): number {
  let s = 85;
  for (const f of findings) {
    if (f.category === "recruitment" && f.severity === "hard") s -= 25;
    if (f.category === "recruitment" && f.severity === "soft") s -= 10;
  }
  return Math.max(0, s);
}
function scoreSalary(findings: ScamCheckFinding[]): number {
  const has = findings.find((f) => f.id === "salary_too_high" || f.id === "salary_too_low");
  if (has) return has.severity === "hard" ? 20 : 55;
  return 80;
}
function scorePayment(findings: ScamCheckFinding[]): number {
  let s = 100;
  for (const f of findings) {
    if (f.category === "payment" && f.severity === "hard") s -= 40;
    if (f.category === "payment" && f.severity === "soft") s -= 15;
    if (f.id === "upfront_fee_requested")     s -= 40;
    if (f.id === "personal_payment_account")  s -= 40;
  }
  return Math.max(0, s);
}
function scoreFraud(findings: ScamCheckFinding[]): number {
  let s = 100;
  for (const f of findings) {
    if (f.severity === "hard") s -= 15;
    else if (f.severity === "soft") s -= 5;
  }
  return Math.max(0, s);
}
function scoreCommunication(findings: ScamCheckFinding[]): number {
  let s = 100;
  for (const f of findings) {
    if ((f.category === "text" || f.category === "identity") && f.severity === "hard") s -= 25;
    if ((f.category === "text" || f.category === "identity") && f.severity === "soft") s -= 8;
  }
  return Math.max(0, s);
}
...
function buildRecommendations(country: CountryVisaResources | null, fields: ExtractedOfferFields, findings: ScamCheckFinding[]): string[] {
  const recs: string[] = [];
...
function describeEmployer(f: ExtractedOfferFields): string {
  const bits: string[] = [];
  if (f.employerName)    bits.push(`name (${f.employerName})`);
...
function describeAgency(f: ExtractedOfferFields): string {
...
function describeDoc(f: ExtractedOfferFields): string {
  const bits: string[] = [];
  if (f.hasLetterhead) bits.push("letterhead");
  if (f.hasSignature)  bits.push("signature");
  if (f.hasStamp)      bits.push("stamp");
  return bits.length ? `Document elements: ${bits.join(" + ")}.` : "No formal document elements (letterhead / signature / stamp) detected.";
}
function describeProcess(findings: ScamCheckFinding[]): string {
...
function describeSalary(f: ExtractedOfferFields, c: CountryVisaResources | null): string {
  if (!f.salaryMonthly) return "No monthly salary figure detected.";
  if (!c) return `${f.salaryMonthly.toLocaleString()} ${f.currency ?? ""}/mo — country unknown, can't benchmark.`;
  return `Offered ${f.salaryMonthly.toLocaleString()} ${f.currency ?? ""}/mo in ${c.name}.`;
}
function describePayment(findings: ScamCheckFinding[]): string {
  const payFindings = findings.filter((f) => f.category === "payment" || f.id === "upfront_fee_requested" || f.id === "personal_payment_account");
  if (payFindings.length === 0) return "No payment red flags in the evidence.";
  return `${payFindings.length} payment-risk indicator(s) detected — see findings.`;
}
function describeFraud(findings: ScamCheckFinding[]): string {
  return findings.length === 0
    ? "No scam patterns detected."
    : `${findings.length} finding(s) in total — see the detailed report.`;
}
function describeComm(findings: ScamCheckFinding[]): string {
...
function inferCategory(id: string): ScamCheckFinding["category"] {
  if (id.includes("email"))          return "identity";
  if (id.includes("payment") || id.includes("fee") || id.includes("bank")) return "payment";
  if (id.includes("website") || id.includes("url") || id.includes("domain")) return "url";
  if (id.includes("phone"))          return "phone";
  if (id.includes("salary") || id.includes("recruitment") || id.includes("agency")) return "recruitment";
  return "text";
}
...
function mapAiError(msg: string): string {
  const lower = msg.toLowerCase();
  if (lower.includes("quota") || lower.includes("insufficient_quota")) return "Our fraud-check AI is temporarily out of capacity. Please try again in a few minutes.";
  if (lower.includes("rate limit") || lower.includes("429")) return "Our fraud-check AI is busy right now. Please try again in a few minutes.";
  if (lower.includes("timeout")) return "The analysis took longer than expected. Try again with less text or a smaller image.";
