...
import type { CountryVisaResources } from "./countries";

export interface RuleCheck {
  /** Machine key for the finding — e.g. "visa_number_format". */
  id: string;
...
/**
 * Fields extracted by the vision model — feed of the rule engine.
 * All optional — the engine handles missing fields gracefully.
 */
 ...
 */
export function runRules(fields: ExtractedFields, country: CountryVisaResources | null): RuleCheck[] {
 ...
 */
export function scoreRules(checks: RuleCheck[]): number {
...
function parseDate(s?: string | null): Date | null {
  ...
  if (m) {
    ...
    if (monthIdx !== undefined) return new Date(Number(m[3]), monthIdx, Number(m[1]));
  }
  return null;
}
