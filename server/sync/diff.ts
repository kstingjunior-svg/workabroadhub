...
import type { NormalizedAgency, ValidatedRecord } from "./types";
import type { CurrentAgencyRow } from "./storage";

// ─────────────────────────────────────────────────────────────────────────────
// Public types
// ─────────────────────────────────────────────────────────────────────────────

/** A new row not previously seen for this provider. */
...
/** An existing row whose fingerprint changed; field_changes lists the deltas. */
...
/** An existing row whose fingerprint matched — touch last_seen_at only. */
export interface UnchangedChange {
  licenseNumber: string;
  agencyId:      string;
}

/** A row present in DB but not in this run — mark deleted-from-source. */
export interface DeletedChange {
  licenseNumber: string;
  agencyId:      string;
  /** Snapshot of current state so Apply can write a full field_changes audit. */
  before:        CurrentAgencyRow;
}

export interface ChangeSet {
  created:   CreatedChange[];
  updated:   UpdatedChange[];
  unchanged: UnchangedChange[];
...
export function computeDiff(
  ...
  for (const [lic, row] of current.entries()) {
    if (seenLicenses.has(lic)) continue;
    deleted.push({
      licenseNumber: lic,
      agencyId:      row.id,
      before:        row,
...
// ─────────────────────────────────────────────────────────────────────────────
// Summary helpers — for log messages and sync_runs counters
// ─────────────────────────────────────────────────────────────────────────────
...
export function countChanges(cs: ChangeSet): ChangeSetCounts {
  return {
