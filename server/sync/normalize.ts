...
import { normalizePhone as kenyaPhoneNormalize } from "../utils/phone";
import {
  SERVICE_TYPES,
  AGENCY_STATUSES,
  type ServiceType,
 ...
 */
export function normalizeLicenseNumber(raw: string | null | undefined): string {
...
export function normalizePhoneNumber(raw: string | null | undefined): string | null {
  ...
  const trimmed = String(raw).trim();
  if (!trimmed) return null;
  const normalised = kenyaPhoneNormalize(trimmed);
 ...
 */
export function normalizeServiceType(raw: string | null | undefined): ServiceType {
  if (raw == null) return "unspecified";
  const s = collapseWhitespace(String(raw)).toUpperCase();
  if (!s || s === "N/A") return "unspecified";

  if (s.includes("BOTH LOCAL") && s.includes("INTERNATIONAL")) return "gulf_and_domestic";
  if (s.includes("INTERNATIONAL") && !s.includes("LOCAL"))      return "gulf";
  if (s.includes("LOCAL"))                                       return "domestic";
  if (s.includes("MEDICAL") || s.includes("HEALTH") || s.includes("NURSING")) return "medical";
  if (s.includes("STUDENT") || s.includes("EDUCATION") || s.includes("AU PAIR")) return "education";
  if (s.includes("SKILLED") || s.includes("PROFESSIONAL"))       return "skilled";

  // Already-canonical pass-through.
  const lower = s.toLowerCase();
  if (SERVICE_TYPE_SET.has(lower)) return lower as ServiceType;
...
const STATUS_SET = new Set<string>(AGENCY_STATUSES);
 ...
 */
export function normalizeStatusSource(raw: string | null | undefined): AgencyStatus {
  if (raw == null) return "unknown";
  const s = collapseWhitespace(String(raw)).toLowerCase();
  if (!s) return "unknown";

  if (s.includes("verified") || s.includes("active") || s === "valid") return "verified";
  if (s.includes("suspend"))                                            return "suspended";
  if (s.includes("expir"))                                              return "expired";
  if (s.includes("revok") || s.includes("cancel"))                      return "revoked";

  // Already-canonical pass-through.
  if (STATUS_SET.has(s)) return s as AgencyStatus;
 ...
 */
export function normalizeDate(raw: string | Date | null | undefined): string | null {
  if (raw == null) return null;
...
function assembleDate(y: string, m: string, d: string): string | null {
...
export function normalizeCountryCode(raw: string | null | undefined): string | null {
  ...
  if (!ISO_COUNTRY_RE.test(s)) return null;
  return s;
}
