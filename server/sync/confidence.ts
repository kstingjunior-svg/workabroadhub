...
import type { DataQualityReport } from "./quality-report";
import type { SafetyVerdict } from "./safety";
import type { SchemaDriftReport } from "./schema-drift";
...
export const CONFIDENCE_SCORE_VERSION = 1;

export type Grade = "A" | "B" | "C" | "D" | "F";

/** One contributing factor with its own 0–100 sub-score + weight. */
export interface ConfidenceFactor {
  key:     string;
  label:   string;
  score:   number;            // 0–100
  weight:  number;            // 0–1; sums to 1 across the report
  notes:   string;            // human-friendly explanation
}

export interface ConfidenceScore {
  version:  typeof CONFIDENCE_SCORE_VERSION;
  score:    number;           // 0–100, rounded
  grade:    Grade;
  factors:  ConfidenceFactor[];
  /** Top three reasons for any deductions, ordered worst-first. */
...
export interface ComputeConfidenceOpts {
  quality:      DataQualityReport;
  safety:       SafetyVerdict;
  /** Null on first-ever run; treated as "no drift signal" → score 100. */
  drift:        SchemaDriftReport | null;
  /** Current health state at the start of the run. */
  health:       ProviderHealth;
  /** Run duration in ms; compared against `expectedDurationMs`. */
  durationMs:   number;
  /** Provider's expected duration (e.g. median of last N runs). Null → 100. */
...
export function computeConfidenceScore(opts: ComputeConfidenceOpts): ConfidenceScore {
  ...
  // ── Factor 1: validity (quarantine rate inverse) ───────────────────────
  {
    ...
    factors.push({
      ...
      score:  round(score),
      weight: CONFIDENCE_WEIGHTS.validity,
  ...
  // ── Factor 2: drift penalty ─────────────────────────────────────────────
  {
    ...
    if (opts.drift) {
      if (opts.drift.matchesPrior) {
        ...
        for (const f of opts.drift.findings) {
          ...
          else if (f.severity === "warning") deduction += 12;
          else deduction += 3;
        }
    ...
    factors.push({
      ...
      score:  round(score),
      weight: CONFIDENCE_WEIGHTS.driftPenalty,
      notes,
    });
  }
  ...
  // ── Factor 3: safety gate (anomaly score inverse + hold = 0) ────────────
  {
    ...
    factors.push({
      ...
      score:  round(score),
      weight: CONFIDENCE_WEIGHTS.safetyGate,
      notes,
    });
  }
  ...
  // ── Factor 4: change ratio (extreme creates/deletes = lower confidence) ─
  {
    ...
    factors.push({
      ...
      score:  round(score),
      weight: CONFIDENCE_WEIGHTS.changeRatio,
  ...
  // ── Factor 5: provider health ───────────────────────────────────────────
  {
    ...
    factors.push({
      ...
      score,
      weight: CONFIDENCE_WEIGHTS.health,
      notes:  `status=${s}`,
    });
  }
  ...
  // ── Factor 6: performance vs expected ──────────────────────────────────
  {
    ...
    factors.push({
      ...
      score:  round(score),
      weight: CONFIDENCE_WEIGHTS.performance,
      notes,
    });
  }
  ...
  return {
    version: CONFIDENCE_SCORE_VERSION,
    score,
    grade,
    factors,
    topDeductions,
