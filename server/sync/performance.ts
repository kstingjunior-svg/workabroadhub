...
export interface PhaseTiming {
  ...
  durationMs: number;
  /** Truthy when the phase threw; the runner still records its duration. */
  errorMessage?: string;
}
...
export interface PerformanceReport {
  ...
  totalMs:    number;
  phases:     PhaseTiming[];
  /** Time spent inside the atomic block specifically. Dashboard-friendly. */
  atomicBlockMs: number;
  /** True when totalMs exceeds 2× the expected duration (if provided). */
...
export class PerformanceRecorder {
  ...
  private readonly hr0:     bigint = process.hrtime.bigint();
   ...
   */
  async withPhase<T>(phase: PhaseName, fn: () => Promise<T> | T): Promise<T> {
   ...
   */
  begin(phase: PhaseName): { commit: (errorMessage?: string) => void } {
    const startedAt = Date.now();
    const start     = process.hrtime.bigint();
    return {
      commit: (errorMessage?: string) => {
        this.timings.push({
          phase,
          startedAt,
          durationMs: nsToMs(process.hrtime.bigint() - start),
  ...
  finalize(opts: { expectedMs?: number | null } = {}): PerformanceReport {
...
// ─────────────────────────────────────────────────────────────────────────────
// Aggregation — pull p50/p95 over historical runs for the dashboard
// ─────────────────────────────────────────────────────────────────────────────
...
export interface PerformanceBaseline {
  ...
  atomicBlockMsP50: number | null;
  atomicBlockMsP95: number | null;
}

/** Pulls the last N successful (non-shadow) runs and computes p50/p95. */
export async function loadPerformanceBaseline(
  providerSlug: string,
  n = 20,
): Promise<PerformanceBaseline> {
  const { pool } = await import("../db");
  const { rows } = await pool.query<{ performance_report: PerformanceReport | null }>(
