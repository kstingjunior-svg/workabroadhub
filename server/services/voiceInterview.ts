...
import { generateVoiceFile } from "../lib/elevenlabs";
...
async function ensureSchema(): Promise<void> {
  ...
  try {
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_interview_sessions_user ON interview_sessions (user_id, created_at DESC);`);
...
function questionPrompt(country: string, role: string, history: QATurn[]): string {
...
function scoringPrompt(country: string, role: string, question: string, answer: string): string {
...
function finalSummaryPrompt(country: string, role: string, transcript: QATurn[]): string {
...
async function scoreAnswer(country: string, role: string, question: string, answer: string) {
  const completion = await openai.chat.completions.create({
    model: "gpt-4o-mini",
...
export async function startSession(args: {
  ...
  await pool.query(
    `INSERT INTO interview_sessions (id, user_id, country, role, transcript, question_number)
     VALUES ($1, $2, $3, $4, $5::jsonb, 0)`,
    [id, args.userId, args.country, args.role, JSON.stringify([{ q: first.text, a: "" }])],
  );
  return {
    id,
    userId: args.userId,
...
export async function respondToSession(args: {
  sessionId: string;
  userId: string;
  answerText: string;
}): Promise<SessionSnapshot> {
  ...
  const { rows } = await pool.query<{
    ...
    transcript: QATurn[];
    created_at: Date;
  }>(`SELECT * FROM interview_sessions WHERE id = $1`, [args.sessionId]);
  const sess = rows[0];
  if (!sess) throw new Error("Session not found");
  if (sess.user_id !== args.userId) throw new Error("Not your session");
  if (sess.status === "completed") throw new Error("Session already completed");

  // Score the current answer (the last entry in transcript whose .a is empty).
  const transcript: QATurn[] = Array.isArray(sess.transcript) ? sess.transcript : [];
  const currentIdx = transcript.findIndex((t) => !t.a);
  if (currentIdx < 0) throw new Error("No pending question");

  const currentQ = transcript[currentIdx].q;
  const cleanAnswer = (args.answerText ?? "").trim().slice(0, 4000);
  if (!cleanAnswer) throw new Error("Answer is empty");
  ...
  let nextQ: { text: string; audioUrl: string | null } | null = null;
  ...
  await pool.query(
    `UPDATE interview_sessions
      ...
      WHERE id = $1`,
    [args.sessionId, JSON.stringify(transcript), answered, status, finalScore, finalSummary],
  );
...
export async function getSession(sessionId: string, userId: string): Promise<SessionSnapshot | null> {
  await ensureSchema();
  const { rows } = await pool.query<any>(`SELECT * FROM interview_sessions WHERE id = $1`, [sessionId]);
  const sess = rows[0];
