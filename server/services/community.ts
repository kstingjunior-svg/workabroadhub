...
export async function postMessage(
  ...
  const insertArgs: any[] = [roomSlug, userId, sanitized, trimmed, stripCount, hidden, hiddenReason];

  let rows: Array<{ id: number; created_at: string }>;
  try {
    const result = await pool.query<{ id: number; created_at: string }>(insertSql, insertArgs);
    rows = result.rows;
  } catch (dbErr: any) {
    ...
    if (code === "42P01" || code === "42703") {
      ...
      try {
        await ensureSchema();
        const retry = await pool.query<{ id: number; created_at: string }>(insertSql, insertArgs);
        rows = retry.rows;
