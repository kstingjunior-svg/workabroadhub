...
export interface ExtractedIeltsFields {
  ...
  testType?: "Academic" | "General Training" | "UKVI" | "Life Skills" | string | null;
  deliveryMethod?: "Paper" | "Computer" | "Online" | string | null;
...
const VALID_BANDS = new Set(
  Array.from({ length: 19 }, (_, i) => i * 0.5), // 0, 0.5, 1, ..., 9
);
 ...
 */
export function computeExpectedOverallBand(l?: number|null, r?: number|null, w?: number|null, s?: number|null): number | null {
  ...
  // .25 → .5, .5 → .5, .75 → 1.0
  if (dec === 0.25) return Math.floor(rounded) + 0.5;
  if (dec === 0.5)  return rounded;
  if (dec === 0.75) return Math.floor(rounded) + 1;
  return rounded;
}
 ...
 */
export function expectedCefr(overall: number | null | undefined): string | null {
  if (overall == null) return null;
  if (overall >= 8.5) return "C2";
  if (overall >= 7.0) return "C1";
  if (overall >= 5.5) return "B2";
  if (overall >= 4.0) return "B1";
  if (overall >= 3.0) return "A2";
  if (overall >= 1.0) return "A1";
  return null;
}

export function runIeltsRules(fields: ExtractedIeltsFields): RuleCheck[] {
  ...
  const bandFields: [string, number|null|undefined][] = [
...
export function scoreIeltsRules(checks: RuleCheck[]): number {
  let s = 100;
  for (const c of checks) {
    if (c.status === "fail") s -= 25;
    else if (c.status === "warn") s -= 8;
  }
  return Math.max(0, s);
}
...
function parseDate(s?: string | null): Date | null {
  ...
  if (m) {
    ...
    if (mi !== undefined) return new Date(Number(m[3]), mi, Number(m[1]));
  }
  return null;
}
