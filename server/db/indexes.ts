...
const INDEX_STATEMENTS: Array<[string, string]> = [
 ...
 */
export async function ensureScalingIndexes(): Promise<void> {
  if (DONE) return;
  DONE = true;
  ...
  console.log(
    `[indexes] boot-time ensure complete: ${created} ensured, ${skipped} skipped (table missing), ${failed} failed in ${ms}ms`,
  );
}
