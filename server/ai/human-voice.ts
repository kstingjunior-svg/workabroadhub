...
// ─── Role vertical → tailored voice + terminology ────────────────────────
//
// Maps common overseas-employment roles to the vocabulary and achievement
// shapes recruiters actually look for. The generator injects the relevant
// block into the prompt so the CV for a farm worker doesn't sound like the
// CV for a pilot.

interface RoleVertical {
  keywords: RegExp;
  label: string;
  voice: string;
}

const ROLE_VERTICALS: RoleVertical[] = [
  {
 ...
 */
export function roleVerticalContext(role: string | null | undefined): string {
  const r = String(role ?? "").trim();
  if (!r) return GENERIC_VERTICAL;
  for (const v of ROLE_VERTICALS) {
    if (v.keywords.test(r)) return v.voice;
  }
...
// ─── Anti-AI-tell post-processor ─────────────────────────────────────────
//
// GPT slips these in even when told not to. We run a fast pass after
// generation to strip the most obvious tells. It doesn't rewrite the
// document — it just fixes the smoke signals that scream "AI wrote this".
