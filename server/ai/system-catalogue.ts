...
export interface CatalogueEntry {
  label:       string;     // human label Nanjila will say
  path:        string;     // route to hand the user
  purpose:     string;     // one-line "use this when..."
  audience?:   "all" | "logged-in" | "paid" | "admin";
}
...
export const SYSTEM_CATALOGUE: CatalogueGroup[] = [
  {
 ...
 * about admin pages.
 */
export function formatCatalogueBlock(opts: { isAdmin: boolean; isPaid: boolean; isLoggedIn: boolean }): string {
  const lines: string[] = [];
  for (const group of SYSTEM_CATALOGUE) {
    const visible = group.entries.filter((e) => {
      if (!e.audience || e.audience === "all") return true;
      if (e.audience === "logged-in") return opts.isLoggedIn;
      if (e.audience === "paid")      return opts.isPaid;
      if (e.audience === "admin")     return opts.isAdmin;
      return true;
    });
