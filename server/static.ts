...
export function serveStatic(app: Express) {
  ...
  const bundleExists = fs.existsSync(distPath) && fs.existsSync(indexHtml);
  ...
  if (!bundleExists) {
    const missing = !fs.existsSync(distPath) ? distPath : indexHtml;
    console.error(
      `[serveStatic][CRITICAL] No frontend bundle at ${missing} — ` +
