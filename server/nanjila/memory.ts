...
export type MemoryCategory = "preference" | "personal" | "career" | "decision";
export type MemorySource   = "user_stated" | "inferred" | "system_derived";
...
export interface RememberInput {
  ...
  confidence:            number;
  source:                MemorySource;
  /** Half-life in days. Preferences fade faster than stable facts. */
  decayHalfLifeDays?:    number;
}

export interface RememberResult {
  ok:      boolean;
  reason?: "sensitive_blocked" | "duplicate_reconfirmed" | "memory_disabled";
  factId?: string;
}
...
const FORBIDDEN_KEY_PATTERNS: Array<{ regex: RegExp; category: string; sensitivity: number }> = [
 ...
 */
export function classifySensitivity(factKey: string, factValue: unknown): {
  sensitivity: number;
...
export const MEMORY_SENSITIVITY_BLOCK_THRESHOLD = 30;
 ...
 */
export async function recallAll(userId: string): Promise<MemoryFact[]> {
  const { rows } = await pool.query(
    `SELECT id, user_id, fact_key, fact_value, category, confidence, sensitivity,
 ...
 */
export async function recallFact(userId: string, factKey: string): Promise<MemoryFact | null> {
  const { rows } = await pool.query(
    `SELECT id, user_id, fact_key, fact_value, category, confidence, sensitivity,
            source, learned_at, last_verified_at, decay_half_life_days, archived
       FROM nanjila_user_memory
...
export async function rememberFact(input: RememberInput): Promise<RememberResult> {
  if (!NanjilaFlags.memoryEnabled) {
    return { ok: false, reason: "memory_disabled" };
  }

  // ── 1. Sensitivity gate ─────────────────────────────────────────────────
  const { sensitivity, category: sensitiveCategory } =
    classifySensitivity(input.factKey, input.factValue);
  if (sensitivity > MEMORY_SENSITIVITY_BLOCK_THRESHOLD) {
    console.warn(
      `[Nanjila/Memory] BLOCKED sensitive fact for user=${input.userId} ` +
  ...
  if (existing) {
    ...
    if (sameValue) {
      await pool.query(
        `UPDATE nanjila_user_memory
            SET last_verified_at = NOW(),
                confidence       = GREATEST(confidence, $2)
          WHERE id = $1`,
        [existing.id, Math.round(Math.max(0, Math.min(100, input.confidence)))],
      );
      return { ok: true, reason: "duplicate_reconfirmed", factId: existing.id };
    }
    // Different value — archive old, insert new.
    await pool.query(
      `UPDATE nanjila_user_memory SET archived = TRUE WHERE id = $1`,
      [existing.id],
    );
  }
  ...
  const { rows } = await pool.query<{ id: string }>(
    `INSERT INTO nanjila_user_memory
 ...
 */
export async function forgetFact(userId: string, factKey: string): Promise<boolean> {
 ...
 */
export async function clearAllFor(userId: string): Promise<number> {
  const { rowCount } = await pool.query(
    `UPDATE nanjila_user_memory
        SET archived = TRUE
      WHERE user_id = $1 AND archived = FALSE`,
 ...
 */
export async function buildPromptContext(userId: string): Promise<string> {
  if (!NanjilaFlags.memoryEnabled) return "";
  const facts = await recallAll(userId);
  const usable = facts.filter((f) =>
    f.confidence >= 60 && f.sensitivity <= MEMORY_SENSITIVITY_BLOCK_THRESHOLD
  );
...
function defaultHalfLife(category: MemoryCategory): number {
  switch (category) {
