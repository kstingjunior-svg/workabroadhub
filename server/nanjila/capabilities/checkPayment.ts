...
export interface CheckPaymentOutput {
  ok:             boolean;
  found:          boolean;
  status?:        string;
  amount?:        number;
  ...
  message?:       string;   // Human-facing summary Nanjila can quote
...
async function checkPaymentHandler(
  input: CheckPaymentInput,
  ctx:   CapabilityContext,
): Promise<CheckPaymentOutput> {
  ...
  if (input.paymentId) {
    const { rows } = await pool.query(
      `SELECT id, user_id, status, amount, currency, method,
...
function composeStatusSentence(
  status:          string | null,
...
export const checkPaymentCapability: CapabilityDefinition<CheckPaymentInput, CheckPaymentOutput> = {
  slug:        "check_payment",
