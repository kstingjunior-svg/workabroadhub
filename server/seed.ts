...
export async function seedDatabase() {
  ...
  await db.insert(services).values([
    ...
    // --- Job Search Tools ---
    {
      ...
      category: "Job Search Tools", badge: "What most Kenyans pick",
...
const CANONICAL_SERVICES = [
  // --- CV & Documents ---
  // 2026-06: voice rewrite. No "AI", no "ATS", no template language.
  // Sound like a Kenyan friend who's been through it.
  { slug: "cv_check",          name: "CV Health Check",                price: 0,    currency: "KES", isActive: true, order: 0,  category: "CV & Documents",       badge: null,           isSubscription: false, subscriptionPeriod: null, description: "Is your CV getting ignored? Upload it and we'll show you exactly what recruiters see — and what's missing. Takes 3 minutes. Costs nothing.", features: ["Recruiter-friendliness score","What's missing (and why it matters)","Format & length sanity check","Honest feedback in minutes","Completely free"] },
  { slug: "cv_fix_lite",       name: "The Quick Fix",                  price: 99,   currency: "KES", isActive: true, order: 1,  category: "CV & Documents",       badge: null,           isSubscription: false, subscriptionPeriod: null, description: "Grammar, structure, formatting — cleaned up in minutes. The price of two mandazi. What a typist would charge: KES 1,500.", features: ["Grammar & spelling sorted","Cleaner structure & formatting","Reads professional in 3 minutes","Best for entry-level CVs","In your inbox fast"] },
  { slug: "ats_cv_optimization",name: "Recruiter-Friendly CV",         price: 499,  currency: "KES", isActive: false, order: 4,  category: "CV & Documents",       badge: "Popular",      isSubscription: false, subscriptionPeriod: null, description: "Get past the gatekeepers. We tune your CV with the words recruiters search for so it actually reaches a human.", features: ["Keywords matched to your target role","Format that recruiter software actually reads","Industry-specific phrasing","Before/after report so you see the change","Word + PDF, both yours"] },
  { slug: "cv_rewrite",        name: "Country-Specific CV Rewrite",    price: 699,  currency: "KES", isActive: false, order: 5,  category: "CV & Documents",       badge: "Our pick",   isSubscription: false, subscriptionPeriod: null, description: "Different country, different rules. We rewrite your CV the way employers in your target country actually expect it.", features: ["Native format for UAE, UK, Canada, EU","Rewritten by a career expert who's done it","Your transferable skills front and centre","Clean, recruiter-friendly layout","Word + PDF"] },
  { slug: "cover_letter",      name: "Cover Letters That Get Read",    price: 149,  currency: "KES", isActive: false, order: 3,  category: "CV & Documents",       badge: null,           isSubscription: false, subscriptionPeriod: null, description: "Not the generic \"I am writing to apply\" nonsense. We write it like you're already in the room. ≈ 2 mandazi. Worth it.", features: ["Custom letter per role and country","Voice that sounds like a human","Handles the visa + relocation question naturally","Your best bits up top","In your inbox in minutes"] },
  { slug: "sop_writing",       name: "Your Story, On Paper",           price: 999,  currency: "KES", isActive: true, order: 8,  category: "CV & Documents",       badge: null,           isSubscription: false, subscriptionPeriod: null, description: "Universities read thousands of these. Yours needs to make them stop scrolling. We've helped 300+ Kenyans get into schools abroad.", features: ["Personal statement that opens with a hook","Tailored to your course and university","Real academic motivation, not template lines","Proofread for grammar & flow","Word + PDF delivery"] },
  ...
  // --- Job Search Tools ---
  { slug: "job_pack_5",          name: "Job Pack — 5 Applications",   price: 1299, currency: "KES", isActive: true, order: 11, category: "Job Search Tools",     badge: "What most Kenyans pick", isSubscription: false, subscriptionPeriod: null, description: "AI-tailored CV + cover letter for 5 jobs in one pack — apply to more, faster", features: ["Tailored CV for each of 5 jobs","Matching cover letter per application","ATS-optimized per job description","Application tracking dashboard","⚡ AI delivery — under 10 minutes"] },
  ...
  { slug: "work_permit_uae_mid",   name: "UAE Work Permit Assist + Form Pre-fill", price: 599, currency: "KES", isActive: true, order: 34, category: "Work Permits", badge: "Our pick", isSubscription: false, subscriptionPeriod: null, description: "Light guide + we pre-fill your Tasheel forms using your CV and intake data, give you the attestation checklist for Kenyan documents, and prep your medical fitness booking", features: ["All Light guide content included","Tasheel form pre-filled","KE document attestation step-by-step (MFA → UAE embassy)","Medical fitness pre-booking instructions","Free-zone routing recommendations","Delivered within 24h"] },
  ...
  { slug: "work_permit_canada_mid",   name: "Canada Work Permit Assist + Form Pre-fill", price: 599, currency: "KES", isActive: true, order: 40, category: "Work Permits", badge: "Our pick", isSubscription: false, subscriptionPeriod: null, description: "Light guide + we pre-fill your IMM forms using your CV/intake, map your work history to NOC codes, and lay out your ECA & IELTS preparation timeline", features: ["All Light guide content included","IMM forms pre-filled (5710 / 1295 as applicable)","NOC code mapping for your work history","ECA + IELTS prep timeline","CRS optimisation tips","Delivered within 24h"] },
...
export async function seedCountryPortals(): Promise<void> {
  try {
    // 1. Ensure all six destination countries exist (lowercase codes).
    const wantedCountries: Array<{ code: string; name: string; flag: string }> = [
    ...
    const { COUNTRY_PORTALS: portalsByCode } = await import("./lib/country-portals");
