...
httpServer.listen(PORT, "0.0.0.0", () => {
  ...
  (async () => {
    ...
    try {
      const { startStuckOrderSweep } = await import("./service-order-routes");
    ...
    // process restarts.
    try {
      const { startReadinessWorker, scheduleNightlyReadiness } =
        await import("./nanjila/jobs/nightlyReadiness");
    ...
    try {
      const { startVerificationReminderSweep } = await import("./lib/verification-reminder-sweep");
    ...
    try {
      const { startPaidButFreeReconciler } = await import("./lib/paid-but-free-reconciler");
    ...
    try {
      const { startServiceOrderReconciler } = await import("./lib/service-order-reconciler");
...
(async () => {
  try {
    ...
    try {
      const { validateConfigOrPanic } = await import("./sync/hardening");
    ...
    try {
      const { registerIeltsRoutes } = await import("./routes/ielts-routes");
    ...
    try {
      app.post("/api/admin/mpesa/reconcile-now", async (req: any, res: any) => {
    ...
    try {
      app.post("/api/payments/recover-mine", async (req: any, res: any) => {
        const userId = req?.user?.claims?.sub ?? req?.user?.id ?? (req?.session as any)?.customUserId;
        if (!userId) return res.status(401).json({ message: "Please sign in first." });
        try {
          const { recoverMyPayment } = await import("./lib/paid-but-free-reconciler");
    ...
    try {
      const { ensurePlansSeeded } = await import("./lib/ensure-plans-seeded");
    ...
    try {
      const { ensureLithuaniaSeeded } = await import("./lib/ensure-lithuania-seeded");
    ...
    try {
      const { ensureNeaAgenciesSeeded } = await import("./lib/ensure-nea-agencies-seeded");
