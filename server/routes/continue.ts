/**
 * GET /api/me/continue — "Pick up where you left off"
 *
 * Aggregates signals from the user's active state and returns the SINGLE
 * highest-priority unfinished action so the dashboard can render a friendly
 * "continue your X" card. Priority order:
 *
 *   1. Service order paid/processing without output → "Your CV is generating"
 *   2. Service order completed but not downloaded → "Download your CV"
 *   3. Mid-interview session (in_progress, transcript has data) → "Resume your interview"
 *   4. Country journey with progress 1-99% → "Continue your UAE journey"
 *   5. Any newly-completed journey (last 7 days) → "🎉 You finished UAE"
 *
 * If NO unfinished action exists, returns { continue: null } and the widget
 * hides itself.
 *
 * Resilient to missing tables — any source that throws is silently skipped.
 *
 * 2026-06 retention #4.
 */
...
import { getJourneySteps, SUPPORTED_JOURNEY_COUNTRIES } from "@shared/country-journey-steps";
...
interface ContinueCandidate {
  kind:
    ...
    | "journey_progress"
    | "journey_finished";
  priority: number;          // higher = pick this
  href: string;              // where the CTA sends them
  headline: string;          // "Continue your UAE journey"
  subhead: string;           // "5 of 12 steps done · last touched today"
  icon: "briefcase" | "mic" | "globe" | "trophy" | "sparkles" | "clock" | "download";
  accent: "emerald" | "blue" | "amber" | "rose" | "indigo" | "cyan";
  // Optional progress 0-100 the client renders as a bar
...
const COUNTRY_NAME: Record<string, string> = Object.fromEntries(
  SUPPORTED_JOURNEY_COUNTRIES.map((c) => [c.code, c.name]),
);

const COUNTRY_FLAG: Record<string, string> = Object.fromEntries(
  SUPPORTED_JOURNEY_COUNTRIES.map((c) => [c.code, c.flag]),
);

function humanAgo(d: Date | string | null | undefined): string {
  ...
  if (sec < 7 * 86400) return `${Math.floor(sec / 86400)} day${Math.floor(sec / 86400) === 1 ? "" : "s"} ago`;
  return new Date(ts).toLocaleDateString("en-KE");
}

// ─── Individual signal sources ────────────────────────────────────────────

async function getServiceOrderCandidate(userId: string): Promise<ContinueCandidate | null> {
  try {
    // First check for a paid/processing order (most urgent — user is waiting)
    const processing = await pool.query<{ id: string; service_slug: string; service_name: string; status: string; created_at: Date }>(
      `SELECT id, service_slug, service_name, status, created_at
         FROM service_orders
        WHERE user_id = $1
    ...
    const completed = await pool.query<{ id: string; service_slug: string; service_name: string; completed_at: Date }>(
      `SELECT id, service_slug, service_name, completed_at
         FROM service_orders
        WHERE user_id = $1
    ...
    if (!isMissingTable(err)) console.warn("[continue/service]", err?.message);
    return null;
  }
}

async function getInterviewCandidate(userId: string): Promise<ContinueCandidate | null> {
  try {
    const { rows } = await pool.query<{ id: string; country: string; role: string; transcript: any; created_at: Date }>(
      `SELECT id, country, role, transcript, created_at
    ...
    const answered = transcript.filter((t: any) => t?.a).length;
    const total = 5;
    if (answered === 0) {
    ...
    if (!isMissingTable(err)) console.warn("[continue/interview]", err?.message);
    return null;
  }
}
...
async function getJourneyCandidate(userId: string): Promise<ContinueCandidate | null> {
  try {
    ...
    const allSteps = getJourneySteps(r.country_code);
    ...
    if (pct === 100) {
      const finishedRecently = r.last_touched_at && Date.now() - new Date(r.last_touched_at).getTime() < 7 * 86400_000;
      if (finishedRecently) {
        return {
    ...
    if (!isMissingTable(err)) console.warn("[continue/journey]", err?.message);
    return null;
  }
}

export function registerContinueRoute(app: Express): void {
  app.get("/api/me/continue", isAuthenticated, async (req: any, res: Response) => {
    try {
      const userId = getUserId(req);
      if (!userId) return res.status(401).json({ continue: null });
      ...
      const candidates = [svc, interview, journey].filter(Boolean) as ContinueCandidate[];
