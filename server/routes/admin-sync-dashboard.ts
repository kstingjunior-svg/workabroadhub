...
export function registerSyncDashboardRoutes(
  app: Express,
  isAuthenticated: any,
  isAdmin: any,
): void {
    ...
    async (_req, res: Response) => {
      try {
        const [providers, lastRunByProvider, last24h, recentAnomalies] = await Promise.all([
          ...
          pool.query<{
            total: string; succeeded: string; failed: string; held: string;
  ...
  app.get(
    ...
    async (req, res: Response) => {
      try {
        ...
        let i = 1;
        if (provider) { where.push(`p.slug = $${i++}`); params.push(provider); }
        ...
        const [run, anomalies, snapshot] = await Promise.all([
          pool.query(`
            SELECT r.*, p.slug AS provider_slug, p.display_name AS provider_name
              ...
              JOIN sync_providers p ON p.id = r.provider_id
             WHERE r.id = $1`, [runId]),
          pool.query(`SELECT * FROM sync_anomalies WHERE run_id = $1 ORDER BY created_at`, [runId]),
          pool.query(`SELECT id, byte_size, checksum_sha256, validated_count, quarantined_count, created_at
  ...
  app.get(
    ...
    async (req, res: Response) => {
      try {
        ...
        const params: unknown[] = [];
        let i = 1;
        if (runId) { where.push(`run_id = $${i++}`); params.push(runId); }
  ...
  app.get(
    "/api/admin/sync/providers/:slug/health",
    isAuthenticated, isAdmin,
    async (req, res: Response) => {
      try {
        ...
        const { rows: healthEvents } = await pool.query(
