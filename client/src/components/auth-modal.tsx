...
import { X, Eye, EyeOff, Loader2, Check, AlertCircle, UserPlus, LogIn, ExternalLink } from "lucide-react";
...
export function AuthModal({ open, onClose, defaultTab = "login", redirectPath }: AuthModalProps) {
  ...
  const resetForm = () => {
    ...
    setServerError(""); setFieldErrors({}); setSuccessMsg("");
  };
  ...
  const validate = (): boolean => {
    ...
    if (tab === "signup" && firstName.trim().length < 2) errs.firstName = "Name must be at least 2 characters";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        onClose();
        resetForm();
        const dest = redirectPath || localStorage.getItem("auth_redirect") || "/dashboard";
      ...
      >
        ...
        <div className="p-6 pb-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-semibold text-primary">WorkAbroad Hub</span>
          </div>
        ...
        <div className="flex mx-6 mt-4 rounded-lg bg-muted p-1 gap-1">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 text-sm font-medium py-2 rounded-md transition-all ${tab === "login" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 text-sm font-medium py-2 rounded-md transition-all ${tab === "signup" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {serverError && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm" data-testid="auth-error-msg">
              ...
              <div>
                {serverError}
                {serverError.includes("Replit") && (
                  <a href="/api/login" className="flex items-center gap-1 mt-1 font-medium underline">
                    Continue with Replit <ExternalLink className="h-3 w-3" />
                  </a>
                )}
              </div>
            </div>
          ...
          {tab === "signup" && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="auth-firstName">First name <span className="text-red-500">*</span></Label>
                <Input
                  id="auth-firstName"
          ...
          <Button
            ...
            data-testid={tab === "login" ? "btn-signin-submit" : "btn-signup-submit"}
          >
            {loading ? (
              <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> {tab === "login" ? "Signing in…" : "Creating account…"}</>
            ) : (
          ...
          <p className="text-center text-sm text-muted-foreground">
            {tab === "login" ? (
              <>Don't have an account?{" "}
                <button type="button" onClick={() => switchTab("signup")} className="text-primary font-medium hover:underline" data-testid="link-switch-to-signup">
                  Sign up free
                </button>
              </>
            ) : (
              <>Already have an account?{" "}
                <button type="button" onClick={() => switchTab("login")} className="text-primary font-medium hover:underline" data-testid="link-switch-to-login">
                  Sign in
                </button>
              </>
            )}
